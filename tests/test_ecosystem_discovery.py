...
def _client(handler, **kwargs) -> EcosystemDiscoveryClient:
    return EcosystemDiscoveryClient(
...
def test_modrinth_search_uses_exact_facets_and_bound_offset_pagination() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert json.loads(request.url.params["facets"]) == [
            ["versions:1.20.1"],
            ["categories:fabric"],
            ["project_type:mod"],
            ["open_source:true"],
        ...
        return httpx.Response(
            ...
            json={
                ...
                "hits": [
                    {
                        "project_id": f"project-{offset}",
                        "slug": f"project-{offset}",
                        "project_type": "mod",
                        "title": f"Project {offset}",
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "items": [
                        {
                            "id": 1,
                            "full_name": "example/fabric-library",
                            "html_url": "https://github.com/example/fabric-library",
                            "description": "Library",
                            "default_branch": "main",
                            "archived": False,
                            "license": {"spdx_id": "MIT"},
            ...
            json=[
                {
                    ...
                    "files": [
                        {
                            "filename": "exact-project.jar",
                            "url": "https://cdn.modrinth.com/data/abc/versions/v1/exact.jar",
                            "size": 123456,
                            "primary": True,
...
def test_modrinth_inspection_does_not_accept_off_target_or_ambiguous_files() -> None:
            ...
            json=[
                {
                    ...
                    "files": [
                        {
                            "filename": "one.jar",
                            "url": "https://cdn.modrinth.com/data/one.jar",
                            "size": 1,
                            "primary": True,
                            "hashes": {"sha1": "1" * 40, "sha512": "a" * 128},
                        },
                        {
                            "filename": "two.jar",
                            "url": "https://cdn.modrinth.com/data/two.jar",
                            "size": 1,
                            "primary": True,
                            "hashes": {"sha1": "2" * 40, "sha512": "b" * 128},
                        },
                    ],
                }
            ],
    ...
    assert "off_target_minecraft_version_or_loader" in version[
        "unresolved_or_rejected_gates"
    ]
    assert "exactly_one_primary_file_is_required" in version[
        "unresolved_or_rejected_gates"
    ]
                ...
                "items": [
                    {
                        ...
                        "html_url": "https://github.com/example/no-license",
                        "archived": False,
                        "license": None,
                    },
                    {
                        "id": 2,
                        "full_name": "example/noassertion",
                        "html_url": "https://github.com/example/noassertion",
                        "archived": False,
                        "license": {"spdx_id": "NOASSERTION"},
                    },
                    {
                        "id": 3,
                        "full_name": "example/reviewable",
                        "html_url": "https://github.com/example/reviewable",
                        "description": "Reviewable source",
                        "default_branch": "main",
                        "archived": False,
                        "license": {"spdx_id": "BSD-3-Clause"},
                        "topics": ["minecraft", "fabric"],
                    },
                ],
            },
        )
...
def test_github_inspection_pins_commit_and_hashes_license_without_cloning() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                ...
                "html_url": "https://github.com/example/reviewable/blob/main/LICENSE",
                "license": {"spdx_id": "MIT"},
            },
        )
    ...
    assert result["license_text_sha256"] == (
...
def test_openverse_keeps_license_attribution_and_origin_verification_without_download() -> None:
        ...
        return httpx.Response(
            ...
            json={
                ...
                "results": [
                    {
                        "id": "museum-ship",
                        "title": "Historic sailing ship",
                        "creator": "Example Museum",
                        "license": "by",
                        "license_version": "4.0",
                        "license_url": "https://creativecommons.org/licenses/by/4.0/",
                        "foreign_landing_url": "https://museum.example/items/ship",
                        "detail_url": "https://api.openverse.org/v1/images/museum-ship/",
                        "thumbnail": "https://images.openverse.org/thumb.jpg",
                        "attribution": "Historic sailing ship by Example Museum, CC BY 4.0",
                        "provider": "wikimedia",
                        "source": "wikimedia",
                        "mature": False,
                    }
    ...
    assert candidate["license_policy"] == (
        "origin_verification_and_attribution_required"
    )
    assert page["download_performed"] is False
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Build a peaceful farming and cooking expansion",
            {
                "title": "Harvest Table",
                "pitch": "Seasonal crops and village recipes",
                "modules": [{"reason": "soil fertility and crop rotation"}],
                "assets": [{"brief": "hand-painted kitchen tools"}],
            },
            ("farming", "seasonal crops", "soil fertility"),
        ),
        (
            "Create an orbital factory automation game",
            {
                "title": "Red Orbit",
                "pitch": "Zero-gravity logistics",
                "modules": [{"reason": "power grids and conveyor routing"}],
                "assets": [{"brief": "industrial space station panels"}],
            },
            ("orbital factory", "zero-gravity logistics", "power grids"),
        ),
        (
            "조선 시대 탐정 추리 모드를 만들어줘",
            {
                "title": "한양의 그림자",
                "pitch": "증거 수집과 인물 심문",
                "modules": [{"reason": "단서 연결과 평판 변화"}],
                "assets": [{"brief": "한옥과 관아 참고 자료"}],
            },
            ("조선 시대", "증거 수집", "단서 연결"),
        ),
    ],
)
def test_seed_bundle_preserves_generic_unrelated_genre_terms(
    monkeypatch,
    prompt: str,
    game_design: dict,
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.host == "api.modrinth.com":
            queries[request.url.path] = request.url.params["query"]
            return httpx.Response(200, json={"total_hits": 0, "hits": []})
        queries[request.url.path] = request.url.params["q"]
        return httpx.Response(
            200,
            json={"result_count": 0, "page_count": 0, "results": []},
        )

    monkeypatch.setenv("MMM_ECOSYSTEM_DISCOVERY", "auto")
    bundle = discover_seed_bundle(
        prompt,
        game_design,
        client=_client(handler),
    )

    mod_query = queries["/v2/search"].lower()
    assert all(term.lower() in mod_query for term in expected_terms)
    assert "boss" not in mod_query
    assert "보스" not in mod_query
    assert "visual reference texture" in queries["/v1/images/"]
    assert "ambience sound effects music" in queries["/v1/audio/"]
    assert bundle["download_performed"] is False
    assert bundle["authorization"] == "none"


def test_seed_bundle_pages_a_large_route_catalog_without_a_global_cap(
    monkeypatch,
) -> None:
    class Discovery:
    ...
    second = discover_seed_bundle(
        "Build every requested system",
        {"title": "Large"},
        research_brief=brief,
        client=discovery,
        route_cursor=first["next_route_cursor"],
        route_limit=7,
    )
