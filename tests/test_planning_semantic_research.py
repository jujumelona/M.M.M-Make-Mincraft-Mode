...
def requirement():
    return {"decision_type": "requirement", "requirement_id": "req_1",
            "semantic_capability": "spacecraft upgrade", "statement": "Players trade to upgrade spacecraft.",
            "acceptance": ["Players can trade to upgrade spacecraft."]}


def pool_for(*bodies):
    return global_grounded_pool({"r_1": {"queries": [{"query": "spacecraft", "evidence_records": [
        {"source_id": f"modrinth:{index}", "content": body, "url": f"https://example.com/{index}"}
...
def observation(req, pool, *, excerpt="Players trade to upgrade spacecraft.", index=0):
    ...
    return {"requirement_sha256": fingerprint(req), "obligation_index": index,
            "source_id": record["source_id"], "content_sha256": semantic._body_sha(record["content"]),
            "window_start": 0, "window_end": len(record["content"]),
            "supports": True, "entailment_verified": True, "excerpt": excerpt, "reason": "Trading changes the ship's performance."}


def review_for(req, pool, observations):
    return {"schema_version": "mmm/semantic-research-review-v1", "requirement_sha256": fingerprint(req),
            "pool_sha256": fingerprint(pool), "observations": observations}
...
@pytest.mark.parametrize("key,value", [
    ("excerpt", "Invented upgrade API"), ("content_sha256", "sha256:stale"),
    ("source_id", "modrinth:other"), ("window_end", 5), ("obligation_index", True),
    ("supports", "true"), ("reason", ""), ("requirement_sha256", "sha256:old"),
])
def test_host_rejects_forged_or_mismatched_observations(key, value):
    req = requirement(); pool = pool_for("Players trade to upgrade spacecraft.")
    obs = observation(req, pool); obs[key] = value
    assert not semantic.validate_semantic_review(req, pool, review_for(req, pool, [obs]))["complete"]


def test_partial_acceptance_and_stale_source_fail_resume():
    req = requirement(); req["acceptance"].append("Players can hire crew members.")
    ...
    changed = deepcopy(pool); changed["queries"][0]["evidence_records"][0]["content"] += " Changed."
...
def test_query_expansion_keeps_topic_and_never_searches_prose_words_alone():
    req = requirement(); req["statement"] += " mine other special resources performance"
...
def test_dedup_preserves_all_source_origins_and_queries():
    ...
    pool = global_grounded_pool({f"r_{i}": {"queries": [{"query": f"query {i}", "evidence_records": [record]}]}
                                 for i in range(10)})
...
def test_small_model_native_tool_calls_have_atomic_schema_and_bounded_input(monkeypatch, tmp_path):
    ...
    class Router:
        ...
        def generate_tool_decision(self, role, messages, **kwargs):
            ...
            return {"supports": False, "excerpt": "", "reason": "No full obligation support"}
    monkeypatch.setattr(semantic, "request_message_budget", lambda *a: 4096)
    req = requirement(); pool = pool_for("Spacecraft documentation.\n" * 1000)
    result = semantic.review_requirement_sources(Router(), req, pool, requirement_candidate_trace(req, pool))
    assert len(calls) > 1
...
def test_log_scale_pool_cannot_expand_mandatory_planner_input(monkeypatch):
    req = requirement()
    pool = pool_for("Players trade to upgrade spacecraft.", *["Unrelated body " * 200 for _ in range(15167)])
    review = semantic.validate_semantic_review(req, pool, review_for(req, pool, [observation(req, pool)]))
    state = {"decisions": [req], "task_candidate_pool": pool,
             "research_queue": [{"research_id": "r_1", "requirement_ref": "req_1", "status": "complete"}],
             "evidence": [{"research_ref": "r_1", "sufficient": True,
                           "claims": ["x" * 50636559], "candidate_trace": {"semantic_review": review}}]}
    ...
    with pytest.raises(CapturedModelBoundary):
        fragments.generate_section_records(object(), requirement=req, criterion=req["acceptance"][0],
            section="state_model", evidence=evidence, allowed_refs=refs)
...
def test_each_obligation_budget_does_not_include_all_sibling_obligations(monkeypatch):
    ...
    monkeypatch.setattr(semantic, "generate_fixed_template_value", lambda *a, **k: {
        "supports": False, "excerpt": "", "reason": "Not enough detail"})
    result = semantic.review_requirement_sources(object(), req, pool, requirement_candidate_trace(req, pool))
    assert not result["complete"]
...
def test_review_restarts_from_durable_observations(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    calls = []
    req = requirement(); pool = pool_for("Spacecraft note A.", "Spacecraft note B.")
    router = SimpleNamespace(profile="fixture", registry=SimpleNamespace(role=lambda *a: SimpleNamespace()))
    def model(*args, **kwargs):
        text = args[2][1]["content"]
        calls.append(text)
        if "note B" in text and len(calls) == 2:
            raise TimeoutError("interrupted")
        return {"supports": False, "excerpt": "", "reason": "Not supported"}
    monkeypatch.setattr(semantic, "generate_fixed_template_value", model)
    with pytest.raises(Exception, match="interrupted"):
        semantic.review_requirement_sources(router, req, pool, requirement_candidate_trace(req, pool))
    result = semantic.review_requirement_sources(router, req, pool, requirement_candidate_trace(req, pool))
    assert not result["complete"]
    assert len(calls) == 3
...
def test_extending_pool_preserves_previously_completed_review(monkeypatch):
    ...
    state, _, _ = _run_collection(monkeypatch, _state(), [{"r_001": grounded("spacecraft upgrade") }])
    state["decisions"].append(req_fixture("req_002"))
    state["research_queue"].append({"research_id": "r_002", "requirement_ref": "req_002",
                                    "queries": ["new"], "status": "pending", "source_kinds": ["existing_mods"],
                                    "resolves": []})
    updated, _, _ = _run_collection(monkeypatch, state, [{"r_002": grounded("spacecraft upgrade new", "modrinth:new")}])
    from minecraft_mod_ai.planning_candidate_evidence import (
...
def test_exact_quote_requires_independent_entailment_check(monkeypatch):
    req = requirement(); pool = pool_for("Spacecraft upgrades are not supported.")
    calls = []
    def model(*args, **kwargs):
        calls.append(kwargs["tool_name"])
        return {"supports": len(calls) == 1, "excerpt": "Spacecraft upgrades are not supported.",
                "reason": "The source negates the requested behavior."}
    monkeypatch.setattr(semantic, "generate_fixed_template_value", model)
    result = semantic.review_requirement_sources(object(), req, pool, requirement_candidate_trace(req, pool))
...
def test_verifier_sees_negating_header_not_only_cherry_picked_quote(monkeypatch):
    ...
    def model(*args, **kwargs):
        ...
        if kwargs["tool_name"] == "verify_requirement_entailment":
            ...
            return {"supports": False, "excerpt": quote, "reason": "The header negates support."}
        return {"supports": True, "excerpt": quote, "reason": "Incorrect initial interpretation"}
    monkeypatch.setattr(semantic, "generate_fixed_template_value", model)
    result = semantic.review_requirement_sources(object(), req, pool, requirement_candidate_trace(req, pool))
    assert not result["complete"]
...
def test_backend_endpoint_change_and_malformed_cache_do_not_reuse_verdict(monkeypatch, tmp_path):
    ...
    router = SimpleNamespace(profile="test", registry=SimpleNamespace(role=lambda *a: config))
    calls = []
    monkeypatch.setattr(semantic, "generate_fixed_template_value", lambda *a, **k: (
        calls.append(1) or {"supports": False, "excerpt": "", "reason": "No evidence"}))
    req = requirement(); pool = pool_for("Spacecraft upgrade")
    trace = requirement_candidate_trace(req, pool)
...
def test_reference_research_without_requirement_does_not_invent_acceptance_review():
    ...
    state = {"research_queue": [{"research_id": "reference", "status": "pending",
                                 "source_kinds": ["repository"], "queries": ["named repository"]}]}
