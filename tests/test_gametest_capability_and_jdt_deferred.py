...
def test_junit_classname_and_method_are_valid_gametest_evidence(tmp_path: Path) -> None:
    report = tmp_path / "gametest-report.xml"
    report.write_text(
        '<testsuite tests="1" failures="0" errors="0" skipped="0">'
        '<testcase classname="dev.mmm.debugfixture.MmmDebugFixtureModGameTests" '
        'name="generatedRegistriesAreLive"/>'
        "</testsuite>",
        encoding="utf-8",
    )
    build_log = tmp_path / "gradle-build.log"
