...
def test_baseline_receipts_are_objective_stable_and_evaluator_compatible(
    ...
    assert {key: value["receipt_id"] for key, value in first.items()} == {
        key: value["receipt_id"] for key, value in second.items()
    }
    assert {key: value["evidence_refs"] for key, value in first.items()} == {
...
def test_completion_claim_is_not_evidence_and_large_receipt_sets_are_exhausted() -> None:
    ...
    performance = {
        ...
        "budgets": {
            "tick_ms": {"observed": 42, "limit": 50, "comparison": "lte"}
        },
    }
    receipts = tuple(
    ...
    rejected = _compile_call(
        ...
        {
            **empty_inputs,
            "module_receipts": ({"performance_validation": claimed},),
        },
    )
