...
def _large_proposal(
    module_count: int = 103,
    *,
    game_design: dict | None = None,
):
    ...
    proposal = complete_proposal_from_parts(
        ...
        base_proposal=base,
        game_design=game_design or {"title": "Sharded"},
        modules=tuple(
...
def test_complete_proposal_storage_scales_by_adding_bounded_shards(
    tmp_path: Path,
) -> None:
    ...
    assert index.stat().st_size < 16_000
...
def test_complete_proposal_storage_rejects_a_changed_shard(
    tmp_path: Path,
) -> None:
    index = write_sharded_complete_proposal(
        _large_proposal(17),
...
def test_index_swap_failure_keeps_previous_immutable_version_readable(
    tmp_path: Path,
    monkeypatch,
) -> None:
    index = write_sharded_complete_proposal(
    ...
    previous_shard = _first_collection_chunk(
        index,
        previous_raw["modules"],
    )
    ...
    monkeypatch.setattr(
        "minecraft_mod_ai.proposal_store.os.replace",
        fail_index_swap,
    )
...
def test_large_game_design_uses_fixed_size_files_and_round_trips(
    tmp_path: Path,
) -> None:
    part_size = 32 * 1024
    lore = (
        '한글 "quoted" line\\path\n'
        + ("0123456789abcdef" * 16_384)
        + " \U0001f409"
    )
    proposal = _large_proposal(
        33,
        game_design={
            "title": "Large streamed design",
            "lore": lore,
            "nested": {"chapters": [lore[:100_000], lore[100_000:]]},
        },
    )
    index = write_sharded_complete_proposal(
        proposal,
    ...
    version_root = (
        index.parent
        / Path(*raw["game_design"]["path_prefix"].split("/"))
    ).parent
    stored_files = [
        path
...
def test_root_index_stays_bounded_across_many_collection_shards(
    tmp_path: Path,
) -> None:
    index = write_sharded_complete_proposal(
        _large_proposal(300),
...
def test_complete_proposal_collection_pages_resume_without_inline_monolith(
    tmp_path: Path,
) -> None:
    index = write_sharded_complete_proposal(
        _large_proposal(257),
...
def test_complete_proposal_page_cursor_is_bound_to_section_and_hash(
    tmp_path: Path,
) -> None:
    index = write_sharded_complete_proposal(
        _large_proposal(9),
    ...
    first = read_sharded_complete_proposal_section(
        index,
        "modules",
        limit=1,
    )
...
def test_nested_game_design_list_can_be_read_as_bounded_pages(
    tmp_path: Path,
) -> None:
    index = write_sharded_complete_proposal(
        _large_proposal(
    ...
    page = read_sharded_complete_proposal_section(
        index,
        "game_design.production_outline",
        limit=10,
    )
...
def test_v1_index_remains_readable() -> None:
    ...
    def part(name: str, value) -> dict[str, str]:
    ...
    def collection(name: str, values: list) -> dict:
        if not values:
            return {"count": 0, "shards": []}
        return {
            "count": len(values),
            "shards": [part(name, {"items": values})],
        }

    raw = {
        "schema_version": "mmm/complete-proposal-index-v1",
        ...
        "game_design": part("design.json", proposal.game_design),
        "modules": collection("modules.json", proposal_payload["modules"]),
        "assets": collection("assets.json", proposal_payload["assets"]),
        "audio": collection("audio.json", proposal_payload["audio"]),
        "acceptance_tests": collection(
            "tests.json",
            proposal_payload["acceptance_tests"],
        ),
        "world_ir": None,
    }
...
def test_v2_chunked_store_loads_from_release_zip(tmp_path: Path) -> None:
    proposal = _large_proposal(
        19,
        game_design={"title": "ZIP", "lore": "z" * 70_000},
    )
    tree = tmp_path / "tree"
    index = write_sharded_complete_proposal(
        proposal,
    ...
    with zipfile.ZipFile(archive_path) as archive:
        loaded = load_sharded_complete_proposal_from_zip(
            archive,
...
def test_streaming_json_combines_surrogate_pairs_and_rejects_lone_values() -> None:
    ...
    assert "".join(iter_canonical_json("\ud83d\udc09")) == '"\U0001f409"'
...
def _first_collection_chunk(index: Path, collection: dict) -> Path:
    ...
    manifest = index.parent / manifest_prefix.with_name(
        manifest_prefix.name + "-00000000.json"
    )
    descriptor = json.loads(manifest.read_text(encoding="utf-8"))["shards"][0]
    data_prefix = manifest.parent / descriptor["path_prefix"]
    return data_prefix.with_name(
        data_prefix.name + ".chunk-00000000.jsonpart"
    )
