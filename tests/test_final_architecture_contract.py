...
from minecraft_mod_ai.repair_diagnostics_contract import diagnostic_errors, flatten_diagnostics
...
def test_atomic_ir_covers_directly_supported_requirement() -> None:
    ...
    assert ir['atoms'][0]['implementation_refs'] == ['implementation:module:frost_sword']
...
def test_enumerated_request_items_become_separate_atoms() -> None:
    proposal = _proposal('Add a frost sword, add a lunar portal, add a frost shield.', related=True)
    ir = compile_ir(proposal)
    assert ir['atom_count'] == 3
    assert [atom['text'] for atom in ir['atoms']] == ['Add a frost sword', 'add a lunar portal', 'add a frost shield.']
...
def test_semantic_reviewer_cannot_invent_implementation_ids() -> None:
    ...
    class Router:
        ...
        def generate_text(self, *_args, **_kwargs):
            return json.dumps({'mappings': [{'atom_id': ir['atoms'][0]['atom_id'], 'supported': True, 'implementation_refs': ['implementation:module:invented_portal'], 'acceptance_refs': ['acceptance:00000000']}]})
...
def test_jdt_mapping_is_flattened_and_only_errors_block() -> None:
    receipt = {'diagnostics': {'file:///A.java': [{'severity': 1, 'message': 'cannot resolve symbol'}, {'severity': 2, 'message': 'warning'}], 'file:///B.java': [{'severity': 3, 'message': 'info'}]}}
    flattened = flatten_diagnostics(receipt)
    assert [item['message'] for item in flattened] == ['cannot resolve symbol', 'warning', 'info']
    assert [item['message'] for item in diagnostic_errors(receipt)] == ['cannot resolve symbol']

def test_repair_signature_uses_real_jdt_mapping() -> None:
    first = RepairEngine._signature({'diagnostics': {'diagnostics': {'file:///A.java': [{'severity': 1, 'message': 'first', 'code': 'x'}]}}, 'build': {'status': 'SKIPPED'}})
    second = RepairEngine._signature({'diagnostics': {'diagnostics': {'file:///A.java': [{'severity': 1, 'message': 'second', 'code': 'x'}]}}, 'build': {'status': 'SKIPPED'}})
    assert first != second
...
def test_clean_room_proof_is_required_for_build_quality(tmp_path) -> None:
    ...
    live_only = {'status': 'PASS', 'commands': [{'name': 'build', 'exit_code': 0, 'timed_out': False}]}
    assert quality_evidence._clean_build_evidence(live_only) is None
    proven = {**live_only, 'clean_room_build': {'schema_version': CLEAN_ROOM_SCHEMA, 'status': 'PASS', 'source_fingerprint': 'abc123', 'build': {'status': 'PASS', 'commands': [{'name': 'build', 'exit_code': 0, 'timed_out': False}]}, 'jar_validation': {'status': 'PASS', 'checks_run': 7}, 'jar_path': str(jar), 'jar_sha256': digest, 'live_jar_content_sha256': 'sha256:' + '1' * 64, 'clean_jar_content_sha256': 'sha256:' + '1' * 64}}
...
def test_incremental_inner_build_alias_does_not_replace_clean_room_quality() -> None:
    build = {'commands': [{'name': 'build', 'exit_code': 0, 'timed_out': False}]}
...
def test_final_architecture_contracts_are_installed() -> None:
    ...
    assert getattr(CompleteProductionOrchestrator.execute, '_mmm_atomic_release_guard', False)
    assert getattr(CompleteProductionOrchestrator._evaluate_quality, '_mmm_clean_room_quality', False)
    assert getattr(quality_evidence._clean_build_evidence, '_mmm_clean_room_required', False)
    ...
    assert getattr(atomic_requirement_contract._implementations, '_mmm_compact_catalog', False)
    assert getattr(atomic_requirement_contract._atom_ranges, '_mmm_enumeration_atomizer', False)
    assert getattr(quality_evidence.compile_quality_evidence, '_mmm_atomic_correctness_evidence', False)
    assert orchestrator_module.compile_quality_evidence is quality_evidence.compile_quality_evidence
