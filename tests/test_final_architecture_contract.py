...
def _proposal(prompt: str, *, related: bool = True):
...
def test_incremental_inner_build_alias_does_not_replace_clean_room_quality() -> None:
    ...
    assert CompleteProductionOrchestrator._command_receipt_passed(
        build, "clean_build"
    )
...
def test_final_architecture_contracts_are_installed() -> None:
    assert getattr(
        CompleteGameDesignPlanner.plan,
        "_mmm_atomic_requirement_ir",
        False,
    )
    assert getattr(
