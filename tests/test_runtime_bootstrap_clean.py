...
def test_removed_runtime_patch_modules_do_not_return() -> None:
    for name in (
        ...
        "generation_boundary_reconciliation.py",
    ):
...
def test_execution_feedback_is_not_late_runtime_patched() -> None:
    ...
    assert "generation_boundary_reconciliation" not in finalization
    assert "install_generation_boundary_reconciliation" not in finalization
