...
def test_installed_wrapper_markers_have_single_layer_owners() -> None:
    duplicates: list[str] = []
    for binding, outer in iter_installed_wrappers():
        ...
        for marker, layer_indexes in owners.items():
            if len(layer_indexes) > 1:
                duplicates.append(
                    f"{binding}: {marker} copied across layers {layer_indexes}"
                )

    assert not duplicates, (
        "runtime contract ownership markers leaked through wrapper metadata:\n"
