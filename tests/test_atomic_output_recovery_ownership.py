...
def test_atomic_stall_is_terminal_for_outer_continuation_layers() -> None:
    ...
    # Diagnostics still retain the original typed llama boundary.
    assert completion_boundary_error(backend) is boundary
    # Recovery policy does not advertise it as recoverable a second time.
    assert completion_boundary_kind(backend) == ""
