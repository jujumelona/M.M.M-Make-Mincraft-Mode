...
from minecraft_mod_ai.audio_generator import _sound_java_files
...
def _project(root: Path) -> Path:
    spec = ModSpec(
        mod_id="catalog_test",
        mod_name="Catalog Test",
        package_name="ai.minecraft.catalog_test",
        version="1.0.0",
        summary="bounded catalog scaling test",
        contents=(
            ContentSpec(
...
def _extended_metrics(root: Path, count: int) -> tuple[int, int]:
    ...
    modules = tuple(
        ProductionModule(
            module_id=f"command_{index:05d}",
            kind="command",
            config={
                "literal": f"catalog{index:05d}",
                "message": f"Catalog command {index:05d}",
            },
        )
        for index in range(count)
    )
    result = generate_extended_content(
    ...
    metadata_files = [catalog_root, *sorted(
        (project / ".minecraft_ai/extended-module-records").rglob("*.json")
    )]
    java_root = (
        project
        / "src/main/java/ai/minecraft/catalog_test/extended"
    )
    ...
    return (
        max(path.stat().st_size for path in metadata_files),
        max(path.stat().st_size for path in java_files),
    )


def test_extended_catalog_and_registrar_files_stay_bounded_as_modules_grow(
    tmp_path: Path,
) -> None:
    small_metadata, small_java = _extended_metrics(
        tmp_path / "small-extended",
        64,
    )
    large_metadata, large_java = _extended_metrics(
        tmp_path / "large-extended",
        2_048,
    )
...
def test_extended_catalog_reader_preserves_legacy_monolith_compatibility(
    tmp_path: Path,
) -> None:
    ...
    assert [
        item["module_id"]
        for item in iter_extended_module_records(tmp_path)
    ] == ["legacy_item"]


def test_extended_generation_updates_only_the_new_content_unit(
    tmp_path: Path,
) -> None:
    project = _project(tmp_path / "incremental")
    first = ProductionModule(
        module_id="first_command",
        kind="command",
        config={"literal": "first", "message": "First"},
    )
    second = ProductionModule(
        module_id="second_command",
        kind="command",
        config={"literal": "second", "message": "Second"},
    )
    generate_extended_content(
        project_root=project,
        mod_id="catalog_test",
    ...
    receipt = generate_extended_content(
        project_root=project,
        mod_id="catalog_test",
        package_name="ai.minecraft.catalog_test",
        modules=(second,),
    ...
    assert {
        item["module_id"]
        for item in iter_extended_module_records(project)
...
def _system_contract_metrics(root: Path, count: int) -> int:
    ...
    result = generate_system_pack(
        ...
        mod_id="catalog_test",
        package_name="ai.minecraft.catalog_test",
        config={"modules": _system_modules(count)},
        policy=ScalePolicy(java_shard_size=8),
    )
    resources = project / "src/main/resources"
    contract = (
        resources
        / "data/catalog_test/mmm_systems/quest-system.json"
    )
    root_index = json.loads(contract.read_text(encoding="utf-8"))
    assert (
        root_index["storage_schema_version"]
        == "mmm/system-pack-directory-v1"
    )
    ...
    records = sorted(
        (
            resources
            / root_index["directory"].removeprefix("/")
        ).glob("*.json")
    )
    ...
    loader = next(project.rglob("MmmSystemConfig.java")).read_text(
        encoding="utf-8"
    )
...
def test_system_contract_files_stay_bounded_as_modules_grow(
    tmp_path: Path,
) -> None:
    small = _system_contract_metrics(tmp_path / "small-system", 64)
    large = _system_contract_metrics(tmp_path / "large-system", 2_048)

    assert large <= small + 512


def test_system_pack_incremental_shards_merge_and_update_one_record(
    tmp_path: Path,
) -> None:
    ...
    for shard in (
        ...
        result = generate_system_pack(
            ...
            mod_id="catalog_test",
            package_name="ai.minecraft.catalog_test",
            config={"modules": shard},
            policy=ScalePolicy(java_shard_size=48),
        )

    loaded = iter_system_module_records(
        project,
        mod_id="catalog_test",
        pack_id="quest-system",
    )
    assert len(loaded) == 120
    assert {item["module_id"] for item in loaded} == {
        f"quest_{index:05d}"
    ...
    records = (
        resources
        / "data/catalog_test/mmm_systems/quest-system/records"
    )
    root_before = contract.read_bytes()
    loader_before = next(
        project.rglob("MmmSystemConfig.java")
    ).read_bytes()
    system_before = next(project.rglob("QuestSystem.java")).read_bytes()
    unchanged_before = (
        records / "quest_00000.json"
    ).read_bytes()
    ...
    update = generate_system_pack(
        project_root=project,
        pack_id="quest-system",
        mod_id="catalog_test",
        package_name="ai.minecraft.catalog_test",
        config={"modules": [replacement]},
        policy=ScalePolicy(java_shard_size=48),
    )

    changed_paths = {
        item["path"]
        for item in update["write_receipt"]["operations"]
    }
    assert changed_paths == {
        (
            "src/main/resources/data/catalog_test/mmm_systems/"
            "quest-system/records/quest_00077.json"
        )
    }
...
def test_system_pack_migrates_legacy_shards_without_losing_modules(
    tmp_path: Path,
) -> None:
    ...
    shard = (
        resources
        / "data/catalog_test/mmm_systems/quest-system/"
        "shards/modules-00000000.json"
    )
    ...
    shard.write_text(
        json.dumps(
            {
                "schema_version": "mmm/system-module-shard-v1",
    ...
    contract.write_text(
        json.dumps(
            {
                "schema_version": "mmm/quest-system-v5",
                "storage_schema_version": "mmm/system-pack-index-v1",
                "pack_id": "quest-system",
                "module_count": 2,
                "shard_size": 8,
                "root": (
    ...
    generate_system_pack(
        ...
        mod_id="catalog_test",
        package_name="ai.minecraft.catalog_test",
        config={"modules": [_system_modules(3)[2]]},
        policy=ScalePolicy(java_shard_size=8),
    )

    migrated = iter_system_module_records(
        project,
        mod_id="catalog_test",
        pack_id="quest-system",
    )
    assert [item["module_id"] for item in migrated] == [
        "quest_00000",
        "quest_00001",
        "quest_00002",
    ]
    ...
    assert (
        root["storage_schema_version"]
        == "mmm/system-pack-directory-v1"
    )


def test_class_skill_template_replays_bounded_catalog_for_dependency_passes(
    tmp_path: Path,
) -> None:
    project = _project(tmp_path / "class-skill")
    generate_system_pack(
        project_root=project,
        pack_id="class-skill-system",
        mod_id="catalog_test",
        package_name="ai.minecraft.catalog_test",
        config={
            "modules": [
                {
                    "module_id": "ranger",
                    "kind": "class",
                    "config": {"display_name": "Ranger"},
                    "depends_on": [],
                    "required_gates": [],
                },
                {
                    "module_id": "ranger_dash",
                    "kind": "skill",
                    "config": {
    ...
    java = next(project.rglob("ClassSkillSystem.java")).read_text(
        encoding="utf-8"
    )
...
def test_sound_registrar_root_stays_constant_as_units_grow() -> None:
    small = _sound_java_files(
        package_name="ai.minecraft.catalog_test",
        mod_id="catalog_test",
        sound_ids=[f"sound_{index:05d}" for index in range(64)],
    )
    large = _sound_java_files(
        package_name="ai.minecraft.catalog_test",
        mod_id="catalog_test",
        sound_ids=[f"sound_{index:05d}" for index in range(2_048)],
    )
    root_path = (
        "src/main/java/ai/minecraft/catalog_test/sound/"
        "GeneratedSounds.java"
    )
    ...
    unit_sizes = [
        ...
        for path, value in large.items()
        if "GeneratedSoundUnit" in path
    ]
    assert len(unit_sizes) == 2_048
