...
def test_truncated_recursive_tree_falls_back_to_complete_subtree_walk(monkeypatch):
    ...
    def fake_json(client, url, *, params=None):
        ...
        if "/git/commits/" in url:
            return {"tree": {"sha": "root-tree"}}
        if url.endswith("/git/trees/root-tree") and params == {"recursive": "1"}:
        ...
        if url.endswith("/git/trees/tree-src"):
            return {
                "truncated": False,
                "tree": [
                    {"path": "main", "type": "tree", "sha": "tree-main"},
                    {"path": "test", "type": "tree", "sha": "tree-test"},
                ],
            }
        if url.endswith("/git/trees/tree-main"):
            return {
                "truncated": False,
                "tree": [{"path": "Planet.java", "type": "blob", "sha": "planet"}],
            }
        if url.endswith("/git/trees/tree-test"):
            return {
                "truncated": False,
