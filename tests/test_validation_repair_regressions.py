...
def test_build_work_node_is_committed_before_final_validation() -> None:
    ...
    build_checkpoint_at = source.index("run_build_repair_checkpoint(")
    repair_refresh_at = source.index(
        "update_execution_project_index_from_receipt(project_root, repair)",
        build_checkpoint_at,
    )
    final_manifest_at = source.index("final_manifest = str(", build_checkpoint_at)
    build_done_at = source.index(
        "self._succeed_work_node(ledger, 'build-project'",
        final_manifest_at,
    )
    ...
    assert "execution_project_index(" in source[:build_checkpoint_at]
    assert "execution_project_index(" in source[final_manifest_at:build_done_at]
