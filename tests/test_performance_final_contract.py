...
def test_generic_integration_is_not_hidden_in_content_commit_lane() -> None:
    ...
    sidecar = ProductionModule('local_sidecar', 'integration', {'integration_type': 'mmm_local_ai_sidecar'})
...
def test_local_gpu_text_role_participates_in_gpu_exclusion() -> None:
    config = ModelRegistry._resolve_role('coder', {'model_id': 'local/test-model', 'adapter': 'llama_cpp', 'exclusive_gpu': False})
    cpu_config = ModelRegistry._resolve_role('embedding', {'model_id': 'local/test-embedding', 'adapter': 'embedding', 'exclusive_gpu': False})
...
def test_overlapping_custom_jobs_share_one_immutable_wave_snapshot(tmp_path: Path) -> None:
    ...
    assert (first / source.relative_to(project)).read_text(encoding='utf-8') == 'class Example {}\n'
...
def test_staged_java_merge_preserves_independent_insertions() -> None:
    ...
    live = 'class Example {\n    void init() {\n        registerSystem();\n    }\n}\n'
    merged = _three_way_merge('src/main/java/example/Example.java', base_text=base, staged_text=staged, live_text=live)
...
def test_staged_json_merge_preserves_parallel_additions() -> None:
    ...
    staged = json.dumps({'entrypoints': {'main': ['example.Base', 'example.Custom']}}, indent=2) + '\n'
    live = json.dumps({'entrypoints': {'main': ['example.Base', 'example.System']}}, indent=2) + '\n'
    merged = json.loads(_three_way_merge('src/main/resources/fabric.mod.json', base_text=base, staged_text=staged, live_text=live))
    assert merged['entrypoints']['main'] == ['example.Base', 'example.System', 'example.Custom']
