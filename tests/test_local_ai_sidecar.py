...
def _module() -> ProductionModule:
    return ProductionModule(
        ...
        "integration",
        {
            ...
            "max_in_flight": 3,
            "capabilities": ["agent_tool_use", "ai_inference", "translation"],
...
def _approved_complete(module: ProductionModule):
    ...
    proposal = complete_proposal_from_parts(
        requested_prompt="Use a reviewed local sidecar for NPC AI inference",
        base_proposal=base,
        game_design={"title": "Local AI integration"},
        modules=(module,),
...
def test_complete_orchestrator_dispatches_reviewed_sidecar_without_model_router(
    ...
    receipts = [
        ...
        if receipt.get("schema_version") == "mmm/local-ai-sidecar-generation-v1"
    ]
    assert len(receipts) == 1
    assert (
        receipts[0]["policy_enforcement"]
