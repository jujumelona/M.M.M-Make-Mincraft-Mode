...
from minecraft_mod_ai.local_ai_sidecar_generator import (
...
def _module(**overrides: object) -> ProductionModule:
    config: dict[str, object] = {
        ...
        "max_in_flight": 3,
        "capabilities": ["speech_recognition", "speech_synthesis", "translation"],
        "authentication": "external_token",
    }
    config.update(overrides)
    return ProductionModule("npc_voice_bridge", "integration", config)


def _base_proposal():
    return MinecraftModPipeline(planner=HeuristicPlanner()).plan(
        "Create one local AI bridge anchor item"
    )


def _approved_complete(module: ProductionModule):
    base = _base_proposal()
    proposal = complete_proposal_from_parts(
        requested_prompt="Use a reviewed local sidecar for NPC speech",
        base_proposal=base,
        game_design={"title": "Local speech integration"},
        modules=(module,),
        acceptance_tests=("Sidecar failures never block the game thread",),
    )
...
def _generate_approved_project(root: Path) -> tuple[Path, object, ProductionModule]:
    ...
    (metadata / "complete-proposal.json").write_text(
        json.dumps(complete.to_dict(), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {"authentication": "embedded_key"},
        {"provider_api_key": "must-never-enter-the-jar"},
    ),
)
def test_sidecar_config_is_strict_and_secret_fields_are_rejected(
    override: dict[str, object],
) -> None:
    module = _module(**override)
    with pytest.raises(LocalAiSidecarGenerationError):
        normalize_local_ai_sidecar_config(module.config)
...
def test_generator_emits_only_bounded_async_localhost_boundary(tmp_path: Path) -> None:
    ...
    receipt = generate_local_ai_sidecar(
        project_root=root,
    ...
    source_path = root / local_ai_sidecar_source_path(
        base.spec.package_name,
        module.module_id,
    )
...
def test_validator_allows_only_exact_approved_source_and_manifest(tmp_path: Path) -> None:
    ...
    source_path.write_text(
        source_path.read_text(encoding="utf-8") + "// unreviewed edit\n",
        encoding="utf-8",
    )
...
def test_extra_java_network_code_remains_forbidden(tmp_path: Path) -> None:
    ...
    extra = (
        ...
        / "integration/UnapprovedNetwork.java"
    )
    extra.parent.mkdir(parents=True, exist_ok=True)
    extra.write_text(
        f"package {spec.package_name}.integration;\n"
    ...
    report = ProjectValidator().validate(root, spec)
    assert any(
        finding.code == "FORBIDDEN_JAVA_API"
        and finding.path.endswith("UnapprovedNetwork.java")
        for finding in report.findings
    )


def test_complete_orchestrator_dispatches_reviewed_sidecar_without_model_router(
    tmp_path: Path,
) -> None:
    module = _module()
    ...
    orchestrator = CompleteProductionOrchestrator(
        ...
        router_factory=lambda: (_ for _ in ()).throw(
            AssertionError("reviewed sidecar must not use the custom-code model router")
        ),
    )
    result = orchestrator.execute(
        proposal,
        approval_hash=proposal.calculate_hash(),
        run_name="sidecar_dispatch",
    ...
    assert receipts[0]["policy_enforcement"] == (
        "exact_source_and_manifest_reconstruction"
    )
