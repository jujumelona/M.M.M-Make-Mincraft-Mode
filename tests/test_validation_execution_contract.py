...
def test_generation_verification_has_no_gradle_fallback_backend() -> None:
    ...
    assert importlib.util.find_spec(
        "minecraft_mod_ai.generation_verifier_fallback_installation"
    ) is None
    source = inspect.getsource(run_generation_verifier)
    assert "GradleRunner" not in source
    assert "_gradle_fallback_receipt" not in source
