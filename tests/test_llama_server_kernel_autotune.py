...
def test_stage_selector_requires_verified_output_and_minimum_gain() -> None:
    ...
    assert kernel._select_stage(
        current,
        [_probe(fast_cfg, tps=30.0, sha="different")],
        minimum_gain=1.01,
    ) == current
    assert kernel._select_stage(
        current,
        [_probe(fast_cfg, tps=40.0, ok=False)],
        minimum_gain=1.01,
    ) == current
    assert kernel._select_stage(
        current,
        [_probe(fast_cfg, tps=20.1)],
        minimum_gain=1.01,
    ) == current
    assert kernel._select_stage(
        current,
        [_probe(fast_cfg, tps=24.0)],
        minimum_gain=1.01,
    ).config == fast_cfg
...
def test_benchmark_is_staged_not_cartesian(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        kernel,
        "_cache_candidates",
        lambda _current, _hardware: (("q4_0", "q4_0"), ("q8_0", "f16")),
    )
    ...
    def fake_run_probe(_autotune, _binary, _model_path, _config, _request, config):
        ...
        if config.flash_attn == "auto":
            bonus += 10.0
        if config.batch == 1024:
            bonus += 10.0
        if (config.cache_type_k, config.cache_type_v) == ("q8_0", "f16"):
            bonus += 10.0
    ...
    # baseline + one alternative for each of three stages. A Cartesian search would
    # need 2*2*2=8 configurations before even adding a baseline.
    assert len(calls) == 4
    assert len(probes) == 4
...
def test_pipeline_keeps_existing_runtime_layers_and_adds_kernel_outer_stage() -> None:
    ...
    names = tuple(stage.name for stage in pipeline.stages())
