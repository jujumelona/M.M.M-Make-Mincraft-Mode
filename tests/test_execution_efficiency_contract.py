...
def test_validation_resume_reuses_only_stable_exact_results(monkeypatch) -> None:
    ...
    execution_efficiency_contract._install_validation_checkpoint_reuse(
        complete_orchestrator
    )

    result = complete_orchestrator.run_named_checkpoint(
        object(),
        "validate-source",
        stage="validate:source",
        input_value={"graph_hash": "g", "project_manifest": "m"},
...
def test_jdt_resume_never_reuses_unavailable_result(monkeypatch) -> None:
    ...
    complete_orchestrator.run_named_checkpoint(
        object(),
        "validate-jdt",
        stage="validate:jdt",
        input_value={"graph_hash": "g", "project_manifest": "m"},
...
def test_validation_checkpoint_scope_changes_with_mmm_runtime_policy(monkeypatch) -> None:
    ...
    first = execution_efficiency_contract._validation_implementation_fingerprint(
        "validate-jdt"
    )
    monkeypatch.setenv("MMM_VALIDATION_CACHE_TEST_SCOPE", "two")
    second = execution_efficiency_contract._validation_implementation_fingerprint(
        "validate-jdt"
    )
    assert first != second
