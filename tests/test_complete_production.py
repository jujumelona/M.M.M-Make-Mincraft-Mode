...
def test_geckolib_generator_accumulates_real_entity_bindings(tmp_path: Path) -> None:
    ...
    server_units = sorted(
        project.rglob("*GeckoRegistration.java")
    )
    assert len(server_units) == 2
    unit_text = "\n".join(
        path.read_text(encoding="utf-8")
        for path in server_units
    )
    ...
    client_units = sorted(
        project.rglob("*GeckoClientRegistration.java")
    )
...
def test_audio_registration_writes_soundevent_and_sounds_json(tmp_path: Path) -> None:
    ...
    assert "GeneratedSoundUnit" in root_java.read_text(encoding="utf-8")
    units = sorted(project.rglob("GeneratedSoundUnit*.java"))
    assert units
...
def test_complete_orchestrator_requires_exact_existing_input_presence(
    ...
    unbound = complete_proposal_from_parts(
        requested_prompt="new project",
        base_proposal=base,
        game_design={"title": "Fresh"},
        modules=(ProductionModule("fresh_item", "item"),),
    ...
    bound = complete_proposal_from_parts(
        requested_prompt="revise project",
        base_proposal=base,
        game_design={"title": "Revision"},
        modules=(ProductionModule("revision_item", "item"),),
        acceptance_tests=("item exists",),
        existing_input_sha256=archive_hash,
    )
    ...
    with pytest.raises(CompleteProductionError, match="same ZIP is required"):
        orchestrator.execute(
            bound,
            approval_hash=bound.calculate_hash(),
            run_name="missing-input",
            options=CompleteExecutionOptions(source_only=True),
        )
    with pytest.raises(
        CompleteProductionError,
        match="approved with that input",
    ):
        orchestrator.execute(
            unbound,
            approval_hash=unbound.calculate_hash(),
            run_name="unexpected-input",
            options=CompleteExecutionOptions(source_only=True),
...
def test_complete_orchestrator_checks_bound_hash_before_extraction(
    tmp_path: Path,
) -> None:
    base = MinecraftModPipeline(planner=HeuristicPlanner()).plan(
        "Create one frost item"
    ...
    with pytest.raises(
        ...
        CompleteProductionOrchestrator(workspace_root=runs).execute(
            proposal,
            approval_hash=proposal.calculate_hash(),
            run_name="changed-input",
            options=CompleteExecutionOptions(source_only=True),
            existing_input=archive,
        )
...
def test_prepare_project_archives_partial_deterministic_directory(
    tmp_path: Path,
) -> None:
    base = MinecraftModPipeline(planner=HeuristicPlanner()).plan(
        "Create one frost item"
    )
    awaiting = complete_proposal_from_parts(
        requested_prompt="new project",
        base_proposal=base,
        game_design={"title": "Fresh"},
        modules=(ProductionModule("fresh_item", "item"),),
    ...
    project = orchestrator._prepare_project(
        proposal,
        run_root=run_root,
        existing_input=None,
    )
    ...
    assert (preserved / "partial.txt").read_text(encoding="utf-8") == (
        "preserve me\n"
    )


def test_required_gate_matrix_is_receipt_backed_and_fail_closed(
    tmp_path: Path,
) -> None:
    base = MinecraftModPipeline(planner=HeuristicPlanner()).plan(
        "Create one frost item"
    ...
    proposal = complete_proposal_from_parts(
        requested_prompt="verified item",
        base_proposal=base,
        game_design={"title": "Verified"},
        modules=(
            ProductionModule(
                "verified_item",
                "item",
    ...
    main_class = (
        "".join(
            part.capitalize()
            for part in base.spec.mod_id.split("_")
        )
        + "Mod"
    )
    gametest_report = tmp_path / "gametest-report.xml"
    gametest_report.write_text(
        "<testsuite failures=\"0\" errors=\"0\" skipped=\"0\">"
        f"<testcase name=\"{main_class}GameTests.generatedRegistriesAreLive\"/>"
        "</testsuite>",
        encoding="utf-8",
    )
    build = {
        "status": "PASS",
        "gametest_report": str(gametest_report),
        "commands": [
            {
                "name": "clean_build",
                "exit_code": 0,
    ...
    common = {
        ...
        "jdt_receipt": {"files_opened": 4, "error_count": 0},
        "build_report": build,
    ...
    assert CompleteProductionOrchestrator._required_gate_failures(
        proposal,
        generated_receipts=(),
        **common,
    ) == []
    ...
    failures = CompleteProductionOrchestrator._required_gate_failures(
        proposal,
        generated_receipts=(
            {"required_gates": ["restart persistence test"]},
        ),
        **{
            **common,
            "build_report": {
                "status": "PASS",
                "commands": [build["commands"][0]],
            },
        },
    )
    assert any("GameTest:missing-gametest" in item for item in failures)
    assert any(
        "restart persistence test:unsupported" in item
        for item in failures
    )


def test_prepare_project_recovers_one_nested_release_source_zip(
    tmp_path: Path,
) -> None:
    base = MinecraftModPipeline(planner=HeuristicPlanner()).plan(
        "Create one frost item"
    ...
    with zipfile.ZipFile(nested_path, "w") as nested:
            ...
            json.dumps(
                {
                    ...
                    "entrypoints": {"main": [main_entrypoint]},
                    "depends": {
        ...
        nested.writestr(
            ...
            + f"/{main_class}.java",
            f"package {spec.package_name}; public final class {main_class} {{}}\n",
        )
    release = tmp_path / "release.zip"
    with zipfile.ZipFile(release, "w") as outer:
        outer.writestr(
            f"source/{spec.mod_id}-{spec.version}-source.zip",
            nested_path.read_bytes(),
        )
    archive_hash = "sha256:" + hashlib.sha256(release.read_bytes()).hexdigest()
    awaiting = complete_proposal_from_parts(
        requested_prompt="revise nested project",
        base_proposal=base,
        game_design={"title": "Nested Revision"},
        modules=(ProductionModule("nested_item", "item"),),
        acceptance_tests=("item exists",),
        existing_input_sha256=archive_hash,
    )
