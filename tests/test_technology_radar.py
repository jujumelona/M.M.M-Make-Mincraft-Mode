...
def _official_target_evidence(target: dict[str, object]) -> dict[str, object]:
    body: dict[str, object] = {'schema_version': 'mmm/official-target-evidence-v1', 'retrieved_by': 'mmm_authoritative_retriever', 'authorization': 'read_only_evidence', 'sources': [{'source_url': 'https://maven.fabricmc.net/net/fabricmc/fabric-api/fabric-api/0.92.11%2B1.20.1/', 'observed_at': '2026-08-01T00:00:00Z', 'content_sha256': 'sha256:' + 'f' * 64, 'facts': target}]}
    return _seal_technology_receipt(body, _RECEIPT_KEY)

def _technology_test_receipt(test_id: str, requirement_id: str, *, candidate_id: str, revision: str, artifact_sha256: str, evidence_sha256: str, candidate_snapshot_sha256: str) -> dict[str, object]:
    body: dict[str, object] = {'schema_version': 'mmm/technology-test-receipt-v1', 'executed_by': 'mmm_quality_runner', 'status': 'pass', 'test_id': test_id, 'requirement_id': requirement_id, 'candidate_id': candidate_id, 'revision': revision, 'artifact_sha256': artifact_sha256, 'evidence_sha256': evidence_sha256, 'candidate_snapshot_sha256': candidate_snapshot_sha256, 'observed_at': '2026-08-01T00:00:00Z', 'environment_sha256': 'sha256:' + '1' * 64, 'result_sha256': 'sha256:' + '2' * 64}
    return _seal_technology_receipt(body, _RECEIPT_KEY)
...
def test_reviewed_candidate_can_pass_but_newest_is_never_an_automatic_winner() -> None:
    ...
    reviewed_assessment = _assess_technology_candidate_with_receipt_key(requirement, reviewed, receipt_key=_RECEIPT_KEY)
    ...
    newest_assessment = _assess_technology_candidate_with_receipt_key(requirement, newest, receipt_key=_RECEIPT_KEY)
...
def test_official_target_receipt_cannot_be_missing_or_tampered() -> None:
    ...
    missing_assessment = _assess_technology_candidate_with_receipt_key(requirement, missing, receipt_key=_RECEIPT_KEY)
    ...
    tampered_assessment = _assess_technology_candidate_with_receipt_key(requirement, tampered, receipt_key=_RECEIPT_KEY)
...
def test_public_hash_cannot_forge_code_owned_target_receipt() -> None:
    ...
    receipt['receipt_sha256'] = 'sha256:' + hashlib.sha256(canonical_json(receipt).encode('utf-8')).hexdigest()
    assessment = _assess_technology_candidate_with_receipt_key(requirement, forged, receipt_key=_RECEIPT_KEY)
...
def test_authenticated_receipts_do_not_validate_without_the_service_key() -> None:
    ...
    with pytest.raises(TypeError):
        assess_technology_compatibility(requirement, candidate, receipt_key=_RECEIPT_KEY)
...
def test_executed_receipts_cannot_be_replayed_for_another_candidate() -> None:
    ...
    assessment = _assess_technology_candidate_with_receipt_key(requirement, replayed, receipt_key=_RECEIPT_KEY)
    assert assessment['eligible'] is False
    assert 'tests' in assessment['unresolved_gates']
    assert 'deterministic_fallback' in assessment['unresolved_gates']
...
def test_executed_receipts_bind_the_entire_assessed_candidate_snapshot() -> None:
    ...
    assessment = _assess_technology_candidate_with_receipt_key(requirement, changed, receipt_key=_RECEIPT_KEY)
    assert assessment['eligible'] is False
    assert 'tests' in assessment['unresolved_gates']
    assert 'deterministic_fallback' in assessment['unresolved_gates']
...
def test_test_names_and_booleans_are_not_execution_evidence() -> None:
    ...
    candidate['fallback'] = {'description': requirement['deterministic_fallback'], 'deterministic': True, 'tested': True}
    assessment = _assess_technology_candidate_with_receipt_key(requirement, candidate, receipt_key=_RECEIPT_KEY)
    assert 'tests' in assessment['unresolved_gates']
    assert 'deterministic_fallback' in assessment['unresolved_gates']
...
def test_large_research_graph_uses_bound_pages_without_a_project_wide_cap() -> None:
    domains = [{'domain_id': f'system_{index:03d}', 'objective': f'Research requested system {index:03d}.', 'requirements': [f'Implement system {index:03d}.'], 'evidence_kinds': ['ai_inference'], 'queries': [f'system {index:03d} inference evidence']} for index in range(137)]
    ...
    with pytest.raises(SpecValidationError, match='does not match'):
        build_technology_radar(prompt + ' changed', brief, page_size=19, cursor=first['pagination']['next_cursor'])
