...
def test_complete_proposal_does_not_normalize_saved_module_identity() -> None:
    install(spec, complete_spec)
    payload = _complete_payload()
    payload['modules'] = [{'module_id': 'Boss System', 'kind': 'custom_java', 'config': {}, 'depends_on': [], 'required_gates': []}]
...
def test_base_proposal_list_fields_do_not_accept_string_iterables() -> None:
    install(spec, complete_spec)
    payload = {'schema_version': 'minecraft-mod-ai/proposal-v1', 'proposal_version': 1, 'status': 'awaiting_user_approval', 'requested_prompt': 'test', 'spec': {'mod_id': 'test_mod', 'mod_name': 'Test Mod', 'package_name': 'com.example.testmod', 'version': '1.0.0', 'summary': 'test', 'platform': {}, 'contents': [], 'boss': None}, 'assumptions': 'not-a-list', 'exclusions': [], 'deferred_requests': [], 'acceptance_tests': ['works'], 'evidence_sources': [], 'evidence_snapshot_hash': '', 'capability_manifest_hash': '', 'imported_source_snapshot_hash': '', 'risk_approvals': [], 'approval_hash': ''}
