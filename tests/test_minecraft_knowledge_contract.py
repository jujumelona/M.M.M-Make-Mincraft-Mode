...
from tests.planning_authority_fixtures import design_with_catalog
...
def _plan(prompt: str, design: dict | None = None) -> dict:
    return compile_minecraft_knowledge_plan(prompt, design_with_catalog(prompt, design))
...
def _fake_research(plan: dict) -> dict:
    ...
    return {
        "research_brief": {"domains": domains, "_mmm_platform_target": target},
        "deterministic": {
            "grounded_rag": {
                "domains": [
                    {
...
def test_boss_expands_host_owned_minecraft_dependency_graph() -> None:
    plan = _plan("체력바와 특수 공격이 있는 보스 몬스터를 추가하고 드롭 아이템과 소리도 넣어줘.")
    ids = _ids(plan)
    assert {
        "platform.fabric_target", "platform.mappings", "project.structure", "registry.content",
        "entity.registration", "entity.attributes", "entity.ai_goals", "entity.tracked_data",
        "entity.spawn", "rendering.entity", "combat.damage", "boss.bossbar",
        "datagen.loot", "resources.sound", "custom.source_extension", "quality.compile",
        "quality.gametest", "quality.runtime", "release.packaging",
    } <= ids
...
def test_machine_gui_activates_only_required_container_and_client_branches() -> None:
    plan = _plan("인벤토리가 있는 기계를 만들고 GUI 버튼을 누르면 서버에서 작동하게 해줘.")
    ids = _ids(plan)
    assert "machine_with_gui" in plan["features"]
    assert {"block.registration", "block_entity.lifecycle", "inventory.container", "ui.screen_handler", "ui.client_screen"} <= ids
...
def test_machine_activates_network_and_persistence_only_when_requested() -> None:
    plan = _plan("Create a machine GUI inventory whose state must save and synchronize with multiplayer clients using a packet.")
    ids = _ids(plan)
    assert {"data.persistence", "networking.payloads", "networking.state_sync"} <= ids
...
def test_plain_hud_does_not_force_block_entity_inventory_or_networking() -> None:
    ids = _ids(_plan("HUD에 스태미나 바를 표시해줘."))
...
def test_plain_entity_does_not_force_mob_ai_persistence_or_custom_packets() -> None:
    ids = _ids(_plan("새 엔티티를 추가하고 렌더링해줘."))
...
def test_unknown_mechanic_has_source_mapping_project_and_runtime_fallback() -> None:
    plan = _plan("완전히 새로운 특이한 게임 기믹을 추가해줘.")
    ids = _ids(plan)
    assert "custom_java" in plan["features"]
    assert {"custom.source_extension", "quality.gametest", "quality.runtime"} <= ids
    custom = next(item for item in plan["requirements"] if item["knowledge_id"] == "custom.source_extension")
    assert custom["evidence"]["rag_queries"]
    assert {"source_search", "mapping_resolution", "mod_examples"} <= set(custom["evidence"]["mcp_capabilities"])


def test_common_mod_features_have_dedicated_contracts() -> None:
    plan = _plan("커스텀 상태 효과와 인챈트를 추가하고 주민 거래, 게임룰, 광석 생성도 넣어줘.")
    assert {"effects.status", "enchantment.custom", "villager.trade", "rules.gamerule", "worldgen.feature"} <= _ids(plan)


def test_pre_design_knowledge_is_target_neutral_and_routes_are_deferred() -> None:
    plan = _plan("Minecraft 1.21.1 Fabric에서 새 보스 몬스터를 추가해줘.")
    routed_text = "\n".join(
        [str(item["objective"]) for item in plan["requirements"]]
        + [str(query) for domain in plan["research_domains"] for query in domain["queries"]]
    )
    ...
    coverage = evaluate_route_coverage(plan, {"research_brief": {"domains": [{"domain_id": "request"}]}, "deterministic": {}, "domain_notes": []})
    assert coverage["status"] == "PASS"
    assert {item["status"] for item in coverage["domains"]} == {"DEFERRED_UNTIL_TARGET_FREEZE"}


def test_route_coverage_requires_actual_route_receipts_after_target_freeze() -> None:
    plan = _plan("새 보스 몬스터를 추가해줘.")
    ...
    research["deterministic"]["grounded_rag"]["domains"] = [
        item for item in research["deterministic"]["grounded_rag"]["domains"] if item["domain_id"] != missing_domain
    ]
...
def test_route_coverage_accepts_terminal_fixed_point_with_deferred_gaps() -> None:
    ...
    note = next(item for item in research["domain_notes"] if item["domain_id"] == domain["domain_id"])
    ...
    receipt = next(item for item in coverage["domains"] if item["domain_id"] == domain["domain_id"])
...
def test_target_frozen_plan_marks_policy_and_active_routes_fail_closed() -> None:
    target = adapter_for_target("1.21.1", "fabric").public_dict()
    plan = _plan("Add multiplayer packet networking.", {"_platform_selection": {"target": target}})
    assert plan["policy"]["target_frozen"] is True
    assert plan["policy"]["target_receipt_sha256"].startswith("sha256:")
    assert {domain["domain_id"] for domain in plan["research_domains"]} >= {"mk_networking", "mk_platform", "mk_project", "mk_quality"}
    coverage = evaluate_route_coverage(plan, {"research_brief": {"_mmm_platform_target": target, "domains": []}, "deterministic": {}, "domain_notes": []})
...
def test_false_optional_branches_have_hashed_not_applicable_receipts() -> None:
    plan = _plan("Add a server command.")
    coverage = evaluate_route_coverage(plan, {"research_brief": {"domains": []}, "deterministic": {}, "domain_notes": []})
    predicates = {item["predicate_id"]: item for item in plan["branch_predicates"]}
    routes = {item["predicate_id"]: item for item in coverage["branch_routes"]}
    for predicate_id in ("needs_registry", "needs_datagen", "needs_persistence", "needs_network", "needs_client_render", "needs_worldgen", "needs_mixin", "needs_loader_leaf"):
...
def test_target_scoped_official_receipts_satisfy_active_routes() -> None:
    plan = _plan("Add a server command.")
    research = _fake_research(plan)
    grounded = research["deterministic"].pop("grounded_rag")
    research["deterministic"]["official_rag"] = {"target": research["research_brief"]["_mmm_platform_target"], "domains": grounded["domains"]}
    coverage = evaluate_route_coverage(plan, research)
    assert coverage["status"] == "PASS"
    assert not coverage["blocking_requirement_refs"]
    assert {item["execution_source"] for item in coverage["domains"]} == {"official_versioned_rag"}


def test_loader_leaf_activates_only_from_host_topology_evidence() -> None:
    target = adapter_for_target("1.21.1", "fabric").public_dict()
    design = {
        "_platform_selection": {"target": target},
        "_evidence_first_plan": {
            "branch_predicates": {
                "needs_loader_leaf": {
                    "predicate": "needs_loader_leaf",
                    "status": "ACTIVE",
                    "evidence_refs": ["existing_snapshot:module:common", "existing_snapshot:module:fabric"],
                }
            }
        },
    }
    plan = _plan("Add a server command.", design)
    predicates = {item["predicate_id"]: item for item in plan["branch_predicates"]}
    assert "project.loader_leaf" in _ids(plan)
    assert predicates["needs_loader_leaf"]["status"] == "ACTIVE"
    assert predicates["needs_loader_leaf"]["evidence"]["host_evidence_refs"] == ["existing_snapshot:module:common", "existing_snapshot:module:fabric"]
...
def test_loader_leaf_rejects_unreceipted_host_activation() -> None:
    prompt = "Add a server command."
    with pytest.raises(ValueError, match="needs_loader_leaf.*evidence_refs"):
        compile_minecraft_knowledge_plan(
            prompt,
            design_with_catalog(prompt, {"_minecraft_branch_predicates": {"needs_loader_leaf": {"value": True, "evidence_refs": []}}}),
        )
