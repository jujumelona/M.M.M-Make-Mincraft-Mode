...
"""Durable content-addressed storage for compacted agent observations.

Context fitting decides *what* leaves the live model window. This module owns only
lossless persistence and the compact host-verifiable ledger that points back to the
exact dropped transcript. Keeping archive I/O separate prevents multiple compaction
policies from competing for the same model history.
"""
...
def archive_preview(messages: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    payload = _canonical_bytes(list(messages))
    digest = _sha256_bytes(payload)
    target = archive_root() / f"{digest.removeprefix('sha256:')}.json"
    return {
        "available": True,
        "sha256": digest,
        "bytes": len(payload),
...
def archive_transcript(messages: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
...
def context_ledger(
    ...
    *,
    ...
    """Summarize only host-verifiable durable facts from archived history."""
    ...
    return {
        ...
        "policy": {
            ...
            "single_context_fitter_owner": True,
        },
    }
