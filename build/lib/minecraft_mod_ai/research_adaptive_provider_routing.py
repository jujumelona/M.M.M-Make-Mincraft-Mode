...
_TECHNOLOGY_EVIDENCE_KINDS = frozenset({'ai_inference', 'agent_tool_use', 'translation', 'model_runtime', 'model_license', 'dataset_provenance', 'consent_privacy', 'latency_budget'})
...
def _technology_not_required(prompt: str, research_brief: Mapping[str, Any] | None) -> dict[str, Any]:
    payload: dict[str, Any] = {'schema_version': 'mmm/technology-radar-not-required-v1', 'status': 'not_required', 'requirements': [], 'errors': [], 'routing': {'policy': 'run_only_for_technology_capability_evidence', 'technology_evidence_kinds': sorted(_TECHNOLOGY_EVIDENCE_KINDS)}, 'request_sha256': _sha256(prompt), 'brief_sha256': str(research_brief.get('brief_sha256', '') if isinstance(research_brief, Mapping) else '')}
    payload['radar_sha256'] = _sha256(payload)
    return payload

def _ecosystem_deferred(prompt: str, research_brief: Mapping[str, Any] | None) -> dict[str, Any]:
    route_count = _external_route_count(research_brief)
    status = 'deferred' if route_count else 'not_required'
    route_basis = {'brief_sha256': str(research_brief.get('brief_sha256', '') if isinstance(research_brief, Mapping) else ''), 'route_count': route_count}
    payload: dict[str, Any] = {'schema_version': 'mmm/ecosystem-planning-deferred-v1', 'status': status, 'phase': 'planning', 'candidate_count': 0, 'processed_route_count': 0, 'remaining_route_count': route_count, 'routes_complete': route_count == 0, 'route_count': route_count, 'route_sha256': _sha256(route_basis), 'query_sha256': _sha256(prompt), 'pages': [], 'errors': [], 'coverage': 'external discovery is deferred until a domain agent reports a relevant evidence gap or validation requires a specialist lookup', 'routing': {'policy': 'defer_external_provider_io_until_gap', 'external_provider_count': len(_EXTERNAL_PROVIDERS)}}
    return payload
...
def harden(agentic_module: Any, adaptive_rag_module: Any) -> None:
    ...
    if not getattr(current_route, _FORCED_ROUTE_MARKER, False):
        ...
        @wraps(current_route)
        def owner_aware_route(domain: Mapping[str, Any], *, code_index_available: bool) -> dict[str, Any]:
            routed = dict(current_route(domain, code_index_available=code_index_available))
            ...
            routed['reason'] = 'official_docs_owns_catalog_code_routed' if routed.get('code') else 'official_docs_owns_catalog'
            return routed
    ...
    if not getattr(current_technology, _PROVIDER_MARKER, False):
        ...
        def adaptive_technology(prompt: str, research_brief: Mapping[str, Any] | None=None, **kwargs: Any) -> dict[str, Any]:
    ...
    if not getattr(current_ecosystem, _PROVIDER_MARKER, False):
        ...
        def adaptive_ecosystem(prompt: str, game_design: dict[str, Any], *, research_brief: dict[str, Any] | None=None, planning_seed_only: bool=False, **kwargs: Any) -> dict[str, Any]:
            if planning_seed_only and research_brief is not None:
                return _ecosystem_deferred(prompt, research_brief)
            return current_ecosystem(prompt, game_design, research_brief=research_brief, planning_seed_only=planning_seed_only, **kwargs)
