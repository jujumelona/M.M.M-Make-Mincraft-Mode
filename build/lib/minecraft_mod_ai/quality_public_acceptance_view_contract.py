...
adapts only those internal callers without weakening the validator. One generic alias-repair
mutation updates already-imported quality helpers without growing the reviewed mutation budget.
"""
...
def install(production_module: Any, quality_evidence_module: Any) -> None:
    ...
    def evaluate_quality_contract(
        ...
        strict_validate(contract, module_ids, _public_acceptance(contract))
        if not isinstance(proposal_hash, str) or not production_module._SHA256.fullmatch(
            proposal_hash
        ):
    ...
    replacements = {
        "quality_contract_summary": (original_summary, quality_contract_summary),
        "evaluate_quality_contract": (original_evaluate, evaluate_quality_contract),
    }
    for name, module in tuple(sys.modules.items()):
        if not name.startswith("minecraft_mod_ai.") or module is None:
            continue
        for attribute, (original, replacement) in replacements.items():
