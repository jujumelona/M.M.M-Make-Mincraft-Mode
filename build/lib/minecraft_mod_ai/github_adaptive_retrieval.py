...
def _search_request_budget() -> int:
    ...
    # GitHub's unauthenticated search bucket is only 10 requests/minute. Use one
    # high-signal best-match probe per requirement unless authentication raises the
    # allowance; callers can explicitly tune the work budget when appropriate.
    authenticated = bool(
        os.environ.get("GITHUB_TOKEN", "").strip()
        or os.environ.get("GH_TOKEN", "").strip()
    )
    default = 3 if authenticated else 1
    return _env_int("MMM_GITHUB_SEARCH_REQUEST_BUDGET", default, minimum=1, maximum=30)
...
def _source_request_budget() -> int:
    ...
    return _env_int("MMM_GITHUB_SOURCE_REQUEST_BUDGET", 24, minimum=4, maximum=512)


def _repository_request_slice() -> int:
    # Prevent one irrelevant seed from consuming the whole per-query source budget.
    return _env_int("MMM_GITHUB_REPOSITORY_REQUEST_SLICE", 12, minimum=4, maximum=128)
...
def repository_query_ladder(query: str) -> tuple[str, ...]:
    ...
    # Put a compact capability phrase first because unauthenticated callers normally
    # have one search request per requirement. Long AND-heavy obligation sentences
    # belong later in the corrective ladder.
    capability = " ".join(terms[:2])
    variants: list[str] = [f"{capability} minecraft", " ".join(terms)]
    for term in terms:
...
def _retrieval_failure_kind(exc: BaseException) -> str:
    ...
    if "rate limit" in text or "secondary rate" in text or "429" in text or "403" in text:
        return "rate_limited"
    if "422" in text or "search limit" in text:
...
def discover_repositories(query: str, *, http_json: HttpJSON) -> RepositoryDiscovery:
    ...
    while active and requests < budget:
        ...
        for variant, page in active:
            ...
            try:
                ...
                if kind in {"rate_limited", "provider_limited"}:
                    reason = kind
    ...
    if provider_blocked:
        pass
    elif requests >= budget and active:
        reason = "search_request_budget_exhausted"
    ...
    *,
    ...
    try:
        ...
        return RepositoryEvidence(
            repository=(owner, repo),
            documents=(),
            errors=(f"github_repo:{owner}/{repo}:{kind}:{type(exc).__name__}: {exc}",),
            ...
            saturation_reason=(
                kind
                if kind in {"rate_limited", "provider_limited"}
                else "repository_metadata_failed"
            ),
        )
    ...
    try:
        ...
        return RepositoryEvidence(
            repository=(owner, repo),
            documents=(),
            errors=(f"github_tree:{owner}/{repo}:{kind}:{type(exc).__name__}: {exc}",),
            requests_used=requests,
            source_bytes=0,
            covered_features=frozenset(),
            coverage_score=0.0,
    ...
    if tree_truncated and requests < request_limit:
        # GitHub may truncate recursive trees. Walk subtrees breadth-first under the
        # same source request budget instead of accepting an incomplete candidate set
        # or traversing the repository without a bound.
        try:
            requests += 1
            root_tree = http_json(f"{repo_api}/git/trees/{quote(branch, safe='')}")
        except Exception as exc:  # noqa: BLE001
            kind = _retrieval_failure_kind(exc)
            errors.append(
                f"github_tree_root:{owner}/{repo}:{kind}:{type(exc).__name__}: {exc}"
            )
            root_tree = {}
        queue: list[tuple[str, Mapping[str, Any]]] = [
            ("", root_tree)
        ] if isinstance(root_tree, Mapping) else []
        pending: list[tuple[str, str]] = []
        while (queue or pending) and requests < request_limit:
            if queue:
                prefix, subtree = queue.pop(0)
            else:
                prefix, sha = pending.pop(0)
                try:
                    requests += 1
                    raw_subtree = http_json(
                        f"{repo_api}/git/trees/{quote(sha, safe='')}"
                    )
                except Exception as exc:  # noqa: BLE001
                    kind = _retrieval_failure_kind(exc)
                    errors.append(
                        f"github_subtree:{owner}/{repo}:{sha}:{kind}:"
                        f"{type(exc).__name__}: {exc}"
                    )
                    if kind in {"rate_limited", "provider_limited"}:
                        break
                    continue
                subtree = raw_subtree if isinstance(raw_subtree, Mapping) else {}
            raw_items = subtree.get("tree", ())
            for item in raw_items if isinstance(raw_items, list) else ():
                ...
                if item.get("type") == "tree" and item.get("sha"):
                    pending.append((path, str(item["sha"])))
                    continue
                if item.get("type") != "blob" or not path.casefold().endswith(
                    _SOURCE_SUFFIXES
                ):
                    continue
        ...
        if pending:
            errors.append(f"github_tree:{owner}/{repo}:subtree_request_budget_exhausted")
    else:
    ...
    for path, advertised_size in candidates:
        ...
        try:
            ...
            kind = _retrieval_failure_kind(exc)
            errors.append(
                f"github_raw:{owner}/{repo}/{path}:{kind}:"
                f"{type(exc).__name__}: {exc}"
            )
            if kind in {"rate_limited", "provider_limited"}:
                reason = kind
    ...
    *,
    ...
    while True:
        ...
        evidence = retrieve_repository_documents(
            ...
            request_budget=min(request_remaining, _repository_request_slice()),
        ...
        if _coverage(query_features, covered) >= target and documents:
            reason = "evidence_coverage_satisfied"
            break
        if evidence.saturation_reason in {"rate_limited", "provider_limited"}:
            reason = evidence.saturation_reason
            break
