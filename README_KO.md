...
엔진이나 설치 방식이 바뀌기 전부터 Colab 탭을 열어 두었다면 배지로 노트북을 다시 열고 런타임을 한 번 재시작한 뒤 전체 셀을 실행합니다. 최신 설치 코드는 모델 가중치를 받기 전에 Qwen CUDA 커널을 검증하며, 메모리를 많이 쓰는 fallback으로 조용히 진행하지 않습니다.
...
`MODEL_PROFILE="t4_local"`이 Colab T4 안전 기본값이며 4B 기획 모델을 4비트로 실행합니다. 9B 기획 모델이 필요하고 더 엄격한 VRAM 사전 검사를 통과하는 실행 환경에서는 `t4_quality`를 선택할 수 있습니다. `remote_quality`로 바꾸고 HTTPS API 주소와 모델 이름을 입력하면 OpenAI 호환 원격 API를 사용합니다. API 키는 노트북에 저장하지 않고 실행할 때 숨김 입력으로 받습니다.
