...
def install(complete_spec_module: Any, complete_planner_module: Any) -> None:
    ...
    def scoped_implementation_prompt(prompt: str, game_design: dict[str, Any]) -> str:
        ...
        if isinstance(target, dict) and isinstance(target.get('target'), dict):
            ...
            target_text = f" The exact host-verified target for this plan is Minecraft {selected.get('minecraft_version')} / {selected.get('loader')} / Java {selected.get('java_version')} / {selected.get('mappings')}. Do not substitute APIs from another version. For a live-discovered target, use the supplied official target evidence and compiler feedback rather than a hard-coded historical source template."
        return original_prompt(prompt, enriched_design) + '\n\nThe resolved mod-development method plan is authoritative for implementation scope. Generate only a mod project for the selected platform target. Never emit a standalone map, world save, world ZIP, schematic, Litematica file, BuildSpec, NPZ block delta, or external Builder handoff. Native worldgen modules are mod-owned code and datapack resources only when fabric_worldgen was selected from the request.' + target_text

    def scoped_complete_proposal_from_parts(*, requested_prompt: str, base_proposal: Any, game_design: dict[str, Any], modules: tuple[Any, ...], assets: tuple[Any, ...]=(), acceptance_tests: tuple[str, ...], existing_input_sha256: str='') -> Any:
        method_plan = resolve_mod_development_methods(requested_prompt, existing_project=bool(existing_input_sha256))
        ...
        return original_builder(requested_prompt=requested_prompt, base_proposal=base_proposal, game_design=scoped_design, modules=modules, assets=assets, acceptance_tests=acceptance_tests, existing_input_sha256=existing_input_sha256)
