...
def install(spec_module: Any, complete_spec_module: Any) -> None:
    ...
    if not getattr(proposal_function, "_mmm_strict_deserialization", False):
        ...
        def proposal_from_dict(cls: Any, data: Any):
            ...
            if "proposal_version" in raw and (
                type(raw["proposal_version"]) is not int or raw["proposal_version"] < 1
            ):
        ...
        proposal_from_dict.__func__._mmm_strict_deserialization = True  # type: ignore[attr-defined]
    ...
    def complete_from_dict(cls: Any, data: Any):
        ...
        audio = _require_list(raw.get("audio"), "audio", error)
        for index, item in enumerate(audio):
            value = _require_dict(item, f"audio[{index}]", error)
            for field in ("sound_id", "kind"):
                _require_string(value.get(field), f"audio[{index}].{field}", error)
            for field in ("subtitle_en", "subtitle_ko"):
                if field in value:
                    _require_string(value[field], f"audio[{index}].{field}", error, empty=True)
            if "loop" in value and type(value["loop"]) is not bool:
                raise error(f"audio[{index}].loop must be a JSON boolean.")
            for field in ("duration_seconds", "frequency_hz", "volume"):
                if field in value:
                    number = value[field]
                    if isinstance(number, bool) or not isinstance(number, (int, float)):
                        raise error(f"audio[{index}].{field} must be a JSON number.")
        return complete_function(cls, raw)

    complete_from_dict.__func__._mmm_strict_deserialization = True  # type: ignore[attr-defined]
