...
if TYPE_CHECKING:
    from .complete_spec import AssetRequest, AudioRequest, ProductionModule
...
_DIMENSIONS: dict[str, dict[str, Any]] = {
    "correctness": {
        ...
        "objective": (
            "Every request-derived requirement is traceable to implementation "
            "and an observable acceptance check."
        ),
        "acceptance": (
            "Every request-derived requirement is linked to implemented output "
            "and independently observed behavior or artifacts."
        ),
    },
    "build": {
        "title": "Reproducible build",
        "objective": (
            "The complete source builds from a clean checkout with the locked "
            "toolchain and declared dependencies."
        ),
        "acceptance": (
            "A clean, locked-toolchain build succeeds and its artifact inventory "
            "is recorded by the build verifier."
        ),
    },
    "research": {
        "title": "Research and compatibility",
        "objective": (
            "Technical choices are supported by current, scoped evidence and "
            "checked for version, license, and provenance compatibility."
        ),
        "acceptance": (
            "Research-backed choices have current source receipts plus version, "
    ...
    "runtime": {
        ...
        "objective": (
            "The built result starts and completes representative requested "
            "scenarios without hidden planner assertions."
        ),
        "acceptance": (
            "The built result starts and representative requested scenarios pass "
            "under an external runtime verifier."
        ),
    },
    "visual_3d": {
        "title": "Visual, 3D, and animation quality",
        "objective": (
            "Requested visual assets, models, animation, rendering, and effects "
            "load correctly and meet observable review criteria."
        ),
        "acceptance": (
            "Requested visual and 3D output loads without missing resources and "
            "passes deterministic render plus visual review checks."
        ),
        "terms": (
            "visual", "texture", "model", "3d", "animation", "shader",
            "render", "particle", "sprite", "image", "시각", "텍스처", "모델",
            "애니메이션", "셰이더", "렌더", "파티클", "이미지", "3차원",
        ),
        "asset_kinds": ("item", "block", "entity", "gui", "environment", "icon"),
    },
    "audio": {
        "title": "Audio quality",
        "objective": (
            "Requested sound, music, and speech assets are registered, audible, "
            "mixed, and backed by usable fallbacks."
        ),
        "acceptance": (
            "Requested audio plays through registered events at reviewed levels, "
            "with subtitles or fallback behavior where declared."
        ),
        "terms": (
            "audio", "sound", "music", "voice", "speech", "tts", "asr",
            "soundtrack", "오디오", "소리", "효과음", "음악", "음성", "목소리",
        ),
    },
    "state_save_migration": {
        "title": "State, saves, and migration",
        "objective": (
            "Requested persistent state survives save/load and evolves without "
            "silently corrupting supported prior data."
        ),
        "acceptance": (
            "Persistent state passes round-trip, restart, corruption handling, and "
            "declared-version migration checks."
        ),
        "terms": (
            "save", "persistence", "persistent", "migration", "upgrade", "nbt",
            "codec", "database", "player state", "저장", "영속", "마이그레이션",
            "업그레이드", "데이터 이전", "플레이어 상태",
        ),
    },
    "multiplayer": {
        "title": "Multiplayer authority and synchronization",
        "objective": (
            "Requested multiplayer behavior has explicit server authority, "
            "validated messages, and synchronized observable state."
        ),
        "acceptance": (
            "Requested multiplayer scenarios pass with multiple clients, server "
            "authority, message validation, and reconnect synchronization."
        ),
        "terms": (
            "multiplayer", "network", "server", "client", "packet", "sync",
            "party", "guild", "co-op", "coop", "멀티플레이", "네트워크", "서버",
            "클라이언트", "패킷", "동기화", "파티", "길드", "협동",
        ),
        "module_kinds": ("networking", "party", "guild"),
    },
    "performance": {
        "title": "Performance and scale",
        "objective": (
            "Explicit scale or performance goals are measured against declared "
            "workloads, budgets, and regression baselines."
        ),
        "acceptance": (
            "Declared large-scale or performance workloads meet recorded tick, "
            "latency, memory, and throughput budgets without skipped work."
        ),
        "phrases": (
            "game-scale", "game scale", "large-scale", "large scale", "massive",
            "performance", "latency", "throughput", "frame rate", "tick budget",
            "memory budget", "stress test", "게임 수준", "대규모", "대형 모드",
            "성능", "지연 시간", "처리량", "프레임", "틱 예산", "메모리 예산",
            "스트레스 테스트",
        ),
    },
    "accessibility": {
        ...
        "objective": (
            "Requested accessibility and localization behavior remains usable "
            "across declared input, language, subtitle, and display paths."
        ),
        "acceptance": (
            "Declared accessibility and localization paths pass keyboard, subtitle, "
            "language, and readable-display checks as applicable."
        ),
        "terms": (
            "accessibility", "subtitle", "localization", "language", "colorblind",
            "screen reader", "key remap", "접근성", "자막", "현지화", "다국어",
            "언어", "색각", "스크린 리더", "키 변경",
        ),
    },
    "ai_voice": {
        "title": "AI and voice runtime",
        "objective": (
            "Requested AI or voice behavior has bounded authority, observable "
            "latency, privacy controls, and deterministic safe fallback paths."
        ),
        "acceptance": (
            "Requested AI or voice scenarios pass tool-authority, privacy, latency, "
            "failure, and fallback checks with the sidecar unavailable as well."
        ),
        "terms": (
            " ai ", "artificial intelligence", "llm", "agent", "rag", "mcp",
            "inference", "npc cognition", "voice", "speech", "asr", "tts", "lora",
            "인공지능", "에이전트", "검색 증강", "추론", "음성", "목소리", "대화형 ai",
        ),
    },
}
...
_COMPLETION_POLICY = {
    "owner": "code",
    "allowed_evidence_statuses": ["PASS", "MISSING", "FAIL"],
    "all_dimensions_must_pass": True,
    "fresh_receipt_required": True,
    "proposal_hash_binding_required": True,
    "independent_verifier_required": True,
    "self_reported_completion_accepted": False,
    "plateau_identical_failure_threshold": _PLATEAU_THRESHOLD,
}
...
def compile_production_contract(
    requested_prompt: str,
    game_design: Mapping[str, Any],
    research_brief: Mapping[str, Any] | Sequence[Any] | None = None,
    modules: Sequence["ProductionModule | Mapping[str, Any]"] = (),
    assets: Sequence["AssetRequest | Mapping[str, Any]"] = (),
    audio: Sequence["AudioRequest | Mapping[str, Any]"] = (),
    ...
    requirements = _compile_requirements(
        requested_prompt, design_snapshot, research_snapshot
    )
    ...
    if not implementation_catalog:
        raise ProductionContractError(
            "at least one module, asset, or audio item is required"
        )

    active_dimensions, activation_reasons = _infer_dimensions(
    ...
    for index, statement in enumerate(input_acceptance):
        acceptance_catalog.append(
            {
                "acceptance_ref": f"acceptance:input:{index:08d}",
                "origin": "input",
                "statement": statement,
            }
        )
    ...
    for dimension_id in active_dimensions:
        ...
        statement = _unique_acceptance_statement(
            f"[{dimension_id}] {statement}", used_acceptance_statements
        )
        ref = f"acceptance:quality:{dimension_id}"
        acceptance_catalog.append(
            {"acceptance_ref": ref, "origin": "quality", "statement": statement}
        )
        used_acceptance_statements.add(statement)
    ...
    for requirement in requirements:
        ...
        if requirement["source"] == "research_brief":
            statement = (
                f"[{requirement['requirement_ref']}] Verify the scoped research "
                "item is current, compatible, licensed, and used only within its "
                f"evidence scope: {requirement['statement']}"
            )
        else:
            statement = (
                f"[{requirement['requirement_ref']}] Demonstrate observable "
                f"behavior or output satisfying: {requirement['statement']}"
            )
        statement = _unique_acceptance_statement(statement, used_acceptance_statements)
        acceptance_catalog.append(
            {"acceptance_ref": ref, "origin": "requirement", "statement": statement}
        )
        used_acceptance_statements.add(statement)
        ...
        quality_catalog.append(
            {
                ...
                "activation": (
                    "baseline"
                    if dimension_id in _BASELINE_DIMENSIONS
                    else "request-derived"
                ),
                "activation_reasons": activation_reasons[dimension_id],
                "objective": definition["objective"],
                "acceptance_ref": dimension_acceptance[dimension_id],
                "evidence_route_ref": route_ref,
            }
        )
    ...
    input_test_search = {
        entry["acceptance_ref"]: _tokens(entry["statement"])
    ...
    for requirement in requirements:
        ...
        direct_implementations = _bounded_matches(
            requirement_ref,
            requirement["statement"] + " " + requirement["source_ref"],
            implementation_search,
            implementation_index,
            _MAX_DIRECT_IMPLEMENTATION_REFS,
        )
        matched_input_tests = _bounded_matches(
            requirement_ref,
        ...
        if requirement["source"] == "requested_prompt":
            relevant_dimensions.extend(
                item for item in _CONDITIONAL_ORDER if item in active_set
            )
        else:
            source_text = (
                requirement["source_ref"] + " " + requirement["statement"]
            )
            for dimension_id in _CONDITIONAL_ORDER:
                if dimension_id in active_set and _text_triggers_dimension(
                    source_text, dimension_id
                ):
        ...
        coverage_groups.append(
            {
                ...
                "implementation_catalog_ref": "catalog:implementations",
                "implementation_refs": direct_implementations,
                "acceptance_catalog_ref": "catalog:acceptance",
                "acceptance_refs": [
                    requirement_acceptance[requirement_ref], *matched_input_tests
                ],
                "quality_dimension_refs": quality_refs,
                "evidence_route_refs": [
                    "evidence:" + value.removeprefix("quality:")
                    for value in quality_refs
                ],
            }
        )
    ...
    source_bindings = {
        "game_design_sha256": _canonical_sha256(design_snapshot),
        "research_brief_sha256": (
            "" if research_snapshot is None else _canonical_sha256(research_snapshot)
        ),
        "module_input_sha256": _canonical_sha256(normalized_modules),
        "asset_input_sha256": _canonical_sha256(normalized_assets),
        "audio_input_sha256": _canonical_sha256(normalized_audio),
    }
    contract: dict[str, Any] = {
        ...
        "quality_dimension_catalog": quality_catalog,
        "evidence_route_catalog": evidence_routes,
        "coverage_groups": coverage_groups,
        "completion_policy": _json_copy(_COMPLETION_POLICY, "completion_policy"),
        "catalog_stats": {
            "requirements": len(requirements),
            "implementations": len(implementation_catalog),
            "acceptance_tests": len(acceptance_catalog),
            "quality_dimensions": len(quality_catalog),
...
def validate_production_contract(
    ...
    if (
        not isinstance(contract["requested_prompt"], str)
        or not contract["requested_prompt"].strip()
    ):
    ...
    if contract["contract_sha256"] != _hash_without_field(
    ...
    implementations = _require_list(
        contract["implementation_catalog"], "implementations"
    )
    acceptances = _require_list(contract["acceptance_catalog"], "acceptance")
    dimensions = _require_list(
        contract["quality_dimension_catalog"], "quality dimensions"
    )
    ...
    if not requirements or not implementations or not acceptances:
        raise ProductionContractError(
            "requirements, implementations, and acceptance catalogs must be non-empty"
        )
    ...
    for item in requirements:
        ...
        expected_requirement_digest = hashlib.sha256(
            _canonical_json(
                {
                    "source": item["source"],
        ...
        group_by_requirement[ref] = _nonempty_string(
            item["coverage_group_ref"], "coverage_group_ref"
        )
    root_requirements = [
        item for item in requirements if item["source"] == "requested_prompt"
    ]
    ...
    for item in implementations:
        ...
        if item["source_kind"] not in {"module", "asset", "audio"}:
            raise ProductionContractError(f"invalid implementation source: {ref}")
    ...
    for item in acceptances:
        _require_exact_keys(
            item, {"acceptance_ref", "origin", "statement"}, "acceptance entry"
        )
        ...
        if item["origin"] == "requirement" and not ref.startswith(
            "acceptance:requirement:"
        ):
            raise ProductionContractError(f"invalid requirement acceptance ref: {ref}")
        catalog_acceptance.append(
            _nonempty_string(item["statement"], "acceptance statement")
        )
    external_acceptance = list(acceptance_tests)
    if catalog_acceptance != external_acceptance:
        raise ProductionContractError(
            "acceptance catalog does not match proposal acceptance tests"
        )
    ...
    for item in dimensions:
        ...
        expected_activation = (
            "baseline" if dimension_id in _BASELINE_DIMENSIONS else "request-derived"
        )
        ...
        reasons = _require_string_list(
            item["activation_reasons"], "activation reasons", nonempty=True
        )
        if len(reasons) > 8:
    ...
    expected_dimension_order = list(_BASELINE_DIMENSIONS) + [
        value for value in _CONDITIONAL_ORDER if value in set(dimension_ids)
    ]
    ...
    for item in routes:
        _require_exact_keys(
            item,
            {"route_ref", "dimension_ref", "accepted_statuses", "requirements"},
            "evidence route",
        )
    ...
    for item in groups:
        ...
        direct_refs = _require_string_list(
            item["implementation_refs"], "implementation refs", nonempty=True
        )
        ...
        test_refs = _require_string_list(
            item["acceptance_refs"], "acceptance refs", nonempty=True
        )
        if len(test_refs) > 3:
        ...
        quality_refs = _require_string_list(
            item["quality_dimension_refs"], "quality refs", nonempty=True
        )
        evidence_refs = _require_string_list(
            item["evidence_route_refs"], "evidence refs", nonempty=True
        )
    ...
    expected_stats = {
        ...
        "acceptance_tests": len(acceptances),
        "quality_dimensions": len(dimensions),
        "coverage_groups": len(groups),
        "max_direct_implementation_refs_per_group": _MAX_DIRECT_IMPLEMENTATION_REFS,
    }
...
def evaluate_quality_contract(
    ...
    for dimension in contract["quality_dimension_catalog"]:
        ...
        result = _evaluate_dimension_receipt(
            dimension_id=dimension_id,
    ...
    unresolved = [
        item["dimension_id"] for item in dimension_results if item["status"] != "PASS"
    ]
    overall_status = (
        "PASS"
        if not unresolved
    ...
    report: dict[str, Any] = {
        ...
        "plateau": {
            ...
            "identical_failure_threshold": _PLATEAU_THRESHOLD,
        },
        "report_sha256": "",
    }
...
def _compile_requirements(
    ...
    for source, source_ref, statement in raw:
        ...
        digest = hashlib.sha256(
            _canonical_json(
                {"source": source, "source_ref": source_ref, "statement": statement}
            ).encode("utf-8")
        ).hexdigest()
        requirements.append(
            {
...
def _source_items(source: str, value: Any) -> list[tuple[str, str, str]]:
    ...
    while stack:
        ...
        if isinstance(current, Mapping):
            ...
            for raw_key in sorted(current, key=lambda item: str(item)):
                if not isinstance(raw_key, str):
                    raise ProductionContractError(
                        f"{source} object keys must be strings"
                    )
        ...
        if isinstance(current, bool):
            ...
            raise ProductionContractError(
                f"{source} contains unsupported value {type(current).__name__}"
            )
...
def _normalize_audio(value: Any) -> dict[str, Any]:
    data = _object_mapping(value, "audio")
    sound_id = _nonempty_string(data.get("sound_id"), "sound_id")
    normalized = {"sound_id": sound_id}
    for key in sorted(data):
        if key == "sound_id":
            continue
        normalized[key] = _json_copy(data[key], f"audio {sound_id}.{key}")
    return normalized
...
def _implementation_catalog(
    ...
    for item in modules:
        add(
            f"implementation:module:{item['module_id']}",
            "module",
            item["module_id"],
            item["kind"],
            item,
    ...
    for item in assets:
        add(
            f"implementation:asset:{item['asset_id']}",
            "asset",
            item["asset_id"],
            item["kind"],
            item,
    ...
    for item in audio:
        add(
            f"implementation:audio:{item['sound_id']}",
            "audio",
            item["sound_id"],
            "audio",
            item,
...
def _infer_dimensions(
    *,
    ...
    reasons: dict[str, list[str]] = {
        value: ["code-owned baseline"] for value in _BASELINE_DIMENSIONS
    }
    primary_text = " ".join(
        [
            requested_prompt,
            *_scalar_text(game_design),
            *_scalar_text(modules),
            *_scalar_text(assets),
            *_scalar_text(audio),
        ]
    )
    text = " ".join(
        [
            primary_text,
            *([] if research_brief is None else _scalar_text(research_brief)),
        ]
    )
    ...
    for dimension_id in _CONDITIONAL_ORDER:
        ...
        trigger_text = (
            primary_text
            if dimension_id in {"performance", "ai_voice"}
            else text
        )
        if _text_triggers_dimension(trigger_text, dimension_id):
            dimension_reasons.append("request/design/research text")
        matching_module_kinds = sorted(
            module_kinds & set(definition.get("module_kinds", ()))
        )
        if matching_module_kinds:
            dimension_reasons.append(
                "module kinds: " + ", ".join(matching_module_kinds[:4])
            )
        matching_asset_kinds = sorted(
            asset_kinds & set(definition.get("asset_kinds", ()))
        )
        if matching_asset_kinds:
            dimension_reasons.append(
                "asset kinds: " + ", ".join(matching_asset_kinds[:4])
            )
        if dimension_id == "audio" and audio:
            dimension_reasons.append("audio requests supplied")
        if dimension_id == "accessibility" and any(
            str(item.get("subtitle_en", "")).strip()
            or str(item.get("subtitle_ko", "")).strip()
            for item in audio
        ):
            dimension_reasons.append("audio subtitles supplied")
        if dimension_id == "ai_voice" and any(
            item["kind"] == "integration"
            and item.get("config", {}).get("integration_type")
            == "mmm_local_ai_sidecar"
            for item in modules
        ):
            dimension_reasons.append("local AI sidecar module supplied")
...
def _bounded_matches(
    ...
    *,
    ...
    ranked_terms = sorted(
        (term for term in query_terms if term in index),
    ...
    scored = sorted(
        (
            (len(query_terms & search[ref]), ref)
            for ref in candidate_refs
            if query_terms & search[ref]
        ),
...
def _normalize_evidence(
    ...
    for value in raw_receipts:
        ...
        dimension_id = _nonempty_string(
            copied.get("dimension_id"), "evidence dimension_id"
        )
...
def _evaluate_dimension_receipt(
    *,
    dimension_id: str,
    route_ref: str,
    ...
    if previous is not None and (
        ...
        or receipt_sha == previous.get("receipt_sha256")
    ):
    ...
    if status == "FAIL":
        ...
        if not isinstance(signature, str) or not signature.strip():
            signature = _canonical_sha256(
                {
                    "dimension_id": dimension_id,
                    "failure": receipt.get("failure", receipt.get("reason", "failed")),
        ...
        if (
            ...
            streak = _strict_nonnegative_int(
                previous.get("failure_streak", 0), "previous failure streak"
            ) + 1
...
def _receipt_problem(
    receipt: Mapping[str, Any], route_ref: str, proposal_hash: str
) -> str:
...
def _validate_quality_report(report: Mapping[str, Any]) -> None:
    ...
    _require_exact_keys(
        plateau,
        {"detected", "dimension_ids", "identical_failure_threshold"},
        "quality plateau",
    )
    if plateau != {
        "detected": bool(plateau_ids),
        "dimension_ids": plateau_ids,
        "identical_failure_threshold": _PLATEAU_THRESHOLD,
    }:
...
def _normalize_acceptance_tests(values: Sequence[str]) -> list[str]:
    ...
    result = [
        _nonempty_string(value, "acceptance test").strip() for value in values
    ]
