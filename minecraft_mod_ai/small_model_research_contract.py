...
_EVIDENCE_FIELDS = ('module_ids', 'asset_ids', 'acceptance_tests')
_PRODUCTION_KEYS = frozenset({'modules', 'assets', 'acceptance_tests', 'completed_deliverables', 'complete', 'next_cursor'})
_AGENTIC_RISK_MARKERS = ('networking', 'multiplayer', 'custom_java', 'integration', 'dimension', 'world_event', 'ai_inference', 'agent_tool_use', 'migration', 'persistence')
...
def _evidence_is_grounded(value: Any, *, module_ids: set[str], asset_ids: set[str], acceptance_tests: set[str]) -> bool:
    if not isinstance(value, Mapping):
        return False
    return bool(_string_set(value.get('module_ids', ())) & module_ids or _string_set(value.get('asset_ids', ())) & asset_ids or _string_set(value.get(())) & None or _string_set(value.get('acceptance_tests', ())) & acceptance_tests)

def _sanitize_production_page(page: Mapping[str, Any], request: dict[str, Any] | str) -> dict[str, Any]:
    ...
    if not isinstance(completed, list):
        completed = []
    evidence = {str(key): dict(value) for key, value in evidence_value.items() if isinstance(key, str) and isinstance(value, Mapping)}
...
def _is_production_decode(request: dict[str, Any] | str, expected_contracts: Sequence[frozenset[str]]) -> bool:
...
def _install_evidence_contract(complete_planner_module: Any) -> None:
    ...
    @wraps(current)
    def generate_with_evidence_guard(router: Any, *, system_prompt: str, request: dict[str, Any] | str, media_paths: Sequence[Any], expected_contracts: Sequence[frozenset[str]], stage: str) -> dict[str, Any]:
        page = current(router, system_prompt=system_prompt, request=request, media_paths=media_paths, expected_contracts=expected_contracts, stage=stage)
        ...
        return sanitized
    generate_with_evidence_guard._mmm_small_model_evidence_guard = True
...
def _install_evidence_aware_scoring(agentic_module: Any) -> None:
    ...
    def score_with_evidence(page: Mapping[str, Any]) -> tuple[float, dict[str, Any]]:
        ...
        details = {**dict(verifier), 'declared_completion_evidence': declared, 'grounded_completion_evidence': grounded, 'unsupported_completion_claims': unsupported}
        return (score, details)
    score_with_evidence._mmm_evidence_aware_plan_score = True
...
def _semantic_ecosystem_key(prompt: str, game_design: Any, research_brief: Any) -> tuple[str, str, str]:
    return (_semantic_digest(prompt), _semantic_digest(game_design), _semantic_digest(research_brief))
...
def _maximal_planner_risk(request: Any, stage: str) -> bool:
    rendered = request if isinstance(request, str) else json.dumps(request, ensure_ascii=False, sort_keys=True, default=str)
    ...
    if isinstance(request, Mapping):
        ...
        target_risk = isinstance(targets, Sequence) and (not isinstance(targets, (str, bytes))) and (len(targets) >= 3)
...
def _install_trace_adaptive_search(agentic_module: Any) -> None:
    ...
    def candidate_count(request: Any, stage: str) -> int:
        ...
        if hint < base and (not _maximal_planner_risk(request, stage)):
            return int(hint)
        return base
    candidate_count._mmm_trace_adaptive_width = True
    candidate_count.__wrapped__ = current
...
def _install_lossless_research_input(central_module: Any, ecosystem_module: Any) -> None:
    ...
    if not getattr(central_module._bounded_text, '_mmm_lossless_text', False):
        ...
        def full_text(value: str, *, field: str='research text') -> str:
            del field
            return value
        full_text._mmm_lossless_text = True
        ...
        def domain_with_query_pages(value: Any):
            ...
            if isinstance(value, dict) and isinstance(value.get('queries'), list):
                ...
                for raw in value['queries']:
                    if isinstance(raw, str) and raw.strip() and (len(raw.encode('utf-8')) > central_module._MAX_QUERY_BYTES):
                        queries.extend((page for page in _lossless_utf8_pages(raw, central_module._MAX_QUERY_BYTES) if page.strip()))
                    else:
                        queries.append(raw)
                normalized = {**value, 'queries': queries}
            return current_domain(normalized)
        domain_with_query_pages._mmm_lossless_query_pages = True
    ...
    full_seed_query._mmm_lossless_seed_query = True
    ...
    def discover_seed_lossless(prompt: str, game_design: dict[str, Any], *, research_brief: dict[str, Any] | None=None, client: Any=None, route_cursor: str='', route_limit: int=12) -> dict[str, Any]:
        if research_brief is not None:
            return current_discover(prompt, game_design, research_brief=research_brief, client=client, route_cursor=route_cursor, route_limit=route_limit)
        query = ecosystem_module._seed_query(prompt, game_design)
        pages = _lossless_utf8_pages(query, central_module._MAX_QUERY_BYTES)
        if len(pages) <= 1:
            return current_discover(prompt, game_design, research_brief=None, client=client, route_cursor=route_cursor, route_limit=route_limit)
        providers = ['modrinth', 'openverse_images']
        if client is not None and client.github_token or __import__('os').environ.get('GITHUB_TOKEN'):
            providers.append('github')
        domains = [{'domain_id': f'request_page_{index + 1}', 'objective': 'Search one lossless page of the complete request.', 'requirements': [page], 'evidence_kinds': ['gameplay_reference'], 'queries': [page], 'providers': providers, 'depends_on': []} for index, page in enumerate(pages)]
        synthetic_brief = {'schema_version': 'mmm/central-research-brief-v1', 'summary': 'Lossless direct ecosystem query pages.', 'origin': 'lossless_direct_seed', 'domains': domains, 'unresolved_questions': []}
        result = current_discover(prompt, game_design, research_brief=synthetic_brief, client=client, route_cursor=route_cursor, route_limit=route_limit)
        result = dict(result)
        result['request_query_ingestion'] = {'schema_version': 'mmm/ecosystem-query-ingestion-v1', 'query_sha256': ecosystem_module._sha256_text(query), 'query_byte_length': len(query.encode('utf-8')), 'page_count': len(pages), 'lossless': ''.join(pages) == query, 'pages': [{'page_index': index, 'byte_length': len(page.encode('utf-8')), 'sha256': ecosystem_module._sha256_text(page)} for index, page in enumerate(pages)]}
        return result
    discover_seed_lossless._mmm_lossless_seed_routes = True
    ecosystem_module.discover_seed_bundle = discover_seed_lossless

def _install_nonblocking_planner_research(complete_planner_module: Any, research_coordinator_module: Any, central_module: Any) -> None:
    """Remove planner-level future joins; inner provider/RAG work may remain parallel."""
    complete_planner_module.collect_technology_radar = research_coordinator_module.collect_technology_radar
    retrieve = complete_planner_module.retrieve_domain_evidence

    def implementation_evidence(prompt: str, game_design: dict[str, Any], research_brief: dict[str, Any] | None=None) -> dict[str, Any]:
        brief = research_brief or complete_planner_module.normalize_research_brief(prompt, game_design)
        return retrieve(brief)
    implementation_evidence._mmm_nonblocking_planner_research = True
    complete_planner_module._retrieve_implementation_evidence = implementation_evidence

    def ecosystem_seed(prompt: str, game_design: dict[str, Any], *, research_brief: dict[str, Any] | None=None, client: Any=None, route_limit: int=12, page_builder: Any=None, planning_seed_only: bool=False) -> dict[str, Any]:
        if planning_seed_only and isinstance(research_brief, dict):
            routes = central_module.external_discovery_routes(research_brief)
            receipts = [{'domain_id': str(route.get('domain_id', '')), 'provider': str(route.get('provider', '')), 'target_profile': str(route.get('target_profile', '')), 'query_sha256': central_module._sha256(str(route.get('query', '')))} for route in routes]
            return {'schema_version': 'mmm/ecosystem-planning-deferred-v1', 'status': 'deferred', 'brief_sha256': str(research_brief.get('brief_sha256', '')), 'route_sha256': central_module._sha256(central_module.canonical_json(receipts)), 'route_count': len(routes), 'processed_route_count': 0, 'remaining_route_count': len(routes), 'routes_complete': not routes, 'candidate_count': 0, 'pages': [], 'errors': [], 'route_receipts': receipts, 'coverage': 'full route graph retained; public provider I/O deferred', 'authorization': 'none', 'download_performed': False, 'planning_critical_path': False}
        builder = page_builder or research_coordinator_module.discover_seed_bundle
        return research_coordinator_module.collect_ecosystem_seed_bundle(prompt, game_design, research_brief=research_brief, client=client, route_limit=route_limit, page_builder=builder, planning_seed_only=False)
    ecosystem_seed._mmm_nonblocking_planner_research = True
