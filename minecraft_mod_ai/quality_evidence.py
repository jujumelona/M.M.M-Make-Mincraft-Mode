...
def compile_quality_evidence(contract: Mapping[str, Any], proposal_hash: str, *, game_design: Mapping[str, Any], source_validation: Mapping[str, Any] | None, build_report: Mapping[str, Any] | None, jar_validation: Mapping[str, Any] | None, module_receipts: Iterable[Mapping[str, Any]]=(), asset_receipt: Mapping[str, Any] | None=None, blockbench_receipts: Iterable[Mapping[str, Any]]=(), runtime_receipt: Mapping[str, Any] | None=None, playtest_receipt: Mapping[str, Any] | None=None, visual_receipt: Mapping[str, Any] | None=None) -> dict[str, dict[str, Any]]:
    ...
    routes = {str(item['dimension_id']): str(item['evidence_route_ref']) for item in contract['quality_dimension_catalog']}
    roots: tuple[Any, ...] = (*modules, asset_receipt, *blockbench, runtime_receipt, playtest_receipt, visual_receipt)
    ...
    for dimension_id in routes:
        ...
        output[dimension_id] = _quality_receipt(dimension_id=dimension_id, route_ref=routes[dimension_id], proposal_hash=proposal_hash, evidence_refs=refs, observed_sources=observed_sources)
    return output

def _source_validation_evidence(value: Mapping[str, Any] | None) -> EvidenceResult | None:
...
def _clean_build_evidence(value: Mapping[str, Any] | None) -> EvidenceResult | None:
    if not _objective_pass(value):
        return None
...
def _jar_evidence(build: Mapping[str, Any] | None, validation: Mapping[str, Any] | None) -> EvidenceResult | None:
    ...
    if digest is None:
        return None
    facts = {'jar_sha256': digest, 'checks_run': checks, 'finding_count': len(findings)}
...
def _research_evidence(game_design: Mapping[str, Any]) -> EvidenceResult | None:
    ...
    if technology.get('aggregate_schema_version') != 'mmm/technology-radar-aggregate-v1' or not isinstance(pagination, Mapping) or pagination.get('complete') is not True or (pagination.get('next_cursor') != '') or (not _is_sequence(requirements)) or (_nonnegative_int(pagination.get('returned')) != len(requirements)) or (_nonnegative_int(pagination.get('total_requirements')) != len(requirements)) or (_positive_int(pagination.get('pages_collected')) is None) or (not isinstance(technology_collection, Mapping)) or (technology_collection.get('schema_version') != 'mmm/technology-page-collection-receipt-v1') or (not _valid_digest(technology.get('radar_sha256'))):
        return None
    ...
    if ecosystem.get('aggregate_schema_version') != 'mmm/ecosystem-seed-aggregate-v1' or ecosystem.get('routes_complete') is not True or ecosystem.get('next_route_cursor') != '' or (route_count is None) or (processed != route_count) or (remaining != 0) or (not _is_sequence(errors)) or (len(errors) != 0) or (not isinstance(ecosystem_collection, Mapping)) or (ecosystem_collection.get('schema_version') != 'mmm/ecosystem-route-collection-receipt-v1') or (not _valid_digest(ecosystem.get('route_sha256'))) or (route_count > 0 and status not in {'available', 'empty'}):
        return None
    unresolved = technical.get('unresolved_official_domains')
    domains = technical.get('domains')
    if technical.get('schema_version') != 'mmm/central-evidence-graph-v1' or not _is_sequence(unresolved) or len(unresolved) != 0 or (not _is_sequence(domains)) or (not _valid_digest(technical.get('evidence_sha256'))):
        return None
...
def _runtime_evidence(runtime: Mapping[str, Any] | None, playtest: Mapping[str, Any] | None) -> EvidenceResult | None:
    ...
    if not isinstance(prepared, Mapping) or prepared.get('schema_version') != 'mmm/runtime-instance-v1' or prepared.get('disposable') is not True or (not isinstance(server, Mapping)) or (server.get('schema_version') != 'mmm/runtime-status-v1') or (server.get('server_running') is not True) or (_positive_int(server.get('server_log_lines')) is None) or (interactions is None) or (assertions is None) or (not _is_sequence(results)) or (len(results) < interactions + assertions):
        return None
    refs = [_evidence_ref('minecraft-runtime', {'minecraft_version': prepared.get('minecraft_version'), 'disposable': True, 'server_log_lines': server.get('server_log_lines')}), _evidence_ref('runtime-playtest', {'interaction_count': interactions, 'assertion_count': assertions, 'results_sha256': _canonical_sha256(_stable_value(results))})]
    return (refs, [runtime, playtest])

def _visual_evidence(contract: Mapping[str, Any], assets: Mapping[str, Any] | None, blockbench: Sequence[Mapping[str, Any]], visual: Mapping[str, Any] | None) -> EvidenceResult | None:
    ...
    if _requires_blockbench(contract):
        ...
        for receipt in blockbench:
            ...
            if not isinstance(uv, Mapping) or uv.get('status') not in {'PASS', 'OK'} or (not isinstance(preview, str)):
                return None
            preview_hash = _regular_file_sha256(Path(preview))
            if preview_hash is None:
                return None
            refs.append(_evidence_ref('blockbench-review', {'entity': receipt.get('entity'), 'uv_status': uv.get('status'), 'preview_sha256': preview_hash}))
            sources.append(receipt)
    return (refs, sources)

def _asset_integrity_evidence(receipt: Mapping[str, Any] | None, expected_ids: set[str]) -> EvidenceResult | None:
    if not isinstance(receipt, Mapping) or receipt.get('status') != 'GENERATED':
        return None
    entries = _named_list_mappings(receipt, 'assets')
    by_id = {str(item.get('asset_id')): item for item in entries if isinstance(item.get('asset_id'), str)}
    ...
    for asset_id in sorted(expected_ids):
        ...
        if actual is None or _digest(item.get('sha256')) != actual:
            return None
        facts.append({'asset_id': asset_id, 'width': width, 'height': height, 'sha256': actual})
...
def _valid_state_validation(value: Mapping[str, Any]) -> bool:
    ...
    not_applicable = value.get('migration_not_applicable') is True or (isinstance(value.get('migration_not_applicable_reason'), str) and bool(value['migration_not_applicable_reason'].strip()))
    return _typed_pass(value, 'mmm/state-validation-v1') and _positive_int(value.get('round_trip_case_count')) is not None and (_positive_int(value.get('restart_case_count')) is not None) and (_positive_int(value.get('corruption_case_count')) is not None) and (migration or not_applicable) and _zero(value.get('data_loss_count'))

def _valid_multiplayer_validation(value: Mapping[str, Any]) -> bool:
    clients = _nonnegative_int(value.get('client_count'))
    return _typed_pass(value, 'mmm/multiplayer-validation-v1') and clients is not None and (clients >= 2) and (_positive_int(value.get('authority_check_count')) is not None) and (_positive_int(value.get('message_validation_check_count')) is not None) and (_positive_int(value.get('reconnect_check_count')) is not None) and (_positive_int(value.get('rejected_invalid_message_count')) is not None) and _zero(value.get('desync_count'))

def _valid_performance_validation(value: Mapping[str, Any]) -> bool:
    if not _typed_pass(value, 'mmm/performance-validation-v1') or _positive_int(value.get('sample_count')) is None or (not _zero(value.get('skipped_work_count'))):
        return False
    budgets = value.get('budgets')
    if not isinstance(budgets, Mapping) or not budgets:
        return False
    return all((isinstance(name, str) and bool(name.strip()) and isinstance(metric, Mapping) and _budget_met(metric) for name, metric in budgets.items()))
...
def _valid_accessibility_validation(value: Mapping[str, Any]) -> bool:
    ...
    covered: set[str] = set()
    for check in checks:
        if not isinstance(check, Mapping) or check.get('passed') is not True or (not isinstance(check.get('path_ref'), str)) or (check.get('path_ref') not in paths) or (not isinstance(check.get('evidence'), str)) or (not check['evidence'].strip()):
            return False
...
def _quality_receipt(*, dimension_id: str, route_ref: str, proposal_hash: str, evidence_refs: Sequence[str], observed_sources: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    refs = sorted(set(evidence_refs))
    receipt_id = 'quality:' + dimension_id + ':' + _canonical_sha256({'dimension_id': dimension_id, 'route_ref': route_ref, 'proposal_hash': proposal_hash, 'evidence_refs': refs}).removeprefix('sha256:')
    return {'dimension_id': dimension_id, 'route_ref': route_ref, 'status': 'PASS', 'proposal_hash': proposal_hash, 'receipt_id': receipt_id, 'observed_at': _observed_at(observed_sources), 'producer': 'mmm.production-orchestrator', 'verified_by': 'mmm.quality-evidence-adapter/v1', 'evidence_refs': refs}
...
def _command_passed(value: Mapping[str, Any]) -> bool:
    return _nonnegative_int(value.get('exit_code')) == 0 and value.get('timed_out', False) is False
...
def _passing_gametest_xml(path: Path) -> dict[str, Any] | None:
    ...
    if failed or test_count <= 0 or suite_count <= 0:
        return None
    return {'report_sha256': digest, 'test_count': test_count, 'suite_count': suite_count}

def _implementation_ids(contract: Mapping[str, Any], kind: str) -> set[str]:
    return {str(item['implementation_id']) for item in contract['implementation_catalog'] if isinstance(item, Mapping) and item.get('source_kind') == kind}
...
def _stable_value(value: Any) -> Any:
    if isinstance(value, Mapping):
        return {str(key): _stable_value(item) for key, item in value.items() if str(key) not in {'observed_at', 'finished_at', 'created_at', 'timestamp'}}
...
def _canonical_sha256(value: Any) -> str:
    try:
        ...
        raise ProductionContractError('quality evidence must be finite JSON') from exc
