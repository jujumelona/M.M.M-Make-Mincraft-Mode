...
def generate_canonical_leaf(inputs, *, leaf_id, router, authority):
    ...
    for slot in spec["slots"]:
        ...
        assert_strict_atomicity_bounds(slot["schema"], surface=f"canonical slot {leaf_id}:{name}")
        values[name] = generate_fixed_template_value(router, "coder", [
            {"role": "system", "content": manifest["task"] + "\n" + "\n".join(manifest.get("rules", ()))},
            {"role": "user", "content": json.dumps({"identity": inputs[manifest["inputs"][0]["name"]],
                "requirement": spec["requirement"], "slot": slot["description"], "bindings": spec["bindings"]}, sort_keys=True)},
        ], response_schema=slot["schema"], enable_tools=False)
    source = render_template({"render": spec["render_mold"]}, values)
    validate_semantic_contract(source, contract=spec, context_id=spec["context_id"], leaf_id=leaf_id)
    result = {}
    for port in manifest["outputs"]:
        if port["type"] == "code_fragment":
            result[port["name"]] = source
        elif port["type"] == "receipt":
            result[port["name"]] = {"leaf_id": leaf_id, "context_id": spec["context_id"],
