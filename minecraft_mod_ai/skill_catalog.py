...
# Keep this reviewed map equal to the staged MCP surface.  It deliberately lives
# outside mcp_server.py so policy compilation does not import and initialize an
# MCP server (mcp_server imports this module to expose the catalog resource).
REVIEWED_TOOL_STAGES: dict[str, frozenset[str]] = {
    "discover_mmm_capabilities": REVIEWED_STAGES,
...
REVIEWED_VALIDATORS = frozenset(
    set(_LEGACY_VALIDATOR_ALIASES.values())
    | {
        "bounded_shards",
...
_YAML_FENCE_RE = re.compile(r"```yaml\s*\r?\n(?P<yaml>.*?)```", re.DOTALL)
_STAGE_PRIORITY = (
    "frontdoor",
    "planning",
    "research",
...
class RetryContract:
    ...
    def allows_retry(
        ...
        *,
        ...
        prior_error_signatures: Iterable[str] = (),
        fresh_evidence: bool,
    ) -> bool:
        ...
        if self.require_fresh_evidence and not fresh_evidence:
            return False
        if (
            self.stop_on_repeated_error_signature
            and error_signature
            and error_signature in frozenset(prior_error_signatures)
        ):
            return False
        return True
...
class ExitContract:
    ...
    def resolve(
        ...
        *,
        ...
        unresolved_external: Iterable[str] = (),
...
class SkillContract:
        ...
        *,
        ...
        if tool not in self.allowed_tools:
            return PolicyDecision(
                False, "tool_not_allowlisted", self.name, tool, stage
            )
        ...
        if (
            ...
            and self.approvals.get("writes", False)
            and not write_approved
        ):
            return PolicyDecision(False, "write_approval_required", self.name, tool, stage)
        if (
            (stage == "runtime" or runtime_requested)
            and self.approvals.get("runtime", False)
            and not runtime_approved
        ):
            return PolicyDecision(
                False, "runtime_approval_required", self.name, tool, stage
            )
    ...
    def failed_validators(self, context: Mapping[str, Any]) -> tuple[str, ...]:
        return tuple(
            ...
            if context.get(validator) is not True
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "tool_routes": dict(self.tool_routes),
...
def compile_skill_contract(
    ...
    if unknown_tools:
        raise SkillPolicyError(
            f"{skill} contains unreviewed tools: {', '.join(unknown_tools)}"
        )
    ...
    if declared_stages is None:
        stage_set = {
            stage
            for tool in tools
            for stage in REVIEWED_TOOL_STAGES[tool]
            if stage != "frontdoor"
        }
        if not stage_set:
            stage_set = {
                stage for tool in tools for stage in REVIEWED_TOOL_STAGES[tool]
            }
    else:
    ...
    if unknown_stages:
        raise SkillPolicyError(
            f"{skill} contains unreviewed stages: {', '.join(unknown_stages)}"
        )
    if not stage_set:
    ...
    for tool in tools:
        ...
        if not candidates:
            raise SkillPolicyError(
                f"{skill} allows {tool}, but none of its reviewed stages are enabled."
            )
        tool_routes[tool] = next(
            stage for stage in _STAGE_PRIORITY if stage in candidates
        )
    ...
    for value in validator_values:
        validator_id = (
            value if value in REVIEWED_VALIDATORS else _LEGACY_VALIDATOR_ALIASES.get(value)
        )
    ...
    retry = RetryContract(
        ...
        stop_on_repeated_error_signature=_bool_field(
            retry_raw,
            "stop_on_repeated_error_signature",
            skill,
        ),
        require_fresh_evidence=bool(
            retry_raw.get(
                ...
                "new " in strategy.casefold() or "diagnostic" in strategy.casefold(),
            )
        ),
    )

    approvals_raw = _mapping(
        policy.get("approval_required"), "approval_required", skill
    )
    approvals = {
        key: _bool_field(approvals_raw, key, skill)
        for key in ("writes", "runtime", "read_only_research")
    }

    exit_raw = _mapping(policy.get("exit_conditions"), "exit_conditions", skill)
    exit_contract = ExitContract(
        success=_string_tuple(exit_raw.get("success"), "exit.success", skill),
        blocked=_string_tuple(exit_raw.get("blocked"), "exit.blocked", skill),
        failed=_string_tuple(exit_raw.get("failed"), "exit.failed", skill),
    )

    return SkillContract(
        name=skill,
        description=str(frontmatter["description"]).strip(),
        activate_when=_string_tuple(
            policy.get("activate_when"), "activate_when", skill
        ),
        stages=tuple(stage for stage in _STAGE_PRIORITY if stage in stage_set),
        allowed_tools=tools,
        tool_routes=tool_routes,
        validators=tuple(validator_ids),
        retry=retry,
        approvals=approvals,
        forbidden_actions=_string_tuple(
            policy.get("forbidden_actions"), "forbidden_actions", skill
        ),
        exit=exit_contract,
    )
...
def validate_skill_catalog(root: str | Path | None = None) -> dict[str, Any]:
    ...
    for skill in CANONICAL_SKILLS:
        ...
        try:
            ...
            if skill in POLICY_NATIVE_SKILLS and set(frontmatter) != {
                "name",
                "description",
            }:
...
def _parse_skill(text: str, expected_name: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for fenced in _YAML_FENCE_RE.finditer(body):
        ...
        if (
            ...
            and candidate.get("schema_version") == "mmm/skill-policy-v1"
        ):
            policy = candidate
            break
    if policy is None:
