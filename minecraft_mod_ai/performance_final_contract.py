...
def install(orchestrator_module: Any, custom_module_generator_module: Any, source_patch_module: Any) -> None:
    _install_locked_source_patcher(source_patch_module)
    _install_serial_shared_writers(orchestrator_module)
    _install_staged_custom_generator(custom_module_generator_module, source_patch_module)
...
def _install_staged_custom_generator(custom_module_generator_module: Any, source_patch_module: Any) -> None:
...
def _clone_snapshot_tree(source_root: Path, *, parent: Path, prefix: str) -> Path:
    ...
    def ignore(directory: str, names: list[str]) -> set[str]:
        ...
        for name in names:
            path = base / name
            if path.is_symlink():
                raise StagedCommitConflict(f'Staging refused project symlink: {path}')
...
def _clone_source_snapshot(live_root: Path) -> Path:
    ...
    return _clone_snapshot_tree(live_root, parent=_snapshot_parent(live_root), prefix='custom-')
...
def _acquire_wave_source_snapshot(live_root: Path) -> Path:
    ...
    with _SNAPSHOT_WAVE_LOCK:
        ...
        with project_write_lock(live_root):
            snapshot = _clone_snapshot_tree(live_root, parent=_snapshot_parent(live_root), prefix='wave-base-')
        _SNAPSHOT_WAVES[live_root] = {'snapshot': snapshot, 'active': 1}
        return snapshot

def _clone_wave_workspace(base_snapshot: Path, live_root: Path) -> Path:
    return _clone_snapshot_tree(base_snapshot, parent=_snapshot_parent(live_root), prefix='custom-')
...
def _commit_staged_operations(*, live_root: Path, staging_root: Path, capture: dict[str, Any], source_patch_module: Any) -> dict[str, Any]:
    ...
    for raw in operations:
        ...
        try:
            ...
            raise StagedCommitConflict(f'Staged custom path escaped project root: {relative}') from exc
        ...
        if kind == 'create':
            ...
            if live_path.exists():
                if live_path.is_file() and live_path.read_text(encoding='utf-8') == desired:
                    unchanged_receipts.append({'path': relative, 'operation': 'create', 'before_sha256': source_patch_module.sha256_file(live_path), 'after_sha256': source_patch_module.sha256_file(live_path)})
                    continue
                raise StagedCommitConflict(f'Concurrent create conflict for {relative}')
            rebased.append(operation)
            continue
        if not live_path.is_file() or live_path.is_symlink():
            raise StagedCommitConflict(f'Concurrent patch target is missing or unsafe: {relative}')
        ...
        if not isinstance(base_bytes, (bytes, bytearray)):
            raise StagedCommitConflict(f'No base snapshot is available to rebase {relative}')
        ...
        if current_text == desired_text:
            unchanged_receipts.append({'path': relative, 'operation': kind, 'before_sha256': current_sha, 'after_sha256': current_sha})
            continue
        merged = _three_way_merge(relative, base_text=base_text, staged_text=desired_text, live_text=current_text)
        rebased.append({'operation': 'replace', 'path': relative, 'expected_sha256': current_sha, 'content': merged})
    if rebased:
        applied = source_patch_module.TransactionalSourcePatcher(live_root).apply(rebased)
    else:
        applied = {'schema_version': 'mmm/source-patch-receipt-v1', 'status': 'UNCHANGED', 'project_root': str(live_root), 'operations': []}
    if unchanged_receipts:
        applied = dict(applied)
        applied['operations'] = [*applied.get('operations', []), *unchanged_receipts]
    return applied

def _three_way_merge(relative: str, *, base_text: str, staged_text: str, live_text: str) -> str:
...
def _merge_json_value(base: Any, staged: Any, live: Any, *, path: str) -> Any:
    ...
    if isinstance(base, dict) and isinstance(staged, dict) and isinstance(live, dict):
        ...
        for key in sorted(set(base) | set(staged) | set(live)):
            ...
            value = _merge_json_value(base_value, staged_value, live_value, path=f'{path}.{key}')
...
def _merge_text_lines(relative: str, *, base_text: str, staged_text: str, live_text: str) -> str:
        ...
        for index, existing in enumerate(merged_edits):
            if candidate[:2] == existing[:2]:
                ...
                if candidate[0] == candidate[1]:
                    ...
                    merged_edits[index] = (existing[0], existing[1], combined)
                    matched = True
                    break
                raise StagedCommitConflict(f'Concurrent text replacement conflict in {relative}')
            if _edit_ranges_overlap(candidate, existing):
                raise StagedCommitConflict(f'Concurrent text edit overlap in {relative}')
        if not matched:
            merged_edits.append(candidate)
    result = list(base)
    for start, end, replacement in sorted(merged_edits, key=lambda item: (item[0], item[1]), reverse=True):
...
def _edit_ranges_overlap(left: tuple[int, int, list[str]], right: tuple[int, int, list[str]]) -> bool:
    l1, l2, _ = left
    r1, r2, _ = right
    if l1 == l2 and r1 == r2:
        return l1 == r1
