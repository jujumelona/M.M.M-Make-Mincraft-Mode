...
_CAPABILITY_LABELS = {'skill_system': '스킬 시스템과 표현', 'general_3d_assets': '사용자 지정 3D 대상', 'custom_entity': '몹과 엔티티', 'unsupported_boss_shape': '요청한 보스 형태', 'unsupported_boss_combat': '요청한 보스 전투 방식', 'item_count_limit': '아이템 수', 'block_count_limit': '블록 수', 'quest_system': '퀘스트', 'class_system': '직업', 'npc_system': 'NPC', 'custom_gui': '사용자 화면과 메뉴', 'custom_animation': '사용자 지정 애니메이션', 'custom_dimension': '사용자 지정 차원', 'sound_system': '사운드와 음악', 'creative_brief': '모드 구성'}
_CAPABILITY_FOCUS_LABELS = {'skill_system': '스킬과 동작 표현', 'general_3d_assets': '3D 모델', 'custom_entity': '몹과 엔티티', 'unsupported_boss_shape': '보스', 'unsupported_boss_combat': '보스', 'item_count_limit': '아이템', 'block_count_limit': '블록', 'quest_system': '퀘스트', 'class_system': '직업', 'npc_system': 'NPC', 'custom_gui': '화면과 메뉴', 'custom_animation': '3D 모델', 'sound_system': '사운드와 음악', 'custom_dimension': '차원'}
_APPROVAL_MESSAGES = {'네', '응', '좋아', '진행', '진행해', '만들어', '만들어줘', '이대로', '이대로만들어', '이대로만들어줘', '이대로진행', '확정', 'yes', 'ok', 'okay', 'go'}

def _initial_history() -> list[dict[str, str]]:
    return [{'role': 'assistant', 'content': _START_MESSAGE}]

def _history_with(history: list[dict[str, str]] | None, role: str, content: str) -> list[dict[str, str]]:
    copied: list[dict[str, str]] = []
    for message in history or []:
        if isinstance(message, dict):
            copied.append({'role': str(message.get('role', 'assistant')), 'content': str(message.get('content', ''))})
        else:
            copied.append({'role': str(getattr(message, 'role', 'assistant')), 'content': str(getattr(message, 'content', ''))})
...
def _existing_input_markdown(report: ExistingProjectReport | None) -> str:
    ...
    return f"기존 모드 수정: **{report.archive_name}** · {report.mod_id or '모드 이름 확인 불가'} · {source_note}"
...
def _explicit_focus(brief: str) -> tuple[str, ...]:
    ...
    for words, label in _FOCUS_LABELS:
        event = latest_intent_event(brief, words, cascade_removals=())
...
def _project_scope(brief: str, proposal: Proposal) -> tuple[str, bool]:
    lowered = brief.lower()
    scope_patterns = ((('(?:대규모|초대형|개큰)(?:\\s+\\S+){0,3}\\s+(?:모드|프로젝트|규모|스케일)', '(?:모드|프로젝트|규모|스케일)(?:\\s+\\S+){0,3}\\s+(?:대규모|초대형|개큰)', '큰\\s*스케일', '(?<![a-z0-9_])(?:huge|massive|large-scale)(?:\\s+[a-z0-9_-]+){0,3}\\s+(?:mod|project|scope|overhaul|conversion)(?![a-z0-9_])', '(?<![a-z0-9_])(?:mod|project|scope|overhaul)(?:\\s+[a-z0-9_-]+){0,3}\\s+(?:huge|massive|large-scale)(?![a-z0-9_])', '(?<![a-z0-9_])total\\s+conversion(?![a-z0-9_])'), '대규모 프로젝트'), (('중규모(?:\\s+\\S+){0,3}\\s+(?:모드|프로젝트|규모|스케일)', '(?:모드|프로젝트|규모|스케일)(?:\\s+\\S+){0,3}\\s+중규모', '(?<![a-z0-9_])(?:medium-scale|medium-sized)(?:\\s+[a-z0-9_-]+){0,3}\\s+(?:mod|project|scope)(?![a-z0-9_])'), '중규모 프로젝트'), (('(?:간단한?|소규모|미니)(?:\\s+\\S+){0,3}\\s+(?:모드|프로젝트)', '(?:모드|프로젝트)(?:\\s+\\S+){0,3}\\s+(?:간단|소규모|미니)', '(?<![a-z0-9_])(?:simple|small)(?:\\s+[a-z0-9_-]+){0,3}\\s+(?:mod|project|scope)(?![a-z0-9_])'), '소규모 프로젝트'))
    candidates = [(match.start(), label) for patterns, label in scope_patterns for pattern in patterns for match in re.finditer(pattern, lowered, re.IGNORECASE)]
    if candidates:
        return (max(candidates, key=lambda item: item[0])[1], True)
    focus = set(_explicit_focus(brief))
    if focus and focus <= {'아이템', '블록'} and (len(proposal.spec.contents) <= 3) and (not proposal.deferred_requests):
...
def _clarification_questions(brief: str, proposal: Proposal) -> tuple[str, ...]:
    ...
    if complex_request and (not scope_is_set):
        ...
        questions.append('대규모 전체 계획 중 첫 플레이 가능 범위에 넣을 구역과 시스템을 말해 주세요.')
    ...
    deferred_capabilities = {request.capability for request in proposal.deferred_requests}
...
def _buildable(proposal: Proposal, questions: tuple[str, ...]) -> bool:
    has_generated_content = bool(proposal.spec.contents or proposal.spec.boss is not None)
...
def _render_plan(proposal: Proposal, questions: tuple[str, ...]) -> str:
    ...
    if scope_label.startswith('현재 요청 기준 소규모') or scope_label == '소규모 프로젝트':
    ...
    lines.extend(('', '## 핵심 게임플레이 루프', '', '- 시작·성장·목표 흐름: 요청에서 확인됨' if _has_core_loop(proposal.requested_prompt) else '- 시작·성장·목표 흐름: 아직 정하지 않음', f'- 첫 플레이 가능 범위: {first_playable}'))
    ...
    if focus:
        ...
        for label in focus:
            if label == '아이템':
                ...
                detail = '형태·전투 정보 확인' if _has_boss_shape(proposal.requested_prompt) and _has_boss_combat(proposal.requested_prompt) else '형태 또는 전투 방식 미정'
                implementation = '지원 형태 검토' if proposal.spec.boss is not None else '전용 구현 연결 필요'
            elif label == '스킬과 동작 표현':
                detail = _skill_behavior_summary(proposal.requested_prompt) or '동작 미정'
                implementation = '전용 구현 연결 필요'
            else:
                detail = '요청에 포함'
                implementation = '전용 구현 연결 필요' if label in deferred_focus else '세부 정보 검토'
    ...
    if scope_label == '대규모 프로젝트':
        ...
        lines.extend(('1. 요구 확정', '2. 콘텐츠 구현과 게임 내 동작 연결', '3. 빌드·GameTest·릴리스 검사'))
    else:
        lines.extend(('1. 전체 규모와 첫 플레이 가능 범위 확정', '2. 대표 기능 버티컬 슬라이스 제작', '3. 검증 결과를 보고 다음 제작 범위 확정'))
    lines.extend(('', '## 검증/릴리스', '', '- 요청한 기능과 생성 결과가 서로 일치해야 합니다.', '- 소스 검증, Fabric 빌드, GameTest가 통과해야 합니다.', '- 실패한 빌드는 설치용 JAR로 제공하지 않습니다.'))
    if scope_label == '대규모 프로젝트':
    ...
    if questions:
        ...
        lines.extend(('', '아직 바로 만들 수 있는 계획이 아닙니다. 필요한 내용을 계속 말해 주세요.'))
    return '\n'.join(lines)

def _is_approval_message(message: str) -> bool:
    normalized = re.sub('[\\s.!?,~]+', '', message.strip().lower())
...
def _result_message(result: Any) -> str:
    ...
    return '## 만들기 중 문제가 발생했습니다\n\n설치용 JAR는 만들지 않았습니다. 계획을 수정하거나 다시 시도해 주세요.'

def create_demo(*, output_root: Path, local_model: bool=False, api_base_url: str | None=None, api_model: str | None=None, api_key: str | None=None, existing_input: str | Path | None=None) -> Any:
    try:
        import gradio as gr
    ...
    existing_report = inspect_existing_project_archive(existing_path) if existing_path is not None else None
    if api_base_url or api_model or api_key:
        if not (api_base_url and api_model and api_key):
            raise ValueError('외부 AI API를 사용하려면 주소, 모델 이름, API 키가 모두 필요합니다.')
        planner = OpenAICompatiblePlanner(base_url=api_base_url, model=api_model, api_key=api_key)
    elif local_model:
    ...
    def execute_proposal(proposal: Proposal | None, history: list[dict[str, str]] | None, source_only: bool) -> tuple[list[dict[str, str]], Proposal | None, Any, str, str | None]:
        if not isinstance(proposal, Proposal):
            response = '먼저 대화로 계획을 만들어 주세요.'
            return (_history_with(history, 'assistant', response), None, gr.update(interactive=False), response, None)
        questions = _clarification_questions(proposal.requested_prompt, proposal)
        if not _buildable(proposal, questions):
            response = '아직 정하지 않았거나 구현과 연결되지 않은 내용이 있습니다. 대화에서 필요한 내용을 더 정해 주세요.'
            return (_history_with(history, 'assistant', response), proposal, gr.update(interactive=False), response, None)
        try:
            result = pipeline.execute(proposal, approval_hash=proposal.approval_hash, output_root=_new_execution_root(output_root), build=not source_only, run_gametest=not source_only, existing_input=existing_path)
            response = _result_message(result)
            return (_history_with(history, 'assistant', response), proposal, gr.update(interactive=False), response, result.release_zip)
        except Exception:
            LOGGER.exception('Mod generation failed')
            response = '만들기에 실패했습니다. 기존 작업과 겹치지 않는 새 작업 공간으로 다시 시도할 수 있습니다. 계획을 수정하거나 다시 눌러 주세요.'
            return (_history_with(history, 'assistant', response), proposal, gr.update(interactive=True), response, None)

    def send_message(message: str, history: list[dict[str, str]] | None, brief: str, proposal: Proposal | None, source_only: bool) -> tuple[str, list[dict[str, str]], str, Proposal | None, Any, str, str | None]:
        message = message.strip()
        if not message:
            return ('', history or _initial_history(), brief, proposal, gr.update(), '', None)
        updated_history = _history_with(history, 'user', message)
        if _is_approval_message(message) and isinstance(proposal, Proposal):
            executed_history, executed_proposal, button_update, result_text, release_path = execute_proposal(proposal, updated_history, source_only)
            return ('', executed_history, brief, executed_proposal, button_update, result_text, release_path)
        updated_brief = _merge_brief(brief, message)
        try:
            updated_proposal = pipeline.plan(updated_brief, existing_input=existing_path)
            ...
            return ('', updated_history, updated_brief, updated_proposal, gr.update(interactive=_buildable(updated_proposal, questions)), '', None)
        except Exception:
            LOGGER.exception('Planning failed')
            response = '요청을 계획으로 정리하지 못했습니다. 내용을 조금 다르게 말해 주세요. 기존 계획은 실행하지 않았습니다.'
            updated_history = _history_with(updated_history, 'assistant', response)
            return ('', updated_history, brief, proposal, gr.update(interactive=False), response, None)

    def approve_current(proposal: Proposal | None, history: list[dict[str, str]] | None, source_only: bool) -> tuple[list[dict[str, str]], Proposal | None, Any, str, str | None]:
        return execute_proposal(proposal, history, source_only)

    def reset_conversation() -> tuple[str, list[dict[str, str]], str, None, Any, str, None]:
        return ('', _initial_history(), '', None, gr.update(interactive=False), '', None)
    with gr.Blocks(title='M.M.M Make Mincraft Mode') as demo:
        ...
        chatbot_kwargs: dict[str, Any] = {'value': _initial_history(), 'label': '모드 제작 대화', 'height': 520, 'layout': 'bubble', 'allow_tags': False}
        if 'type' in __import__('inspect').signature(gr.Chatbot).parameters:
            chatbot_kwargs['type'] = 'messages'
        chatbot = gr.Chatbot(**chatbot_kwargs)
        message = gr.Textbox(label='AI에게 말하기', placeholder='만들고 싶은 모드나 수정할 내용을 자유롭게 적으세요.', lines=3)
        with gr.Row():
            send_button = gr.Button('보내기', variant='primary')
            reset_button = gr.Button('새 대화')
        with gr.Accordion('고급 옵션', open=False):
            source_only = gr.Checkbox(label='소스만 생성하고 빌드는 나중에 하기', value=False)
        build_button = gr.Button('이대로 만들기', variant='primary', interactive=False)
        result_status = gr.Markdown()
        release_file = gr.File(label='완성된 release ZIP')
        brief_state = gr.State('')
        proposal_state = gr.State(None)
        event_inputs = [message, chatbot, brief_state, proposal_state, source_only]
        event_outputs = [message, chatbot, brief_state, proposal_state, build_button, result_status, release_file]
        ...
        _bind_event(build_button.click, approve_current, inputs=[proposal_state, chatbot, source_only], outputs=[chatbot, proposal_state, build_button, result_status, release_file])
        _bind_event(reset_button.click, reset_conversation, inputs=[], outputs=event_outputs)
    return demo

def launch(*, output_root: str | Path='mmm-output', local_model: bool=False, api_base_url: str | None=None, api_model: str | None=None, api_key: str | None=None, share: bool=False, server_name: str='127.0.0.1', existing_input: str | Path | None=None, auth: tuple[str, str] | None=None) -> Any:
    demo = create_demo(output_root=Path(output_root), local_model=local_model, api_base_url=api_base_url, api_model=api_model, api_key=api_key, existing_input=existing_input)
