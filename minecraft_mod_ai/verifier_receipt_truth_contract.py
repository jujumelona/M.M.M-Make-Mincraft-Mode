...
"""Validate durable work-node completion evidence before receipt persistence/reuse.
...
checkable evidence tied to the ledger's exact input hash. Cached verified receipts are
reusable only while verifier input, implementation, and configuration identities match.
"""
...
def _build_evidence(node_id: str, receipt: Mapping[str, Any]) -> dict[str, Any]:
    if node_id != "build-project":
        raise VerifierReceiptTruthError(f"VERIFIER_RECEIPT_UNSUPPORTED_BUILD_NODE: {node_id!r}.")
...
def _verifier_implementation_hash(verifier: str) -> str:
    ...
    return _canonical_hash(
        {"contract": _VERIFIER_REUSE_CONTRACT, "verifier": verifier, "implementation_code_hash": code_hash}
    )


def _stage_evidence(stage: str, node_id: str, receipt: Mapping[str, Any]) -> dict[str, Any]:
    if stage.startswith("generate:"):
        return {"completion_scope": "phase_only", "verifier": None}
    if stage.startswith("validate:"):
        return {"completion_scope": "verified_stage", **_validation_evidence(stage, node_id, receipt)}
    if stage == "build":
        return {"completion_scope": "verified_stage", **_build_evidence(node_id, receipt)}
    if stage.startswith("package"):
        return {"completion_scope": "packaging_stage", **_package_evidence(node_id, receipt)}
    return {"completion_scope": "phase_only", "verifier": None}


def _completion_evidence(row: Mapping[str, Any], receipt: Mapping[str, Any]) -> dict[str, Any]:
        ...
        result.update(
            {
                ...
                "verifier_config_hash": _canonical_hash({"stage": stage, "payload": dict(payload)}),
            }
        )
    return result


def _assert_reusable(node_id: str, expected: Mapping[str, Any], existing: Mapping[str, Any]) -> None:
    if existing.get("input_hash") != expected.get("input_hash") or existing.get("stage") != expected.get("stage"):
        raise VerifierReceiptTruthError(
...
def _validate_arguments(row: Mapping[str, Any], receipt: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    if not isinstance(row, Mapping):
        raise VerifierReceiptTruthError("VERIFIER_RECEIPT_INVALID: work row must be an object.")
    node_id = str(row.get("node_id") or "")
    if not isinstance(receipt, Mapping):
        raise VerifierReceiptTruthError(
            f"VERIFIER_RECEIPT_INVALID: work node {node_id!r} receipt must be an object."
        )
    if not str(row.get("input_hash") or ""):
        raise VerifierReceiptTruthError(
            f"VERIFIER_INPUT_HASH_MISSING: work node {node_id!r} has no exact input identity."
        )
    return node_id, dict(receipt)


def _decorate_receipt(row: Mapping[str, Any], receipt: Mapping[str, Any]) -> dict[str, Any]:
    node_id, result = _validate_arguments(row, receipt)
...
def _assert_receipt_reusable(row: Mapping[str, Any], receipt: Mapping[str, Any]) -> dict[str, Any]:
    """Validate an already-persisted receipt without silently upgrading legacy evidence.

    Cache/read paths must never decorate an old verified receipt on demand: that would
    convert an unverifiable historical PASS into a current PASS. Verified stages fail
    closed unless persisted reuse fingerprints match the current verifier contract.
    """
    ...
    existing = result.get("_mmm_completion_evidence")
    if existing is None:
        if expected.get("verifier") is not None:
            raise VerifierReceiptTruthError(
                f"VERIFIER_RECEIPT_STALE: work node {node_id!r} lacks verifier reuse fingerprints."
            )
        return result
    _assert_reusable(node_id, expected, _mapping(existing))
    return result
...
def install(work_graph_module: Any) -> None:
    ...
    if not getattr(succeed, "_mmm_verifier_receipt_truth_integrated", False):
        raise RuntimeError(
            "verifier receipt truth is not integrated into the canonical durable receipt owner"
        )
