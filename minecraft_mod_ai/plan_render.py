...
_KIND_LABELS_EN = {'item': 'items', 'block': 'blocks', 'tool': 'tools', 'weapon': 'weapons and combat equipment', 'armor': 'armor', 'food': 'food and cooking', 'crop': 'crops and farming', 'fluid': 'fluids', 'machine': 'machines and automation', 'recipe': 'recipes', 'effect': 'status effects', 'enchantment': 'enchantments', 'entity': 'creatures and characters', 'boss': 'boss encounters', 'npc': 'NPCs and interactions', 'quest': 'quests', 'class': 'classes', 'skill': 'skills', 'economy': 'economy', 'shop': 'shops', 'gui': 'game screens and menus', 'networking': 'multiplayer synchronization', 'party': 'parties', 'guild': 'guilds', 'command': 'game commands', 'structure': 'structures and places', 'biome': 'biomes', 'dimension': 'dimensions', 'world_event': 'world events', 'advancement': 'advancements', 'loot': 'rewards and loot', 'integration': 'integration with other mods', 'custom_java': 'custom gameplay logic for the request'}

def render_complete_plan(*, requested_prompt: str, game_design: dict[str, Any], modules: Iterable[ProductionModule], acceptance_tests: Iterable[str]) -> str:
    """Render the user-facing game plan without internal protocol machinery."""
    korean = bool(re.search('[가-힣]', requested_prompt))
    title = _bounded_text(str(game_design.get('title', '')).strip(), korean=korean) or ('새 Minecraft 모드' if korean else 'New Minecraft Mod')
    pitch = _bounded_text(str(game_design.get('pitch', '')).strip(), korean=korean)
    core_loop = _bounded_list(_strings(game_design.get('core_loop')), korean=korean)
    ...
    technology_lines = _bounded_list(_technology_lines(game_design.get('_technology_radar'), korean=korean), korean=korean)
    quality_lines = _quality_lines(game_design.get('_production_contract'), korean=korean)
    if korean:
...
def _art_direction_lines(value: Any, *, korean: bool) -> list[str]:
    ...
    if visual_tone:
        lines.append(f'분위기: {visual_tone}' if korean else f'Tone: {visual_tone}')
...
def _bounded_text(value: str, *, korean: bool) -> str:
    ...
    suffix = f' … (나머지 {remaining}자는 저장된 계획에 보존됨)' if korean else f' … ({remaining} more characters remain in the stored plan)'
    return value[:_VISIBLE_TEXT_CHARS].rstrip() + suffix

def _bounded_list(values: list[str], *, korean: bool) -> list[str]:
    bounded = [_bounded_text(value, korean=korean) for value in values[:_VISIBLE_SECTION_ITEMS]]
    remaining = len(values) - len(bounded)
    if remaining > 0:
        bounded.append(f'그 밖의 {remaining}개 항목은 저장된 계획에 그대로 보존됨' if korean else f'{remaining} more entries remain in the stored plan')
    return bounded
...
def _technology_lines(value: Any, *, korean: bool) -> list[str]:
    ...
    if not isinstance(requirements, list) or not requirements:
        return []
    labels_ko = {'ai_inference': 'AI 추론', 'agent_tool_use': '에이전트 도구 사용', 'language_intersection': '전체 음성 경로의 지원 언어', 'translation': '번역'}
    labels_en = {'ai_inference': 'AI inference', 'agent_tool_use': 'agent tool use', 'language_intersection': 'full-pipeline language support', 'translation': 'translation'}
    topology_ko = {'in_process_java': 'Java 내부', 'local_sidecar': '로컬 보조 프로세스', 'remote_api': '동의된 원격 API', 'offline_build_tool': '빌드 시 오프라인 도구'}
    topology_en = {'in_process_java': 'in-process Java', 'local_sidecar': 'local sidecar', 'remote_api': 'consented remote API', 'offline_build_tool': 'offline build-time tool'}
    ...
    for item in requirements:
        ...
        topologies = [topology_labels.get(str(name), str(name)) for name in item.get('allowed_topologies', []) if str(name).strip()]
...
def _quality_lines(value: Any, *, korean: bool) -> list[str]:
    ...
    if type(requirement_count) is not int or type(check_count) is not int:
        return []
    titles = [str(item.get('title') or '').strip() for item in dimensions if isinstance(item, dict) and str(item.get('title') or '').strip()]
    if korean:
        return [f'요청에서 추적한 요구사항 {requirement_count}개와 관찰 가능한 확인 항목 {check_count}개를 끝까지 연결합니다.', '관련 품질 영역마다 현재 결과에 묶인 독립 검증 증거가 있어야 완성으로 판정합니다.']
    return [f'Trace all {requirement_count} request-derived requirements through {check_count} observable checks.', 'Completion requires fresh, artifact-bound independent evidence for every relevant quality dimension' + (f": {', '.join(titles)}." if titles else '.')]
