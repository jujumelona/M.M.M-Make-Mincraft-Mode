...
Instead, retry the same semantic turn once against the host-selected current action while
keeping the frozen surface available only for parser validation. If that one explicit
re-synchronization fails, fail closed rather than multiplying large-context decodes.
...
def _install_generate_turn(base: type[Any]) -> None:
    ...
    def generate_turn(self: Any, request: Any) -> Any:
        ...
        # here before the core tool loop can execute it. This layer is the only owner
        # of stale-frontier recovery; inner forced-tool handling returns validation-
        # only stale calls unchanged so retries cannot multiply across wrappers.
        ...
        if requested_forced and requested_forced in visible_set:
            forced_name = requested_forced
        else:
            # Preserve the causal adapter's deterministic frontier ordering. Repeated
            # retries against different legal tools would change the intended action
            # while spending additional full-context generations.
            forced_name = visible[0]
        forced_schema = by_name[forced_name]
        ...
        feedback = {
            ...
            "content": (
                ...
                f"Rejected: {', '.join(stale)}. Call exactly {forced_name!r} now with "
                "schema-valid arguments; do not emit any other tool name."
            ),
        }
        ...
        retry_request = replace(
            ...
            # Expose exactly one current legal action. The frozen authorized surface
            # remains validation-only so a repeated stale name can still be parsed and
            # rejected without granting it execution authority.
            tools=forced_tools,
