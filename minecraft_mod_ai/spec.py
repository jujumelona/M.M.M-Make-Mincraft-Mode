...
SHA256_PATTERN = re.compile('^sha256:[0-9a-f]{64}$')
JAVA_RESERVED_WORDS = frozenset({'abstract', 'assert', 'boolean', 'break', 'byte', 'case', 'catch', 'char', 'class', 'const', 'continue', 'default', 'do', 'double', 'else', 'enum', 'exports', 'extends', 'final', 'finally', 'float', 'for', 'goto', 'if', 'implements', 'import', 'instanceof', 'int', 'interface', 'long', 'module', 'native', 'new', 'non-sealed', 'open', 'opens', 'package', 'permits', 'private', 'protected', 'provides', 'public', 'record', 'requires', 'return', 'sealed', 'short', 'static', 'strictfp', 'super', 'switch', 'synchronized', 'this', 'throw', 'throws', 'to', 'transient', 'transitive', 'true', 'try', 'uses', 'var', 'void', 'volatile', 'while', 'with', 'yield', 'false', 'null'})
RESERVED_MOD_IDS = frozenset({'minecraft', 'fabric', 'fabricloader', 'java'})
RESERVED_GENERATED_FIELDS = frozenset({'MOD_ID', 'LOGGER'})
RESERVED_PACKAGE_PREFIXES = ('java.', 'javax.', 'jdk.', 'sun.', 'net.minecraft.', 'com.mojang.', 'net.fabricmc.')
BOSS_MODEL_KINDS = frozenset({'biped_blockbench', 'quadruped_blockbench', 'flying_blockbench', 'serpentine_blockbench', 'construct_blockbench', 'custom_geckolib'})
...
class PlatformLock:
    ...
    def is_unresolved(self) -> bool:
        return not any((self.edition, self.loader, self.minecraft_version, self.java_version, self.yarn_mappings, self.fabric_loader, self.fabric_api, self.fabric_loom, self.gradle))
...
class ContentSpec:
    ...
    def validate(self) -> None:
        ...
        if not ID_PATTERN.fullmatch(self.content_id):
            raise SpecValidationError(f'Invalid content_id {self.content_id!r}; use lowercase snake_case.')
...
class BossSpec:
    ...
    def validate(self) -> None:
        numeric_fields = {'max_health': self.max_health, 'attack_damage': self.attack_damage, 'movement_speed': self.movement_speed, 'scale': self.scale}
        for field_name, value in numeric_fields.items():
            if isinstance(value, bool) or not isinstance(value, (int, float)) or (not math.isfinite(float(value))) or (float(value) <= 0):
                raise SpecValidationError(f'Boss {field_name} must be a positive finite JSON number.')
        ...
        if self.model_kind not in BOSS_MODEL_KINDS:
            raise SpecValidationError(f'Unsupported boss model_kind {self.model_kind!r}; use one of {sorted(BOSS_MODEL_KINDS)}.')
...
class ModSpec:
    ...
    def validate(self) -> None:
        ...
        if not ID_PATTERN.fullmatch(self.mod_id):
            raise SpecValidationError(f'Invalid mod_id {self.mod_id!r}; use lowercase snake_case.')
        ...
        if not PACKAGE_PATTERN.fullmatch(self.package_name):
            raise SpecValidationError(f'Invalid package_name {self.package_name!r}; use a lowercase dotted Java package.')
        if self.package_name.startswith(RESERVED_PACKAGE_PREFIXES):
            raise SpecValidationError(f'Reserved platform package prefix is not allowed: {self.package_name}')
        reserved_components = set(self.package_name.split('.')) & JAVA_RESERVED_WORDS
        if reserved_components:
            raise SpecValidationError(f'Java reserved package component is not allowed: {sorted(reserved_components)}')
        if not re.fullmatch('\\d+\\.\\d+\\.\\d+(?:[-+][A-Za-z0-9._-]+)?', self.version):
        ...
        for content in self.contents:
            ...
            if content.content_id.upper() in RESERVED_GENERATED_FIELDS:
                raise SpecValidationError(f'Content ID collides with a generated Java field: {content.content_id}')
        ...
        if self.boss is not None:
            ...
            if self.boss.entity_id.upper() in RESERVED_GENERATED_FIELDS:
                raise SpecValidationError(f'Boss ID collides with a generated Java field: {self.boss.entity_id}')
            spawn_egg_id = f'{self.boss.entity_id}_spawn_egg'
            if spawn_egg_id in seen:
                raise SpecValidationError(f'Boss spawn egg ID collides with content ID: {spawn_egg_id}')
...
class Proposal:
    ...
    approval_hash: str = ''
    _TOP_LEVEL_KEYS = frozenset({'schema_version', 'proposal_version', 'status', 'requested_prompt', 'spec', 'assumptions', 'exclusions', 'deferred_requests', 'acceptance_tests', 'evidence_sources', 'evidence_snapshot_hash', 'capability_manifest_hash', 'imported_source_snapshot_hash', 'risk_approvals', 'approval_hash'})
    _BACKWARD_COMPATIBLE_KEYS = frozenset({'evidence_snapshot_hash', 'capability_manifest_hash', 'imported_source_snapshot_hash'})
    ...
    def validate(self) -> None:
        ...
        validate_trusted_evidence(self.evidence_sources, minecraft_version=self.spec.platform.minecraft_version)
        if not SHA256_PATTERN.fullmatch(self.evidence_snapshot_hash):
            raise SpecValidationError('evidence_snapshot_hash must be a lowercase sha256 digest.')
        if self.evidence_snapshot_hash != evidence_snapshot_hash(self.evidence_sources):
            raise SpecValidationError('evidence_snapshot_hash does not match the trusted evidence records.')
        if not SHA256_PATTERN.fullmatch(self.capability_manifest_hash):
            raise SpecValidationError('capability_manifest_hash must be a lowercase sha256 digest.')
        if self.imported_source_snapshot_hash and (not SHA256_PATTERN.fullmatch(self.imported_source_snapshot_hash)):
            raise SpecValidationError('imported_source_snapshot_hash must be empty or a lowercase sha256 digest.')
        if self.approval_hash:
            ...
            if self.approval_hash != self.calculate_hash():
                raise SpecValidationError('approval_hash does not match the immutable proposal payload.')

    def _hash_payload(self) -> dict[str, Any]:
        return {'schema_version': self.schema_version, 'proposal_version': self.proposal_version, 'status': ProposalStatus.AWAITING_APPROVAL.value, 'requested_prompt': self.requested_prompt, 'spec': self.spec, 'assumptions': self.assumptions, 'exclusions': self.exclusions, 'deferred_requests': self.deferred_requests, 'acceptance_tests': self.acceptance_tests, 'evidence_sources': self.evidence_sources, 'evidence_snapshot_hash': self.evidence_snapshot_hash, 'capability_manifest_hash': self.capability_manifest_hash, 'imported_source_snapshot_hash': self.imported_source_snapshot_hash, 'risk_approvals': self.risk_approvals, 'approval_hash': ''}
    ...
    def approve(self, supplied_hash: str) -> 'Proposal':
        ...
        if supplied_hash != expected:
            raise SpecValidationError('Approval hash mismatch. The displayed proposal changed or the wrong hash was used.')
