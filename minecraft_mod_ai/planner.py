...
class HeuristicPlanner:
    ...
    def plan(self, prompt: str) -> Proposal:
        ...
        for index in range(item_count_intent.count):
            suffix = (
                item_suffixes[index]
        ...
        block_english = (
            "Ore" if block_base == "ore" else "Crystal Block",
            "Bricks",
            "Tiles",
            "Pillar",
            "Lamp",
        ...
        block_korean = (
            "광석" if block_base == "ore" else "결정 블록",
            "벽돌",
            "타일",
            "기둥",
            "등불",
        ...
        for index in range(block_count_intent.count):
            suffix = (
        ...
        unsupported_boss_shape = asks_boss and _contains(
            prompt,
            ("드래곤", "용형", "짐승형", "골렘", "dragon", "beast", "quadruped", "golem"),
        )
        ...
        evidence_query = f"{prompt} project build metadata dependency " + (
            "boss entity gametest "
            if boss
            else "item block recipe data generation "
        )
...
class OpenAICompatiblePlanner:
    ...
    def plan(self, prompt: str) -> Proposal:
        ...
        return _proposal_from_model_data(prompt, _extract_json_object(content))
...
def _proposal_from_model_data(prompt: str, data: dict[str, Any]) -> Proposal:
    ...
    if not isinstance(raw_contents, list):
        raise SpecValidationError("Local model contents must be a list.")
    ...
    return replace(base, spec=spec, approval_hash="").with_hash()
