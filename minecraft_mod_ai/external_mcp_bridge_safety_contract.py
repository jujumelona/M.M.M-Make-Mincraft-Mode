...
def install(router_module: Any) -> None:
    ...
    more orphan workers. Normal non-nested calls execute independently via anyio so
    unrelated providers are not globally serialized.
    """
    ...
    def call_provider(
        ...
        *,
        ...
        # Normal synchronous calls own independent provider sessions and therefore do
        # not need the router-wide lock. Holding ``self._lock`` here serializes every
        # provider and defeats the runtime hot-path contract. Only the shared orphaned
        # nested-bridge state below requires global synchronization.
        try:
