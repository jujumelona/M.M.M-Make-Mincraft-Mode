...
_ACTIVE_POLICY: ContextVar[dict[str, Any] | None] = ContextVar(
    "mmm_adaptive_central_compute_policy",
    default=None,
)
_SEVERITY = {"none": 0, "low": 1, "medium": 2, "high": 3, "critical": 4}
_HIGH_RISK_EVIDENCE = frozenset(
    {
        "source_code",
        "local_project",
        "testing",
...
def _compute_policy(agentic_module: Any, prompt: str) -> dict[str, Any]:
    ...
    if mode == "lean":
        return {
            "schema_version": "mmm/adaptive-central-compute-v1",
            "tier": "lean",
            "score": 0,
            "reason": "environment_override",
            "central_amplification": False,
    ...
    try:
        brief = agentic_module.normalize_research_brief(
            prompt,
            {"title": "adaptive central compute"},
        )
    except Exception as exc:
        # Unknown routing state is not evidence that a request is cheap. Preserve the
        # historical full path when the cheap host classifier cannot prove otherwise.
        return {
            "schema_version": "mmm/adaptive-central-compute-v1",
            "tier": "full",
            "score": 99,
            "reason": f"classification_unavailable:{type(exc).__name__}",
            "central_amplification": True,
            "review_policy": "full_parallel",
        }
    ...
    if prompt_bytes >= 5_000:
        score += 3
        reasons.append("large_request")
    elif prompt_bytes >= 1_800:
        score += 2
    ...
    # Lists/paragraphs are a cheap proxy for multi-constraint instructions. Require
    # several boundaries so ordinary prose does not get escalated by punctuation.
    ...
    return {
        ...
        "tier": tier,
        "score": score,
        "reason": ",".join(reasons) if reasons else "cheap_host_features",
        "central_amplification": tier != "lean",
        "review_policy": review_policy,
        "domain_count": len(domains),
        "prompt_bytes": prompt_bytes,
        "high_risk_evidence": risky,
...
def harden(agentic_module: Any, central_module: Any) -> None:
    ...
    if not getattr(current_enabled, _MARKER, False):
        ...
        amplification_enabled.__wrapped__ = current_enabled  # type: ignore[attr-defined]
    ...
    if not getattr(current_reviews, _MARKER, False):
        ...
        adaptive_reviews.__wrapped__ = current_reviews  # type: ignore[attr-defined]
