...
_PREFETCH_EXECUTOR = ThreadPoolExecutor(max_workers=2, thread_name_prefix='mmm_model_prefetch')
...
def _ordered_parallel_map(function: Callable[[_T], _R], values: Iterable[_T], *, workers: int) -> list[_R]:
    """Run independent work concurrently while preserving deterministic order."""
    items = list(values)
    if len(items) <= 1 or workers <= 1:
        return [function(item) for item in items]
    with ThreadPoolExecutor(max_workers=min(workers, len(items)), thread_name_prefix='mmm_parallel_io') as pool:
...
def _model_key(config: Any) -> tuple[str, str]:
    ...
    filename = str(extra.get('gguf_filename', '')).strip() if isinstance(extra, dict) else ''
...
def _install_model_prefetch(*, model_registry_module: Any, llama_server_autotune_module: Any) -> None:
...
def _parallel_discover_seed_bundle_factory(ecosystem_module: Any, original_discover: Callable[..., dict[str, Any]]) -> Callable[..., dict[str, Any]]:

    @wraps(original_discover)
    def discover_seed_bundle_parallel(prompt: str, game_design: dict[str, Any], *, research_brief: dict[str, Any] | None=None, client: Any=None, route_cursor: str='', route_limit: int=12) -> dict[str, Any]:
        mode = os.environ.get('MMM_ECOSYSTEM_DISCOVERY', 'auto').strip().lower()
        if mode not in {'auto', 'on', 'off'}:
            raise ecosystem_module.SpecValidationError('MMM_ECOSYSTEM_DISCOVERY must be auto, on or off.')
        if type(route_limit) is not int or not 1 <= route_limit <= 100:
            raise ecosystem_module.SpecValidationError('route_limit must be between 1 and 100.')
        ...
        route_receipt = ecosystem_module._sha256_text(ecosystem_module.canonical_json(routes))
        route_offset = ecosystem_module._decode_seed_route_cursor(route_cursor, route_sha256=route_receipt, route_limit=route_limit)
        if route_offset > len(routes):
            raise ecosystem_module.SpecValidationError('Seed route cursor is beyond the route catalog.')
        selected_routes = routes[route_offset:route_offset + route_limit]
        next_route_offset = route_offset + len(selected_routes)
        next_route_cursor = ecosystem_module._encode_seed_route_cursor(next_route_offset, route_sha256=route_receipt, route_limit=route_limit) if next_route_offset < len(routes) else ''
        if mode == 'off':
            return {'schema_version': 'mmm/ecosystem-seed-bundle-v1', 'status': 'disabled', 'query_sha256': ecosystem_module._sha256_text(query), 'route_sha256': route_receipt, 'route_count': len(routes), 'route_offset': route_offset, 'processed_route_count': 0, 'remaining_route_count': len(routes) - route_offset, 'next_route_cursor': next_route_cursor, 'routes_complete': not next_route_cursor, 'candidate_count': 0, 'pages': [], 'errors': [], 'coverage': 'specialist discovery still required per production batch', 'authorization': 'none', 'download_performed': False}
        ...
        def search_route(route: dict[str, Any]) -> tuple[str, dict[str, Any]]:
            ...
            try:
                page = discovery.search(provider, provider_query, limit=10, target_profile=str(route.get('target_profile', 'minecraft_mod')))
                return ('page', {**page, 'research_domain_id': route['domain_id'], 'route_query_sha256': ecosystem_module._sha256_text(route['query'])})
            except ecosystem_module.EcosystemDiscoveryUnavailable as exc:
                return ('error', {'domain_id': route['domain_id'], 'provider': provider, 'query_sha256': ecosystem_module._sha256_text(route['query']), 'error_type': type(exc).__name__, 'message': str(exc)})
        workers = 1 if client is not None else _env_workers('MMM_DISCOVERY_WORKERS', 8, maximum=32)
        outcomes = _ordered_parallel_map(search_route, selected_routes, workers=workers)
        ...
        if mode == 'on' and (not pages):
            raise ecosystem_module.EcosystemDiscoveryUnavailable('Required ecosystem discovery providers were unavailable.')
        return {'schema_version': 'mmm/ecosystem-seed-bundle-v1', 'status': status, 'query_sha256': ecosystem_module._sha256_text(query), 'route_sha256': route_receipt, 'route_count': len(routes), 'route_offset': route_offset, 'processed_route_count': len(selected_routes), 'remaining_route_count': len(routes) - next_route_offset, 'next_route_cursor': next_route_cursor, 'routes_complete': not next_route_cursor, 'candidate_count': candidate_count, 'pages': pages, 'errors': errors, 'coverage': 'seed pages only; continue each provider cursor and run exact project inspection for every dependency or third-party asset considered', 'authorization': 'none', 'download_performed': False}
    discover_seed_bundle_parallel._mmm_parallel_routes = True
    return discover_seed_bundle_parallel

def _parallel_retrieve_domain_evidence_factory(central_module: Any, original_retrieve_graph: Callable[..., dict[str, Any]]) -> Callable[..., dict[str, Any]]:
    original_default_retrieve = central_module.retrieve_official_evidence

    @wraps(original_retrieve_graph)
    def retrieve_domain_evidence_parallel(research_brief: dict[str, Any], *, retrieve: Callable[..., Any] | None=None) -> dict[str, Any]:
        ...
        if len(ordered_jobs) <= 1:
            return original_retrieve_graph(research_brief, retrieve=selected_retrieve)
        ...
        with ThreadPoolExecutor(max_workers=min(workers, len(ordered_jobs)), thread_name_prefix='mmm_official_rag') as pool:
            future_to_job = {pool.submit(selected_retrieve, query, minecraft_version=adapter.minecraft_version, loader=adapter.loader, mappings=adapter.yarn_mappings, limit=8): (domain_index, query_index, query) for domain_index, query_index, query in ordered_jobs}
            for future in as_completed(future_to_job):
                ...
                for correction_index, correction_query in enumerate(primary.correction_queries):
                    correction_futures[domain_index, query_index, correction_index] = pool.submit(selected_retrieve, correction_query, minecraft_version=adapter.minecraft_version, loader=adapter.loader, mappings=adapter.yarn_mappings, limit=4)
            ...
            for domain_index, domain in enumerate(domains):
                ...
                for query_index, query in enumerate(domain.queries):
                    ...
                    for correction_index, _correction_query in enumerate(primary.correction_queries):
                        correction = correction_futures[domain_index, query_index, correction_index].result()
                        corrections.append(correction.to_dict())
                        has_hits = has_hits or bool(correction.hits)
                    has_hits = has_hits or bool(primary.hits)
                    query_results.append({'query_sha256': central_module._sha256(query), 'strategy': 'single' if not primary.correction_required else 'corrective_multi_hop', 'primary': primary.to_dict(), 'corrections': corrections})
                if not has_hits:
        ...
        payload = {'schema_version': 'mmm/central-evidence-graph-v1', 'brief_sha256': research_brief.get('brief_sha256', ''), 'domains': results, 'unresolved_official_domains': unresolved, 'authorization': 'none', 'retrieval_is_authority': False}
        payload['evidence_sha256'] = central_module._sha256(central_module.canonical_json(payload))
        return payload
    retrieve_domain_evidence_parallel._mmm_parallel_rag = True
    return retrieve_domain_evidence_parallel

def install(*, complete_planner_module: Any, model_registry_module: Any, llama_server_autotune_module: Any) -> None:
    ...
    _install_model_prefetch(model_registry_module=model_registry_module, llama_server_autotune_module=llama_server_autotune_module)
    ...
    if getattr(original_discover, '_mmm_parallel_routes', False):
        ...
        parallel_discover = _parallel_discover_seed_bundle_factory(ecosystem_module, original_discover)
    ...
    if getattr(original_retrieve, '_mmm_parallel_rag', False):
        ...
        parallel_retrieve = _parallel_retrieve_domain_evidence_factory(central_module, original_retrieve)
