...
def compile_minecraft_knowledge_plan(prompt: str, game_design: Mapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    requirements = [{'requirement_id': f'mk:{node_id}', 'knowledge_id': node_id, 'domain': NODES[node_id].domain, 'objective': NODES[node_id].objective, 'depends_on': [f'mk:{dep}' for dep in NODES[node_id].deps], 'feature_refs': sorted((f for f, closure in closures.items() if node_id in closure)), 'side': NODES[node_id].side, 'evidence': {'rag_queries': [NODES[node_id].query], 'mcp_capabilities': list(NODES[node_id].mcp), 'providers': list(NODES[node_id].providers), 'evidence_kinds': list(NODES[node_id].kinds)}, 'validations': list(NODES[node_id].checks)} for node_id in order]
    ...
    plan = {'schema_version': SCHEMA_VERSION, 'features': list(features), 'knowledge_order': list(order), 'requirements': requirements, 'research_domains': research_domains, 'mcp_requirements': [{'capability': cap, 'requirement_refs': sorted((str(item['requirement_id']) for item in requirements if cap in item['evidence']['mcp_capabilities']))} for cap in all_mcp], 'policy': {'dependency_expansion_owner': 'host', 'model_may_remove_requirements': False, 'versioned_rag_required_before_design': True, 'mcp_capability_selection_owner': 'host', 'mcp_execution': 'research_agent_or_downstream_specialist_when_exact_lookup_is_needed', 'validation_selection_owner': 'host'}, 'plan_sha256': ''}
...
def evaluate_route_coverage(plan: Mapping[str, Any], research: Mapping[str, Any]) -> dict[str, Any]:
    ...
    brief_ids = {str(x.get('domain_id', '')) for x in research.get('research_brief', {}).get('domains', []) if isinstance(x, Mapping)}
    ...
    for domain_id, domain in expected.items():
        ...
        if status in {'MISSING_RESEARCH_DOMAIN', 'MISSING_FORCED_RAG_RECEIPT', 'MISSING_FORCED_RAG_QUERY', 'MISSING_RESEARCH_AGENT_NOTE', 'RESEARCH_UNRESOLVED'}:
        ...
        receipts.append({'domain_id': domain_id, 'status': status, 'query_count': len(domain['queries']), 'forced_query_count': len(raw_queries), 'missing_query_sha256': missing, 'research_agent_sufficient': bool(note.get('sufficient')) if isinstance(note, Mapping) else False, 'research_agent_fixed_point': bool(note.get('fixed_point')) if isinstance(note, Mapping) else False})
    result = {'schema_version': COVERAGE_SCHEMA_VERSION, 'plan_sha256': plan['plan_sha256'], 'status': 'PASS' if not blocking else 'BLOCK', 'blocking_requirement_refs': sorted(set(blocking)), 'deferred_requirement_refs': sorted(set(deferred)), 'domains': receipts, 'semantics': 'PASS proves every host-required domain entered the brief, every deterministic forced-RAG query has an execution receipt, and every domain research agent produced a terminal note. A terminal fixed point may retain explicit deferred gaps for downstream exact lookup/validation; PASS does not claim those gaps are resolved or that optional MCP lookups ran.', 'coverage_sha256': ''}
