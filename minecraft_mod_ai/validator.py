...
class ProjectValidator:
    ...
    def validate(self, root: Path, spec: ModSpec) -> ValidationReport:
        ...
        if raw_root.is_symlink():
            return _invalid_project_root(raw_root)
        try:
            ...
            try:
                ...
                findings.append(
                    Finding(
                        ...
                        "error",
                        relative,
                        f"Could not stat validation input: {type(exc).__name__}: {exc}",
                    )
                )
        ...
        if not java_files:
            findings.append(
                Finding("NO_JAVA", "error", ".", "No Java sources were generated.")
            )
            ...
            try:
                ...
                findings.append(
                    Finding(
                        ...
                        "error",
                        self._rel(root, path),
                        f"Could not read Java source: {type(exc).__name__}: {exc}",
                    )
                )
                continue
            relative = self._rel(root, path)
    ...
    def _validate_content(
        ...
        if content.recipe:
            ...
            if (
                ...
                or result.get("item") != f"{spec.mod_id}:{content.content_id}"
            ):
                findings.append(
                    Finding(
                        "BAD_RECIPE_RESULT",
    ...
    def _validate_fabric_metadata(
        ...
        checks += 1
        if main_class not in _entrypoint_values(entrypoints.get("main")):
            findings.append(
                Finding(
                    "BAD_ENTRYPOINTS",
                    "error",
                    self._rel(root, path),
                    f"main must include {main_class}.",
                )
            )
    ...
    def _validate_no_unapproved_client_or_entity(
        ...
        for forbidden_root in forbidden_roots:
            ...
            if forbidden_root.is_dir() and any(
                ...
                for path in forbidden_root.rglob("*")
            ):
                findings.append(
                    Finding(
    ...
    def _validate_local_ai_sidecar_sources(
        ...
        for module in complete.modules:
            ...
            for relative, expected, code, label in (
                ...
                path = root / relative
                try:
                    matches = _regular_file(path) and path.read_text(
                        encoding="utf-8",
                        errors="replace",
                    ) == expected
                except OSError:
                ...
                if not matches:
                    findings.append(
                        Finding(
                            ...
                            "error",
                            relative,
                            f"Reviewed local AI sidecar {label} must exactly match the approved deterministic reconstruction.",
                        )
                    )
        return checks
        ...
        if not _regular_file(path):
            findings.append(
                Finding(
                    ...
                    "error",
                    self._rel(root, path),
                    "Required JSON is missing or unsafe.",
                )
            )
            return {}
        try:
    ...
    def _validate_boss(
        ...
        required = (
            ...
            / package_path
            / "client"
            / f"{renderer_class}.java",
            root
            / "src/main/java"
            / package_path
            / "client"
            / f"{main_class}Client.java",
        ...
        try:
            ...
            findings.append(
                Finding(
                    ...
                    "error",
                    self._rel(root, path),
                    f"Could not read PNG: {type(exc).__name__}: {exc}",
                )
            )
            return
        relative = self._rel(root, path)
...
def _invalid_project_root(root: Path) -> ValidationReport:
    return ValidationReport(
        status="FAIL",
        checks_run=1,
        findings=(
    ...
    if not _regular_file(path):
        findings.append(
            Finding(
                ...
                "Complete proposal path is unsafe.",
            )
        )
        return None
    try:
...
def _reviewed_local_ai_sidecar_network_sources(
    ...
    for module in complete.modules:
        ...
        try:
            ...
            if (
                _regular_file(source_path)
                and _regular_file(manifest_path)
...
def _load_complete_jar_proposal(
    ...
    try:
        ...
        findings.append(
            Finding("JAR_INVALID_COMPLETE_PROPOSAL", "error", name, str(exc))
        )
        return None
...
def validate_jar(
    ...
    *,
    ...
    raw_jar = Path(jar_path).expanduser()
    if raw_jar.is_symlink():
        return _invalid_jar(raw_jar, "Built JAR is a symbolic link.")
    try:
        jar_path = raw_jar.resolve(strict=True)
    except (FileNotFoundError, OSError, RuntimeError):
        return _invalid_jar(raw_jar, "Built JAR is missing or unsafe.")
    ...
    try:
            ...
            for group, raw in entrypoints.items():
                for value in _entrypoint_values(raw):
                    ...
                    if class_path not in names:
                        ...
                        try:
                            magic = archive.read(class_path)[:4]
        ...
        findings.append(
            Finding(
                ...
                "error",
                str(jar_path),
                f"JAR integrity validation failed: {type(exc).__name__}: {exc}",
            )
        )
...
def _invalid_jar(path: Path, message: str) -> ValidationReport:
    return ValidationReport(
        status="FAIL",
        checks_run=1,
        findings=(Finding("JAR_MISSING", "error", str(path), message),),
    )
