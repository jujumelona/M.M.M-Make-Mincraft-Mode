    ...
    while True:
        try:
            raw = router.generate_text(
                "planner",
                _messages(prompt, clauses, diagnostic, previous),
                response_format="json",
                response_schema=_semantic_schema(len(clauses)),
                enable_tools=False,
            )
            ...
            diagnostic = _diagnostic(
                ...
                "valid JSON matching the semantic requirement graph schema",
                "whole_response",
            )
        else:
            nodes, validation_error = _validate_candidate(
                payload,
                prompt=prompt,
                clauses=clauses,
            )
        ...
        if failure_state in seen_failures:
            raise _evidence.EvidencePlanError(
                "semantic requirement approval reached a no-progress fixed point: "
                + _canonical(diagnostic)
            )
        seen_failures.add(failure_state)
        previous = dict(payload) if isinstance(payload, Mapping) else None
