...
def _context_size(config: Any | None = None) -> int:
    ...
    The model advertises a larger native context, but MMM already pages planning and
    source evidence. Reserving 32K KV for every request wastes T4 memory and reduces
    decode headroom. Operators can explicitly raise the window for an exceptional
    request without re-enabling runtime tuning sweeps.
    """
...
def _install_measured_fast_base_args(autotune: Any) -> None:
    ...
    def measured_base_args(
        ...
        # Keep llama.cpp's native KV representation used by the measured path.
        # Runtime KV sweeps repeatedly restart the 6 GB model and belong in offline
        # benchmarking, not in a production generation request.
        _drop_option(args, ("--cache-type-k", "-ctk"))
        _drop_option(args, ("--cache-type-v", "-ctv"))

        # Load-mode/prompt-cache experiments affect startup/reuse rather than the
        # target single-stream TG path and previously added avoidable VRAM pressure.
