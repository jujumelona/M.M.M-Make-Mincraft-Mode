...
def _install_model_runtime_contracts() -> None:
    ...
    from . import complete_orchestrator_services, complete_planner, llama_server_autotune, llama_server_hardware_policy, llama_server_runtime_tuning, model_registry, model_router
    ...
    install_gpu_handoff(services_module=complete_orchestrator_services, model_router_module=model_router)
    install_native_llama_tuning_pipeline(autotune=llama_server_autotune, hardware_policy=llama_server_hardware_policy, runtime_tuning=llama_server_runtime_tuning)
    install_llama_stream_efficiency(llama_server_hardware_policy)
    install_parallel_runtime(complete_planner_module=complete_planner, model_registry_module=model_registry, llama_server_autotune_module=llama_server_autotune)
...
def _install_generation_contracts() -> None:
    ...
    from . import complete_orchestrator, custom_module_generator, extended_content_generator, performance_final_contract, project_index, source_patch
...
def _install_platform_contracts() -> None:
    ...
    from . import agentic_pre_design_rag, agentic_research_game_design, central_intelligence_amplifier, central_research, complete_orchestrator, complete_planner, complete_spec, custom_module_generator, game_design, geckolib_generator, generator, mineflayer_bridge, minecraft_knowledge_contract, production_contract, repair_engine, retrieval, runtime_manager, spec, system_pack_generator, system_templates_common, technology_radar, validator
    ...
    install_platform_runtime(orchestrator_module=complete_orchestrator, runtime_manager_module=runtime_manager, mineflayer_module=mineflayer_bridge)
    ...
    install_platform_planning(game_design_module=game_design, complete_planner_module=complete_planner, central_research_module=central_research)
    ...
    install_platform_central_ai(game_design_module=game_design, complete_planner_module=complete_planner)
    ...
    install_specialized_generator_guards(system_module=system_pack_generator, geckolib_module=geckolib_generator, orchestrator_module=complete_orchestrator)
    install_system_quality(templates_module=system_templates_common, system_module=system_pack_generator, production_contract_module=production_contract, runtime_module=runtime_manager, orchestrator_module=complete_orchestrator)
...
def _install_architecture_contracts() -> None:
    ...
    from . import agentic_optimization_contract, atomic_requirement_contract, complete_orchestrator, complete_planner, custom_module_generator, production_contract, quality_evidence, repair_engine, validation_execution_contract, work_graph
    ...
    agentic_optimization_contract.install(complete_planner_module=complete_planner, repair_module=repair_engine, work_graph_module=work_graph)
...
def _install_late_safety_contracts() -> None:
    ...
    from . import complete_orchestrator, model_router, production_tools, runner, scheduler_parallel_safety_contract, validation_execution_contract, work_graph
    ...
    install_scheduler_parallel_safety(work_graph_module=work_graph, orchestrator_module=complete_orchestrator)
    ...
    install_scheduler_claim_fencing(work_graph_module=work_graph, orchestrator_module=complete_orchestrator)
...
def _install_post_bootstrap_contracts() -> None:
    ...
    from . import agentic_optimization_contract, agentic_pre_design_rag, agentic_research_game_design, central_research, ecosystem_discovery, model_router, production_tools, repair_engine, research_coordinator, work_graph
    ...
    install_agent_security(pre_design_rag_module=agentic_pre_design_rag, agentic_research_module=agentic_research_game_design, model_router_module=model_router)
    install_small_model_max_agent(model_router_module=model_router, pre_design_rag_module=agentic_pre_design_rag, production_tools_module=production_tools, repair_module=repair_engine, optimization_module=agentic_optimization_contract)
    ...
    install_temporary_skill(model_router_module=model_router, work_graph_module=work_graph, repair_module=repair_engine)
    ...
    if not getattr(current_tool_loop, '_mmm_lossless_context_compaction', False):
        ...
        def _generate_with_compaction(self, *, adapter, request, runtime, stage, role):
            return current_tool_loop(self, adapter=CompactingAdapter(adapter), request=request, runtime=runtime, stage=stage, role=role)
