...
class ScalePolicy:
    ...
    model_context_bytes: int = 1500000
    ...
    def validate(self) -> None:
        positive = {'java_shard_size': self.java_shard_size, 'entity_shard_size': self.entity_shard_size, 'function_shard_size': self.function_shard_size, 'world_placements_per_tick': self.world_placements_per_tick, 'model_context_bytes': self.model_context_bytes, 'mcp_page_bytes': self.mcp_page_bytes, 'max_patch_bytes': self.max_patch_bytes, 'max_single_file_bytes': self.max_single_file_bytes, 'max_texture_dimension': self.max_texture_dimension, 'repair_attempts': self.repair_attempts, 'nbt_piece_axis': self.nbt_piece_axis, 'nbt_piece_volume': self.nbt_piece_volume, 'gradle_min_heap_mb': self.gradle_min_heap_mb, 'gradle_max_heap_mb': self.gradle_max_heap_mb, 'runtime_min_heap_mb': self.runtime_min_heap_mb, 'runtime_max_heap_mb': self.runtime_max_heap_mb}
        ...
        if self.nbt_piece_axis > 48:
            raise ScalePolicyError('nbt_piece_axis may not exceed the reviewed vanilla structure-template axis of 48')
    ...
    def runtime_heap_mb(self, *, module_count: int, entity_count: int, structure_count: int) -> int:
        estimate = self.runtime_min_heap_mb + module_count * 3 + entity_count * 24 + structure_count * 16
