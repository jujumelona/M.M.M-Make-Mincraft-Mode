...
class MMMToolService:
    ...
    def __init__(self, *, workspace_root: str | Path='mmm-output', profile: str='t4_local', router_factory: Callable[[], ModelRouter] | None=None, discovery_client_factory: Callable[[], EcosystemDiscoveryClient] | None=None, policy: ScalePolicy | None=None) -> None:
        self.workspace_root = Path(workspace_root).expanduser().resolve()
        self.workspace_root.mkdir(parents=True, exist_ok=True)
        self.profile = profile
        self.router_factory = router_factory or (lambda: ModelRouter(profile=profile))
        self.discovery_client_factory = discovery_client_factory or EcosystemDiscoveryClient
    ...
    def discover_ecosystem_resources(self, provider: str, query: str, cursor: str='', limit: int=20, target_profile: str='minecraft_mod') -> dict[str, Any]:
        """Search one read-only, evidence-only ecosystem page."""
        return self.discovery_client_factory().search(provider, query, cursor=cursor, limit=limit, target_profile=target_profile)

    def inspect_modrinth_project(self, project_id: str) -> dict[str, Any]:
        """Inspect project version metadata without downloading a JAR."""
        return self.discovery_client_factory().inspect_modrinth_project(project_id)
    ...
    def inspect_github_repository(self, full_name: str) -> dict[str, Any]:
        ...
        return self.discovery_client_factory().inspect_github_repository(full_name)
    ...
    def inspect_huggingface_model(self, repo_id: str) -> dict[str, Any]:
        ...
        return self.discovery_client_factory().inspect_huggingface_model(repo_id)

    @staticmethod
    def build_technology_radar(prompt: str, research_brief: dict[str, Any] | None=None, cursor: str='', page_size: int=50) -> dict[str, Any]:
        return create_technology_radar(prompt, research_brief, cursor=cursor, page_size=page_size)

    def assess_technology_compatibility(self, requirement: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
        """Fail closed on compatibility, license, consent and benchmark gates."""
        assessed_candidate = dict(candidate)
        assessed_candidate['official_target_evidence'] = _build_signed_official_target_evidence(requirement, receipt_key=self._technology_receipt_key)
    ...
    def _plan_complete_game_impl(self, prompt: str, media_paths: Sequence[str]=(), existing_input_sha256: str='') -> dict[str, Any]:
    ...
    def revise_complete_plan(self, original_prompt: str, revision: str, media_paths: Sequence[str]=(), existing_input_sha256: str='') -> dict[str, Any]:
        try:
    ...
    def approve_complete_plan(self, complete_proposal: dict[str, Any] | None=None, approval_hash: str='', proposal_ref: str='') -> dict[str, Any]:
        return self._approve_complete_plan_impl(complete_proposal=complete_proposal, approval_hash=approval_hash, proposal_ref=proposal_ref)

    def _approve_complete_plan_impl(self, complete_proposal: dict[str, Any] | None=None, approval_hash: str='', proposal_ref: str='') -> dict[str, Any]:
    ...
    def execute_complete_project(self, complete_proposal: dict[str, Any] | None=None, approval_hash: str='', run_name: str='', options: dict[str, Any] | None=None, existing_input: str | None=None, proposal_ref: str='') -> dict[str, Any]:
        return self._execute_complete_project_impl(complete_proposal=complete_proposal, approval_hash=approval_hash, run_name=run_name, options=options, existing_input=existing_input, proposal_ref=proposal_ref)

    def _execute_complete_project_impl(self, complete_proposal: dict[str, Any] | None=None, approval_hash: str='', run_name: str='', options: dict[str, Any] | None=None, existing_input: str | None=None, proposal_ref: str='') -> dict[str, Any]:
        ...
        if scoped_options.get('server_launcher'):
            scoped_options['server_launcher'] = str(self._existing_file(str(scoped_options['server_launcher'])))
    ...
    def read_complete_plan_section(self, proposal_ref: str, section: str='overview', cursor: str='', limit: int=100) -> dict[str, Any]:
        return self._read_complete_plan_section_impl(proposal_ref, section=section, cursor=cursor, limit=limit)

    def _read_complete_plan_section_impl(self, proposal_ref: str, section: str='overview', cursor: str='', limit: int=100) -> dict[str, Any]:
    ...
    def read_quality_contract(self, proposal_ref: str) -> dict[str, Any]:
        return self._read_quality_contract_impl(proposal_ref)

    def _read_quality_contract_impl(self, proposal_ref: str) -> dict[str, Any]:
        ...
        return {'schema_version': 'mmm/quality-contract-summary-v1', 'proposal_ref': stored_ref, 'summary': quality_contract_summary(contract), 'catalog_stats': dict(contract['catalog_stats']), 'quality_dimensions': [{'dimension_id': item['dimension_id'], 'title': item['title'], 'activation': item['activation'], 'objective': item['objective'], 'evidence_route_ref': item['evidence_route_ref']} for item in contract['quality_dimension_catalog']], 'completion_policy': dict(contract['completion_policy'])}
    ...
    def quality_status(self, run_name: str) -> dict[str, Any]:
        ...
        if not isinstance(run_name, str) or not _RUN_NAME.fullmatch(run_name):
            raise SpecValidationError('run_name must use lowercase letters, numbers, underscore or hyphen.')
        ...
        if run_root.is_symlink() or target.is_symlink() or (not target.is_file()):
            raise FileNotFoundError(f'Quality report not found for run: {run_name}')
        ...
        return {**report, 'run_name': run_name, 'unresolved_dimension_ids': list(unresolved)}

    def apply_source_patch(self, project_root: str, operations: list[dict[str, Any]]) -> dict[str, Any]:
        root = self._existing_dir(project_root)
        encoded = json.dumps(operations, ensure_ascii=False, allow_nan=False).encode('utf-8')
        if len(encoded) > self.policy.max_patch_bytes:
            raise SpecValidationError('Patch exceeds MMM_MAX_PATCH_BYTES host resource policy.')
    ...
    def approve_plan(self, proposal: dict[str, Any], approval_hash: str) -> dict[str, Any]:
    ...
    def generate_assets(self, assets: dict[str, str], output_dir: str='assets-generated', seed: int=0) -> dict[str, Any]:
        if not isinstance(assets, dict) or not assets:
            raise SpecValidationError('assets must be a non-empty id-to-prompt object.')
        if type(seed) is not int:
            raise SpecValidationError('seed must be a JSON integer.')
        try:
            encoded = json.dumps(assets, ensure_ascii=False, allow_nan=False, sort_keys=True).encode('utf-8')
        except (TypeError, ValueError) as exc:
            raise SpecValidationError('assets must contain JSON string prompts.') from exc
        if len(encoded) > self.policy.max_patch_bytes:
            raise SpecValidationError('Asset request exceeds MMM_MAX_PATCH_BYTES host resource policy.')
        ...
        for asset_id, prompt in assets.items():
            ...
            if len(prompt.encode('utf-8')) > self.policy.max_single_file_bytes:
                raise SpecValidationError(f'Asset prompt exceeds host resource policy: {asset_id}')
        ...
        for index, (asset_id, prompt) in enumerate(sorted(normalized)):
            concept = router.generate_image('image_generator', prompt='Minecraft Java resource-pack source art, centered object, clean silhouette, no text, no watermark, square composition. ' + prompt, output_path=concepts / f'{asset_id}.png', width=512, height=512, seed=seed + index)
            texture = textures / f'{asset_id}.png'
            _minecraft_texture(concept, texture)
            generated.append({'asset_id': asset_id, 'concept_png': str(concept), 'texture_png': str(texture), 'concept_sha256': _sha256(concept), 'texture_sha256': _sha256(texture)})
        return {'schema_version': 'mmm/asset-result-v2', 'output_dir': str(target), 'generated': generated, 'asset_count': len(generated), 'warning': 'Generated textures require VisualCritic review before release.'}

    def run_static_validation(self, project_root: str, proposal: dict[str, Any], approval_hash: str) -> dict[str, Any]:
        approved = self._approved(proposal, approval_hash)
        root = self._existing_dir(project_root)
        self.broker.authorize(approved_request(ToolAction.VALIDATE, project_root=root, workspace_root=self.workspace_root, proposal=approved), approved)
        return ScalableProjectValidator(policy=self.policy).validate(root, approved.spec).to_dict()

    def run_gradle_build(self, project_root: str, proposal: dict[str, Any], approval_hash: str) -> dict[str, Any]:
        return self._run_gradle(project_root, proposal, approval_hash, run_gametest=False)

    def run_gametest(self, project_root: str, proposal: dict[str, Any], approval_hash: str) -> dict[str, Any]:
        return self._run_gradle(project_root, proposal, approval_hash, run_gametest=True)

    def inspect_jar(self, jar_path: str, proposal: dict[str, Any]) -> dict[str, Any]:
        parsed = Proposal.from_dict(proposal)
    ...
    def package_release(
        ...
        source_report = ScalableProjectValidator(policy=self.policy).validate(root, approved.spec)
    ...
    def _run_gradle(self, project_root: str, proposal: dict[str, Any], approval_hash: str, *, run_gametest: bool) -> dict[str, Any]:
        ...
        self.broker.authorize(approved_request(action, project_root=root, workspace_root=self.workspace_root, proposal=approved), approved)
    ...
    def _store_complete_proposal(self, proposal: CompleteProposal) -> str:
        return self._store_complete_proposal_impl(proposal)

    def _store_complete_proposal_impl(self, proposal: CompleteProposal) -> str:
        if isinstance(proposal, AuthoredPlan):
        ...
        if index.exists():
            if not index.is_file() or index.is_symlink():
                raise SpecValidationError('Stored proposal index is unsafe.')
        ...
        if index.parent.is_symlink():
            raise SpecValidationError('Stored proposal directory may not be a symlink.')
        write_sharded_complete_proposal(proposal, index, shard_size=self.policy.java_shard_size, policy=self.policy)
        stored = load_sharded_complete_proposal(index)
        if stored.calculate_hash() != expected_hash:
            raise SpecValidationError('Stored proposal verification failed.')
        self._proposal_cursor_key(index, create=True)
        return f"plan_{digest}_{_sha256(index).removeprefix('sha256:')}"

    def _resolve_complete_proposal(self, *, complete_proposal: dict[str, Any] | None, proposal_ref: str) -> tuple[CompleteProposal, str]:
        return self._resolve_complete_proposal_impl(complete_proposal=complete_proposal, proposal_ref=proposal_ref)

    def _resolve_complete_proposal_impl(self, *, complete_proposal: dict[str, Any] | None, proposal_ref: str) -> tuple[CompleteProposal, str]:
        has_inline = complete_proposal is not None
        has_ref = bool(proposal_ref)
        if has_inline == has_ref:
            raise SpecValidationError('Supply exactly one of complete_proposal or proposal_ref.')
        if has_inline:
    ...
    def _proposal_index_for_ref(self, proposal_ref: str, *, require_existing: bool) -> tuple[Path, str, str]:
        ...
        if match is None:
            raise SpecValidationError('proposal_ref is not a valid M.M.M plan reference.')
        digest = match.group(1)
        index_digest = match.group(2)
        index = self._proposal_index_for_digest(digest, require_existing=require_existing)
        return (index, f'sha256:{digest}', f'sha256:{index_digest}')

    def _proposal_index_for_digest(self, digest: str, *, require_existing: bool) -> Path:
        if not re.fullmatch('[0-9a-f]{64}', digest):
            raise SpecValidationError('Proposal digest is invalid.')
        logical_parts = (self.workspace_root / '.minecraft_ai', self.workspace_root / '.minecraft_ai' / 'plans', self.workspace_root / '.minecraft_ai' / 'plans' / digest)
        for part in logical_parts:
            if part.exists() and part.is_symlink():
                raise SpecValidationError('Proposal storage directories may not be symlinks.')
        index = (logical_parts[-1] / 'complete-proposal.json').resolve()
        try:
            index.relative_to(self.workspace_root)
        except ValueError as exc:
            raise SpecValidationError('Proposal reference escaped the configured workspace.') from exc
        if require_existing and (not index.is_file() or index.is_symlink()):
            raise SpecValidationError('Stored proposal reference was not found.')
        return index

    def _proposal_cursor_key(self, index: Path, *, create: bool) -> bytes:
        key_path = (index.parent / 'cursor.key').resolve()
        try:
            key_path.relative_to(self.workspace_root)
        except ValueError as exc:
            raise SpecValidationError('Proposal cursor key escaped the configured workspace.') from exc
        if key_path.exists():
            if not key_path.is_file() or key_path.is_symlink():
                raise SpecValidationError('Proposal cursor key is unsafe.')
            value = key_path.read_bytes()
            if len(value) != 32:
                raise SpecValidationError('Proposal cursor key has an invalid size.')
            return value
        if not create:
            raise SpecValidationError('Proposal cursor key is missing.')
        value = os.urandom(32)
        try:
    ...
    def _complete_proposal_counts(proposal: CompleteProposal) -> dict[str, int]:
        return MMMToolService._complete_proposal_counts_impl(proposal)

    @staticmethod
    def _complete_proposal_counts_impl(proposal: CompleteProposal) -> dict[str, int]:
        if isinstance(proposal, AuthoredPlan):
