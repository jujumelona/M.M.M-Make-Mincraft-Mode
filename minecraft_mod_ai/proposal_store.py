...
def write_sharded_complete_proposal(
    ...
    *,
    ...
    if policy is not None:
        ...
        if part_size_bytes < MIN_PART_SIZE_BYTES:
            raise ValueError(
                "Configured per-file policy is too small for proposal shards."
            )
    ...
    try:
        ...
        payload: dict[str, Any] = {
            ...
            "metadata": _write_chunked_json(
                staging,
                "metadata",
                metadata,
                part_size_bytes=part_size_bytes,
            ),
            "base_proposal": _write_chunked_json(
                staging,
                "base-proposal",
                proposal.base_proposal.to_dict(),
                part_size_bytes=part_size_bytes,
            ),
            "game_design": _write_chunked_json(
                staging,
                "game-design",
                proposal.game_design,
                part_size_bytes=part_size_bytes,
            ),
            "modules": _write_collection_v2(
                staging,
                "modules",
                (asdict(value) for value in proposal.modules),
                shard_size,
                part_size_bytes=part_size_bytes,
            ),
            "assets": _write_collection_v2(
                staging,
                "assets",
                (asdict(value) for value in proposal.assets),
                shard_size,
                part_size_bytes=part_size_bytes,
            ),
            "audio": _write_collection_v2(
                staging,
                "audio",
                (asdict(value) for value in proposal.audio),
                shard_size,
                part_size_bytes=part_size_bytes,
            ),
            "acceptance_tests": _write_collection_v2(
                staging,
                "acceptance-tests",
...
def load_sharded_complete_proposal(index_path: str | Path) -> CompleteProposal:
    ...
    return complete_proposal_from_index(
        raw,
        lambda relative: _read_file_part(index.parent, relative),
    )


def read_sharded_complete_proposal_section(
    index_path: str | Path,
    section: str,
    *,
    cursor: str = "",
    limit: int = 100,
    max_bytes: int = DEFAULT_PAGE_SIZE_BYTES,
    ...
    if selected == "overview":
        ...
        if not isinstance(metadata, dict):
            raise SpecValidationError(
                "Complete proposal metadata part must be an object."
            )
    ...
    if selected in {"modules", "assets", "audio", "acceptance_tests"}:
...
def complete_proposal_from_index(
    raw: dict[str, Any],
    ...
    *,
    ...
    if set(value) == {"count", "shards"}:
        ...
        (
            ...
            manifest_count,
            manifest_prefix,
        ) = _validate_v2_collection(value, read_part)
...
def _validate_v2_collection(
    ...
    if (
        ...
        or shard_count < 0
        or type(manifest_count) is not int
        or manifest_count < 0
        or manifest_count > shard_count
        or shard_count > count
        or ((count == 0) != (shard_count == 0))
        or ((shard_count == 0) != (manifest_count == 0))
    ):
        raise SpecValidationError(
            "Collection shard and manifest counts are inconsistent."
        )
...
def _decode_collection_manifest(
    path: str,
    data: bytes,
) -> dict[str, Any]:
    try:
        ...
        raise SpecValidationError(
            f"Collection manifest is invalid JSON: {path}"
        ) from exc
    if (
        not isinstance(payload, dict)
        or set(payload) != {"shards"}
...
def _encode_page_cursor(
    *,
    ...
    return (
        f"p_{offset}_{manifest_index}_{shard_index}_{item_index}_"
        f"{fragment_offset}_{checksum}"
    )
...
def _bounded_page_result(
    ...
    *,
    ...
    if len(encoded) > max_bytes:
        raise SpecValidationError(
            "Complete proposal page exceeded its transport byte budget."
        )
    return result


def _write_collection_v2(
    root: Path,
    prefix: str,
    ...
    *,
    ...
    def flush_manifest() -> None:
        ...
        if len(rendered) > part_size_bytes:
            raise SpecValidationError(
                "Proposal collection manifest exceeded the configured part size."
            )
        _write_bytes_durable(
            root / f"{prefix}-manifest-{manifest_count:08d}.json",
            rendered,
        )
    ...
    def flush_page() -> None:
        ...
        if not page:
            return
        descriptor = _write_chunked_json(
            root,
            f"{prefix}-shard-{shard_count:08d}",
            {"items": page},
            part_size_bytes=part_size_bytes,
        )
        candidate_manifest = canonical_json(
            {"shards": [*manifest_page, descriptor]}
        ).encode("utf-8")
    ...
    for index in range(manifest_count):
        ...
        for descriptor in payload["shards"]:
            page_wrapper = _read_json_part(
                _qualify_descriptor(descriptor, manifest_parent),
                read_part,
            )
            if (
                not isinstance(page_wrapper, dict)
                or set(page_wrapper) != {"items"}
    ...
    *,
    ...
    def flush() -> None:
        ...
        if not pending:
            return
        _write_bytes_durable(
            root / f"{prefix}.chunk-{chunk_count:08d}.jsonpart",
            bytes(pending),
        )
        pending.clear()
        chunk_count += 1
    ...
    return {
        "encoding": CHUNKED_JSON_ENCODING,
        "path_prefix": prefix,
...
def _write_part(root: Path, name: str, value: Any) -> dict[str, str]:
    ...
    return {
        "path": name,
        "sha256": "sha256:" + hashlib.sha256(rendered).hexdigest(),
    }
...
def _read_json_part(
    ...
    try:
        ...
        raise SpecValidationError(
            f"Proposal chunked JSON is invalid: {prefix.as_posix()}"
        ) from exc
