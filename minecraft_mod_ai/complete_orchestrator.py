...
from .complete_orchestrator_support import CompleteProductionError, _external_gates, _handled_module_ids, _jar_path, _locate_existing_fabric_root, _module_dict, _normalize_modules, _system_groups
...
class CompleteProductionOrchestrator:
    ...
    def execute(self, proposal: CompleteProposal | dict[str, Any], *, approval_hash: str, run_name: str, options: CompleteExecutionOptions | None=None, existing_input: str | Path | None=None) -> CompletePipelineResult:
        ...
        unresolved: list[str] = []
        project_root = self._prepare_project(approved, run_root=run_root, existing_input=existing_input)
        ...
        extended = generate_extended_content(project_root=project_root, mod_id=spec.mod_id, package_name=spec.package_name, modules=ordered, policy=self.policy)
        if extended.get('status') != 'SKIPPED':
            module_receipts.append(extended)
        for pack_id, pack_modules in _system_groups(ordered).items():
            module_receipts.append(generate_system_pack(project_root=project_root, pack_id=pack_id, mod_id=spec.mod_id, package_name=spec.package_name, config={'modules': [_module_dict(item) for item in pack_modules]}))
        ...
        if approved.world_ir is not None:
            ...
            world_receipt = compile_scalable_world_ir(approved.world_ir, mod_id=spec.mod_id, output_root=world_output, package_world_zip=True, policy=self.policy)
        ...
        if approved.audio:
            audio_receipt = generate_audio_assets(project_root=project_root, mod_id=spec.mod_id, package_name=spec.package_name, requests=approved.audio)
        ...
        for module in custom_modules:
            router = router or self.router_factory()
            module_receipts.append(CustomModuleGenerator(router, policy=self.policy).generate(project_root, module=module, minecraft_version=spec.platform.minecraft_version, loader=spec.platform.loader, mappings=spec.platform.yarn_mappings))
        index = ProjectIndex(project_root, policy=self.policy)
        heap_receipt = tune_gradle_resources(project_root, module_count=len(ordered), source_file_count=len(index.files), policy=self._policy_with_heap_override(options.gradle_heap_mb))
        ...
        source_report = ScalableProjectValidator(policy=self.policy).validate(project_root, spec).to_dict()
        ...
        if options.source_only:
            release = self._package_source_only(run_root, project_root, approved)
            unresolved.extend(_external_gates(approved, options))
            return CompletePipelineResult(schema_version='mmm/complete-pipeline-result-v2', status='SOURCE_READY', project_root=str(project_root), release_zip=release, jar_path=None, complete_proposal_hash=approved.calculate_hash(), source_validation=source_report, build_report=None, jar_validation=None, module_receipts=tuple(module_receipts), world_receipt=world_receipt, asset_receipt=asset_receipt, audio_receipt=audio_receipt, blockbench_receipts=tuple(blockbench_receipts), runtime_receipt=None, playtest_receipt=None, visual_receipt=None, distribution_receipt=None, unresolved_gates=tuple(sorted(set(unresolved))), release_ready=False)
        ...
        if build.get('status') != 'PASS' and options.auto_repair:
            router = router or self.router_factory()
            repair = RepairEngine(router=router, gradle_cache=cache, policy=self.policy).repair(project_root, run_gametest=options.run_gametest, max_attempts=options.max_repair_attempts)
        ...
        try:
            if options.run_runtime:
                ...
                memory = options.server_memory_mb or self.policy.runtime_heap_mb(module_count=len(ordered), entity_count=len(entity_modules), structure_count=len(approved.world_ir.get('structures', [])) if approved.world_ir else 0)
        ...
        release_result = tool_service.package_release(str(project_root), base.to_dict(), base.calculate_hash(), output_zip='releases/complete-release.zip', jar_path=str(jar_path))
        ...
        return CompletePipelineResult(schema_version='mmm/complete-pipeline-result-v2', status='VERIFIED' if release_ready else 'BUILT_WITH_UNRESOLVED_GATES', project_root=str(project_root), release_zip=release_zip, jar_path=str(jar_path), complete_proposal_hash=approved.calculate_hash(), source_validation=source_report, build_report=build, jar_validation=jar_validation, module_receipts=tuple(module_receipts), world_receipt=world_receipt, asset_receipt=asset_receipt, audio_receipt=audio_receipt, blockbench_receipts=tuple(blockbench_receipts), runtime_receipt=runtime_receipt, playtest_receipt=playtest_receipt, visual_receipt=visual_receipt, distribution_receipt=distribution_receipt, unresolved_gates=tuple(sorted(set(unresolved))), release_ready=release_ready)

    def _prepare_project(self, approved: CompleteProposal, *, run_root: Path, existing_input: str | Path | None) -> Path:
        if existing_input is not None:
            report = inspect_existing_project_archive(existing_input, extract_root=run_root / 'existing-source')
        ...
        FabricProjectGenerator(policy=self.policy).generate(base.spec, project_root)
    ...
    def _write_complete_approval(project_root: Path, proposal: CompleteProposal) -> None:
        rendered = json.dumps(proposal.to_dict(), ensure_ascii=False, indent=2, sort_keys=True) + '\n'
