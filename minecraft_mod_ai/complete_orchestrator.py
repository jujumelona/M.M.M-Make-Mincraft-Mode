...
class CompleteProductionOrchestrator:
    ...
    def execute(self, proposal: CompleteProposal | dict[str, Any], *, approval_hash: str, run_name: str, options: CompleteExecutionOptions | None=None, existing_input: str | Path | None=None) -> CompletePipelineResult:
        ...
        project_root = run_named_checkpoint(
            ledger,
            "prepare-project",
            stage="prepare",
            input_value={
            ...
            action=lambda: self._prepare_project(
                approved,
                run_root=run_root,
                existing_input=existing_input,
            ),
            encode=lambda value: {"project_root": str(value)},
            decode=lambda receipt: Path(str(receipt["project_root"])).resolve(),
            validate_cached=self._valid_project_root,
        )
        ...
        generation = self._execute_generation_work(
            approved=approved,
            ordered=ordered,
            work_plan=work_plan,
            ledger=ledger,
        ...
        heap_receipt = run_named_checkpoint(
            ledger,
            "tune-resources",
            ...
            input_value={
                ...
                "gradle_heap_mb": options.gradle_heap_mb,
            },
            action=lambda: tune_gradle_resources(
                project_root,
                module_count=len(ordered),
                source_file_count=len(index.files),
                policy=self._policy_with_heap_override(options.gradle_heap_mb),
            ),
        ...
        source_report = run_named_checkpoint(
            ledger,
            "validate-source",
            ...
            action=lambda: ScalableProjectValidator(
                policy=self.policy
        ...
        self._succeed_work_node(
            ...
            "validate-source",
            {
                ...
                "project_manifest": generated_manifest_hash,
            },
        )
        self._persist_work_evidence(
            project_root,
            ledger,
            work_plan,
        )
        if options.run_jdt and (not options.source_only):
            def run_jdt() -> dict[str, Any]:
                try:
                    return JavaLanguageService().diagnostics(
                        project_root,
                        timeout_seconds=90,
                    )
            ...
            jdt_receipt = run_named_checkpoint(
                ledger,
                "validate-jdt",
                stage="validate:jdt",
                input_value={
        ...
        if options.source_only:
            release = run_named_checkpoint(
                ...
                action=lambda: self._package_source_only(
                    run_root,
                    project_root,
                    approved,
                ),
                encode=lambda value: {"release_zip": value},
                decode=lambda cached: str(cached["release_zip"]),
        ...
        build_bundle = run_named_checkpoint(
            ledger,
            "gradle-build",
            stage="build",
            input_value={
        ...
        if isinstance(repair, dict):
            module_receipts.append(
                {'schema_version': 'mmm/repair-receipt-v2', **repair}
            )
        ...
        jar_validation = run_named_checkpoint(
            ...
            encode=lambda value: value,
            decode=lambda cached: cached,
            validate_cached=lambda _cached: jar_path.is_file(),
        )
        ...
        self._succeed_work_node(
            ...
            "validate-jar",
            {
                ...
                "status": "PASS",
                "jar_sha256": self._file_hash(jar_path),
                "checks_run": jar_validation.get("checks_run", 0),
            },
        )
        ...
        runtime_verified = (
            not approved.external_runtime_required
            or (
                runtime_receipt is not None
                and playtest_receipt is not None
                and visual_receipt is not None
            )
        )
        if runtime_verified:
            self._succeed_work_node(
                ...
                "runtime-playtest",
                {
                    ...
                    "status": (
                        "NOT_REQUIRED"
                        if not approved.external_runtime_required
                        else "PASS"
                    ),
            ...
            ledger.fail(
                "runtime-playtest",
                "Runtime, interaction, and visual evidence are still required.",
                input_required=True,
            )
        self._persist_work_evidence(
            project_root,
        ...
        *,
        ...
        def module_node_action(
            ...
            if stage == "content":
                ...
                raise CompleteProductionError(
                    f"Unsupported generation work stage: {stage}"
                )
            return {
        ...
        for node in work_plan.nodes:
            ...
            if kind == "module-shard":
                ...
                shard_proposal = replace(
                    approved,
                    assets=tuple(asset_lookup[item] for item in ids),
                    approval_hash="",
                )
                asset_shards.append(
                    self._run_work_node(
                ...
                audio_shards.append(
                    self._run_work_node(
                        ledger,
                        node,
                        action=lambda ids=ids: generate_audio_assets(
                            project_root=project_root,
                            mod_id=spec.mod_id,
                            package_name=spec.package_name,
                            requests=tuple(
        ...
        asset_receipt = (
            {
                ...
                "asset_count": sum(
                    len(item.get("assets", [])) for item in asset_shards
                ),
                "shards": asset_shards,
            }
            if asset_shards
        ...
        audio_receipt = (
            {
                ...
                "status": "GENERATED",
                "shard_count": len(audio_shards),
                "sound_count": sum(
                    len(item.get("sounds", [])) for item in audio_shards
                ),
                "shards": audio_shards,
            }
            if audio_shards
    ...
    def _prepare_project(self, approved: CompleteProposal, *, run_root: Path, existing_input: str | Path | None) -> Path:
        if existing_input is not None:
            ...
            project_root = self._locate_imported_project(
                report,
                run_root=run_root,
            )
        ...
        *,
        ...
        try:
            ...
            nested_members = sorted(
                ...
                & {
                    path.split("!/", 1)[0]
                    for path in report.gradle_files
                    if "!/" in path
                }
            )
            if len(nested_members) != 1:
                raise outer_error
            nested_archive = (
                extracted / Path(*nested_members[0].split("/"))
            ).resolve()
            try:
                nested_archive.relative_to(extracted)
            except ValueError as exc:
                raise CompleteProductionError(
                    "Nested source archive escaped the validated import."
                ) from exc
            if not nested_archive.is_file() or nested_archive.is_symlink():
                raise CompleteProductionError(
                    "Nested source archive is missing after validated extraction."
                )
            try:
                nested_report = inspect_existing_project_archive(
                    nested_archive,
                    extract_root=run_root / "existing-source-nested",
                )
            except ExistingProjectImportError as exc:
                raise CompleteProductionError(
                    f"Nested source archive failed validation: {exc}"
                ) from exc
            if (
                not nested_report.has_sources
                or not nested_report.has_gradle_project
                or not nested_report.extracted_to
            ):
                raise CompleteProductionError(
                    "Nested release source is not an editable Gradle project."
                )
            return _locate_existing_fabric_root(
                Path(nested_report.extracted_to).resolve()
            )
    ...
    def _project_matches_spec(project_root: Path, spec: Any) -> bool:
        ...
        return (
            has_build
            and info.main_java.is_file()
            and not info.main_java.is_symlink()
            and info.mod_id == spec.mod_id
            and info.package_name == spec.package_name
        )
    ...
    def _write_base_proposal(project_root: Path, base: Any) -> None:
        ...
        if metadata.exists() and (
            not metadata.is_dir() or metadata.is_symlink()
        ):
            raise CompleteProductionError(
                "Generated project metadata path is not a safe directory."
            )
    ...
    def _preserve_partial_project(path: Path) -> Path:
        ...
        if resolved.parent != parent:
            raise CompleteProductionError(
                "Partial project path escaped its run-owned workspace."
            )
        revision = 1
        while True:
    ...
    def _persist_work_evidence(
        ...
        metadata = project_root / ".minecraft_ai"
        metadata.mkdir(parents=True, exist_ok=True)
        (metadata / "work-graph.json").write_text(
            json.dumps(
                plan.to_dict(include_payloads=False),
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            )
    ...
    def _write_complete_approval(
        ...
        for path in (
            ...
            write_sharded_complete_proposal(
                proposal,
                path,
                shard_size=max(1, self.policy.java_shard_size),
                policy=self.policy,
            )
    ...
    def _open_run(
        ...
        *,
        resume: bool,
    ) -> tuple[Path, DurableWorkLedger, bool]:
                ...
                if (
                    ...
                    try:
                        ...
                        if "different approved proposal" not in str(exc):
                            raise CompleteProductionError(
                                f"Existing run ledger is invalid: {exc}"
                            ) from exc
                    else:
            ...
            if isinstance(value, dict):
                ...
                for key, nested in value.items():
                    if key in {"files", "generated_files"} and isinstance(nested, list):
                        raw_paths.extend(
                            str(item) for item in nested if isinstance(item, str)
                        )
    ...
    def _required_gate_failures(
        ...
        *,
        ...
        gradle_passed = (
            ...
            and CompleteProductionOrchestrator._command_receipt_passed(
                build_report,
                "clean_build",
            )
        )
        evidence = {
            ...
            "gradle": gradle_passed,
            "gametest": (
                gradle_passed
                and CompleteProductionOrchestrator._gametest_receipt_passed(
                    build_report,
                    proposal.base_proposal.spec,
                )
            ),
            "jar": (
                isinstance(jar_validation, dict)
                and jar_validation.get("status") == "PASS"
            ),
            "runtime_client": (
                isinstance(runtime_receipt, dict)
                and isinstance(runtime_receipt.get("server"), dict)
                and runtime_receipt["server"].get("server_running") is True
                and isinstance(runtime_receipt.get("client"), dict)
                and runtime_receipt["client"].get("client_running") is True
            ),
            "playtest": (
        ...
        evidence["runtime_visual"] = (
            evidence["runtime_client"] and evidence["visual"]
        )
        evidence["playtest_visual"] = (
            evidence["playtest"] and evidence["visual"]
        )
        ...
        def blockbench_passed(owner: str) -> bool:
            ...
            passed = {
                ...
                if (
                    isinstance(receipt.get("uv"), dict)
                    and receipt["uv"].get("status") in {"PASS", "OK"}
                    and isinstance(receipt.get("preview"), str)
                    and Path(receipt["preview"]).is_file()
                    and not Path(receipt["preview"]).is_symlink()
                )
            }
        ...
        for owner, gate in sorted(requirements):
            ...
            if evidence_key is None:
                failures.append(
                    f"required-gate:{owner}:{gate}:unsupported"
                )
                continue
            passed = (
                blockbench_passed(owner)
                if evidence_key == "blockbench"
                else bool(evidence.get(evidence_key, False))
            )
            if not passed:
                failures.append(
                    f"required-gate:{owner}:{gate}:missing-{evidence_key}"
                )
        return failures

    @staticmethod
    def _command_receipt_passed(
        build_report: dict[str, Any] | None,
        name: str,
    ) -> bool:
    ...
    @staticmethod
    def _gametest_receipt_passed(
        build_report: dict[str, Any] | None,
        spec: Any,
    ) -> bool:
        if (
            not CompleteProductionOrchestrator._command_receipt_passed(
                build_report,
                "gametest",
            )
            or not isinstance(build_report, dict)
            or not isinstance(build_report.get("gametest_report"), str)
        ):
            return False
        ...
        if any(
            ...
            return False
        main_class = (
            "".join(part.capitalize() for part in spec.mod_id.split("_"))
            + "Mod"
        )
