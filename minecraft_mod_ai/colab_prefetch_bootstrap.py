...
_WORK_GRAPH_FAST_SERIALIZE: ContextVar[bool] = ContextVar('mmm_work_graph_fast_serialize', default=False)
_PROPOSAL_HASH_CACHE: ContextVar[dict[int, tuple[Any, str]] | None] = ContextVar('mmm_proposal_store_hash_cache', default=None)
_PROPOSAL_CONSTRUCTION_POLICIES: ContextVar[set[tuple[int, int, int]] | None] = ContextVar('mmm_proposal_construction_policies', default=None)
_PROPOSAL_PROOF_LOCK = threading.RLock()
_PROPOSAL_VALIDATION_PROOFS: dict[int, tuple[weakref.ReferenceType[Any], dict[tuple[int, int, int], str]]] = {}
...
def _start_platform_future() -> Future[Any]:
    ...
    with _PLATFORM_LOCK:
        ...
        threading.Thread(target=worker, daemon=True, name='mmm_platform_prefetch').start()
        return future
...
def _start_platform_catalog_prefetch(live: Any) -> None:
    ...
    def worker() -> None:
        try:
            ...
            return
    threading.Thread(target=worker, daemon=True, name='mmm_platform_catalog_prefetch').start()
...
def _proposal_policy_key(policy: Any) -> tuple[int, int, int]:
    ...
    return (int(policy.max_single_file_bytes), int(policy.max_texture_dimension))

def _remember_proposal_proof(proposal: Any, policy_key: tuple[int, int, int], digest: str) -> None:
    ...
    with _PROPOSAL_PROOF_LOCK:
        ...
        _PROPOSAL_VALIDATION_PROOFS[object_id] = (reference, {policy_key: digest})

def _proposal_proof(proposal: Any, policy_key: tuple[int, int, int]) -> str:
    with _PROPOSAL_PROOF_LOCK:
...
def _retarget_loaded_proposal_aliases(current: Any, replacement: Any) -> None:
    for module_name, loaded in tuple(sys.modules.items()):
        if not (module_name == 'minecraft_mod_ai' or module_name.startswith('minecraft_mod_ai.')):
            continue
        if loaded is None:
            continue
        try:
...
def _install_work_graph_fastpath() -> None:
    ...
    if not getattr(current_to_dict, '_mmm_compile_shallow_dict', False):
        ...
        def to_dict(self: Any) -> dict[str, Any]:
            ...
            return {'node_id': self.node_id, 'stage': self.stage, 'input_hash': self.input_hash, 'dependencies': self.dependencies, 'payload': self.payload, 'resource_class': self.resource_class}
    ...
    if not getattr(current_topological, '_mmm_heap_deterministic_no_child_sort', False):
        ...
        def topological_modules(modules: Any):
            ...
            if len(ordered) != len(modules):
                raise work_graph.WorkGraphError('Production module graph contains a cycle.')
