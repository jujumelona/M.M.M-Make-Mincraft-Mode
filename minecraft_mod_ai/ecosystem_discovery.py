...
_MODRINTH_ID = re.compile(r"^[A-Za-z0-9_-]{3,64}$")
_SHA1_HEX = re.compile(r"^[0-9a-f]{40}$")
_SHA512_HEX = re.compile(r"^[0-9a-f]{128}$")
_GITHUB_REPOSITORY = re.compile(
    r"^[A-Za-z0-9_.-]{1,100}/[A-Za-z0-9_.-]{1,100}$"
)
_GIT_COMMIT = re.compile(r"^[0-9a-f]{40,64}$")
_CURSOR = re.compile(r"^(offset|page):(\d+):([0-9a-f]{16})$")
_TOKEN_CURSOR = re.compile(
    r"^token:([A-Za-z0-9_-]{1,2048}):([0-9a-f]{16})$"
)
_SEED_ROUTE_CURSOR = re.compile(r"^routes:(\d+):([0-9a-f]{16})$")
_HF_REVISION = re.compile(r"^[0-9a-f]{40,64}$")
_HF_MODEL_COMPONENT = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
...
_REVIEWABLE_MODEL_LICENSES = frozenset(
    {
        "apache-2.0",
        "bsd-2-clause",
        "bsd-3-clause",
...
_RESTRICTED_MODEL_LICENSES = frozenset(
    {
...
_UNSAFE_SERIALIZATION_SUFFIXES = frozenset(
...
class EcosystemDiscoveryClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.openverse_token = openverse_token or os.environ.get(
            "MMM_OPENVERSE_TOKEN",
            "",
        )
    ...
    def search(
        ...
        *,
        ...
        if provider not in _PROVIDERS:
            raise SpecValidationError(
                f"Unsupported ecosystem discovery provider: {provider!r}"
            )
        ...
        if provider in {
            ...
            position_kind = (
                "offset" if provider in {"modrinth", "wikipedia"} else "page"
            )
    ...
    def inspect_huggingface_model(self, repo_id: str) -> dict[str, Any]:
        ...
        encoded_repo_id = "/".join(
            quote(part, safe="._-") for part in normalized_repo_id.split("/")
        )
        ...
        if normalized_current is None:
            raise EcosystemDiscoveryUnavailable(
                "Hugging Face returned model metadata without an immutable revision."
            )
        if normalized_current["model_id"].lower() != normalized_repo_id.lower():
            raise EcosystemDiscoveryUnavailable(
                "Hugging Face returned metadata for a different model repository."
            )
        revision_sha = normalized_current["revision_sha"]
        pinned_url = model_url + "/revision/" + quote(revision_sha, safe="")
        pinned = self._get_json(
            pinned_url,
            params={"full": "true", "cardData": "true", "blobs": "true"},
            provider="huggingface",
        )
        normalized = _normalize_hf_model(pinned)
        if (
            normalized is None
            or normalized["model_id"].lower() != normalized_repo_id.lower()
            or normalized["revision_sha"] != revision_sha
        ):
            raise EcosystemDiscoveryUnavailable(
        ...
        payload = {
            ...
            "gates": {
                "access": {
                    "status": "blocked" if access_blocked else "metadata_only",
                    "reason": (
                        "private, gated or disabled model metadata"
                        if access_blocked
                        else "public metadata does not authorize an artifact download"
                    ),
                },
                "model_license": {
            ...
            "code_executed": False,
            "required_next_gates": [
                "Resolve access and the exact model-artifact license terms.",
                "Verify dataset provenance, voice or likeness consent when applicable.",
                "Verify licenses for runtime and integration code separately.",
                "Benchmark the pinned revision in the intended local, sidecar or remote topology.",
                "Approve an artifact and verify its immutable digest before any download.",
            ],
        }
        payload["inspection_sha256"] = _sha256_text(canonical_json(payload))
        return payload
    ...
    def _search_github(
        ...
        *,
        ...
        for item in raw["items"]:
            ...
            license_id = (
                str(license_value.get("spdx_id", "")).strip()
                if isinstance(license_value, dict)
                else ""
            )
            if not license_id or license_id == "NOASSERTION":
            ...
            candidates.append(
                EcosystemCandidate(
                    ...
                    attribution="",
                    preview_urls=(),
                    reuse_status="candidate_only_not_cloned",
                    evidence_sha256=_sha256_text(canonical_json(stable)),
                )
            )
    ...
    def _search_huggingface_models(
        ...
        *,
        cursor: str,
        limit: int,
    ) -> tuple[list[EcosystemCandidate], None, str | None]:
        params = {
            "search": query,
            "limit": str(limit),
            "full": "true",
        ...
        for item in raw:
            ...
            encoded_model_id = "/".join(
                quote(part, safe="._-") for part in model_id.split("/")
            )
            metadata = {
                **normalized,
                "card": card,
            ...
            candidates.append(
                EcosystemCandidate(
                    candidate_id=f"huggingface:{model_id.lower()}",
    ...
    def _search_wikipedia(
        ...
        *,
        ...
        for item in search:
            ...
            candidates.append(
                EcosystemCandidate(
                    candidate_id=f"wikipedia:{language}:{page_id}",
                    provider="wikipedia",
                    resource_kind="gameplay_or_domain_reference",
                    title=title,
                    summary=snippet,
                    source_url=source_url,
                    api_url=url,
                    ...
                    attribution="Wikipedia contributors; exact revision required if quoted",
                    preview_urls=(),
                    reuse_status="reference_only_not_code_or_asset",
                    evidence_sha256=_sha256_text(canonical_json(stable)),
                )
            )
            ...
            pages.append(
                {
                    **page,
                    ...
                    "route_query_sha256": _sha256_text(route["query"]),
                }
            )
            ...
            errors.append(
                {
                    ...
                    "provider": provider,
                    "query_sha256": _sha256_text(route["query"]),
...
def _normalize_hf_model(value: Any) -> dict[str, Any] | None:
    ...
    return {
        "model_id": model_id,
        "revision_sha": revision_sha,
        "author": _bounded_metadata_text(value.get("author")),
        "pipeline_tag": _bounded_metadata_text(value.get("pipeline_tag")),
        "library_name": _bounded_metadata_text(value.get("library_name")),
        "last_modified": _bounded_metadata_text(value.get("lastModified")),
        "created_at": _bounded_metadata_text(value.get("createdAt")),
        "downloads": _nonnegative_int(value.get("downloads")),
        "likes": _nonnegative_int(value.get("likes")),
        ...
        "tags": tags,
        "card": card,
        "files": _normalize_hf_files(value.get("siblings")),
    }
...
def _normalize_hf_card(value: Any, *, tags: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "base_models": sorted(_normalized_string_list(card.get("base_model"))),
        "datasets": sorted(_normalized_string_list(card.get("datasets"))),
        "languages": sorted(_normalized_string_list(card.get("language"))),
        "pipeline_tag": _bounded_metadata_text(card.get("pipeline_tag")),
...
def _hf_format_inventory(files: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_gguf": any(path.lower().endswith(".gguf") for path in safe_paths),
        "has_onnx": any(path.lower().endswith(".onnx") for path in safe_paths),
        "safe_data_format_files": safe_paths,
        "unsafe_serialization_files": unsafe_paths,
        "repository_code_files": code_paths,
        "safe_format_is_not_execution_authorization": True,
    }
...
def _normalize_modrinth_version(
    ...
    *,
    ...
    for item in version.get("files", []):
        ...
        safe_filename = bool(
            filename
            and len(filename.encode("utf-8")) <= 512
            and "/" not in filename
            and "\\" not in filename
        ...
        files.append(
            {
                ...
                "sha1": sha1 if _SHA1_HEX.fullmatch(sha1) else "",
                "sha512": sha512 if _SHA512_HEX.fullmatch(sha512) else "",
                "safe_filename": safe_filename,
                "safe_origin": safe_origin,
                "strong_digest_valid": bool(_SHA512_HEX.fullmatch(sha512)),
            }
        )
    ...
    if len(primary_files) != 1:
        ...
        primary_files[0]["safe_filename"]
        and primary_files[0]["safe_origin"]
        and primary_files[0]["size"] > 0
    ...
    return {
        ...
        "status": status,
        "date_published": _bounded_metadata_text(version.get("date_published")),
...
def _media_license_policy(license_name: str) -> str:
    return {
        "cc0": "origin_verification_required; attribution_optional",
        "pdm": "origin_public_domain_verification_required",
        "by": "origin_verification_and_attribution_required",
        "by-sa": "origin_verification_attribution_and_share_alike_required",
    }[license_name]
