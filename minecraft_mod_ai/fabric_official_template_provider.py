...
def _install_host_gametest_contract(root: Path, spec: Any) -> dict[str, str]:
    """Install the host-owned GameTest run/evidence contract into an official scaffold."""

    build = root / "build.gradle"
    if not build.is_file() or build.is_symlink():
        raise FabricTemplateProviderError(
            "Fabric official template omitted the Groovy build.gradle required by the "
            "host GameTest contract."
        )
    ...
    if "configureTests" not in build_text:
        additions.append(
            """// M.M.M host GameTest configuration
fabricApi {
    configureTests {
...
"""
    ...
    if gametest_entrypoint not in gametest:
        gametest.append(gametest_entrypoint)
    ...
    source = f"""package {spec.package_name};
...
public final class {gametest_class} {{
    @GameTest
    public void generatedRegistriesAreLive(GameTestHelper helper) {{
