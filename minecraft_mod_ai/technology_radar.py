...
def build_technology_radar(prompt: str, research_brief: Mapping[str, Any] | None = None, *, page_size: int = 50, cursor: str = '', target: TechnologyTarget | Mapping[str, Any] | PlatformLock | None = None) -> dict[str, Any]:
    ...
    source_payload = {'prompt_sha256': _sha256(prompt), 'research_brief_sha256': _research_brief_sha(research_brief), 'target': normalized_target.to_dict(), 'target_evidence_policy': {'coordinates_are_declared_constraints': True, 'official_exact_version_receipt_required': True, 'current_documentation_requires_target_translation': True, 'receipt_schema': 'mmm/official-target-evidence-v1', 'authenticated_code_owned_mac_required': True, 'executed_tests_bind_candidate_snapshot': True}, 'requirement_ids': [item.requirement_id for item in requirements]}
    source_sha256 = _sha256(canonical_json(source_payload))
    offset = _decode_cursor(cursor, source_sha256=source_sha256, page_size=page_size)
    ...
    next_cursor = _encode_cursor(next_offset, source_sha256=source_sha256, page_size=page_size) if next_offset < len(requirements) else ''
    flags = _request_flags(prompt, domains)
    payload: dict[str, Any] = {'schema_version': 'mmm/technology-radar-page-v1', 'source_sha256': source_sha256, 'target': normalized_target.to_dict(), 'target_evidence_policy': {'coordinates_are_declared_constraints': True, 'official_exact_version_receipt_required': True, 'current_documentation_requires_target_translation': True, 'receipt_schema': 'mmm/official-target-evidence-v1', 'authenticated_code_owned_mac_required': True, 'executed_tests_bind_candidate_snapshot': True}, 'classification': {'ai_requested': flags['ai'], 'agent_tools_requested': flags['agent'], 'translation_requested': flags['translation'], 'offline_required': flags['offline'], 'real_time_requested': flags['real_time']}, 'requirements': [item.to_dict() for item in page], 'pagination': {'offset': offset, 'page_size': page_size, 'returned': len(page), 'total_requirements': len(requirements), 'next_cursor': next_cursor}, 'discovery_policy': {'catalog': 'request-derived queries; no embedded product or model list', 'refresh': 'rerun discovery at each planning or execution session', 'untrusted_results': True, 'download_or_execution_authorized': False, 'selection': 'Pass exact compatibility, authority, license, provenance, safe-format, privacy, benchmark, test and fallback gates first; use maintenance and recency only as later tie-breakers.'}, 'scale_policy': 'Only this response page is bounded. Continue next_cursor until empty; there is no project-wide capability or domain count cap.'}
...
def normalize_technology_target(value: TechnologyTarget | Mapping[str, Any] | PlatformLock | None) -> TechnologyTarget:
    ...
    if isinstance(value, TechnologyTarget):
        ...
        if unknown:
            raise SpecValidationError(f'Unknown technology target fields: {sorted(unknown)}')
        target = TechnologyTarget(**{key: str(item) for key, item in value.items()})
    else:
...
def normalize_technology_requirement(value: Any) -> TechniqueRequirement:
    ...
    if capability_kind not in _CAPABILITY_KINDS:
        raise SpecValidationError(f'Unsupported technology capability: {capability_kind!r}')
...
def _assess_technology_candidate_with_receipt_key(requirement: TechniqueRequirement | Mapping[str, Any], candidate: Mapping[str, Any], *, receipt_key: bytes | None = None) -> dict[str, Any]:
    ...
    _assess_official_target_evidence(requirement, normalized, add, receipt_key=receipt_key)
    mismatches = [key for key, expected_value in expected.items() if str(compatibility.get(key, '')) != expected_value]
    if mismatches:
        add('exact_minecraft_bridge', 'fail' if compatibility else 'unresolved', 'Exact target evidence is missing or mismatched for: ' + ', '.join(mismatches))
    elif compatibility.get('bridge_verified') is not True:
        add('exact_minecraft_bridge', 'unresolved', 'Target fields match, but the Fabric bridge has not been verified.')
    else:
    ...
    payload: dict[str, Any] = {'schema_version': 'mmm/technology-assessment-v1', 'requirement_id': requirement.requirement_id, 'candidate_id': normalized['candidate_id'], 'capability_kind': requirement.capability_kind, 'status': status, 'eligible': status == 'eligible', 'blocking_gates': failures, 'unresolved_gates': unresolved, 'gates': [gate.to_dict() for gate in gates], 'candidate': normalized, 'selection_policy': {'latest_is_automatically_best': False, 'recency_used_only_after_required_gates': True, 'benchmark_on_declared_target_required': True, 'auto_download_or_execution': False}}
...
def _build_signed_official_target_evidence(requirement: TechniqueRequirement | Mapping[str, Any], *, receipt_key: bytes) -> dict[str, Any]:
    ...
    for document_id, source_url, authority, facts in source_facts:
        evidence_record = {'source_url': source_url, 'facts': facts, 'retrieval_document_id': document_id, 'authority': authority}
...
def _research_domains(prompt: str, research_brief: Mapping[str, Any] | None) -> tuple[dict[str, Any], ...]:
    ...
    for index, raw in enumerate(raw_domains):
        ...
        if domain_id in seen_domain_ids:
            raise SpecValidationError(f'Duplicate normalized technology domain ID: {domain_id}')
...
def _assess_official_target_evidence(requirement: TechniqueRequirement, candidate: Mapping[str, Any], add: Any, *, receipt_key: bytes | None) -> None:
    ...
    if not evidence:
        add('official_target_evidence', 'unresolved', 'A code-retrieved exact-version official evidence receipt is required.')
        return
    if evidence.get('schema_version') != 'mmm/official-target-evidence-v1':
        add('official_target_evidence', 'fail', 'The official target evidence receipt schema is invalid.')
        return
    if evidence.get('retrieved_by') != 'mmm_authoritative_retriever':
        add('official_target_evidence', 'fail', 'Target facts must come from the code-owned authoritative retriever.')
        return
    if evidence.get('authorization') != 'read_only_evidence':
        add('official_target_evidence', 'fail', 'The target receipt does not preserve the read-only evidence boundary.')
        return
    sources = evidence.get('sources')
    if not isinstance(sources, list) or not sources:
        add('official_target_evidence', 'unresolved', 'The official target receipt has no source records.')
        return
    if receipt_key is None:
        add('official_target_evidence', 'unresolved', 'Authenticated target evidence must be verified by the code-owned service.')
        return
    if not _verify_technology_receipt(evidence, receipt_key):
        add('official_target_evidence', 'fail', 'The official target receipt is missing a valid code-owned MAC or its body was changed.')
        return
    expected = requirement.target.to_dict()
    covered: set[str] = set()
    failures: list[str] = []
    ...
    if failures:
        add('official_target_evidence', 'fail', 'Official exact-version evidence failed: ' + '; '.join(failures))
    else:
...
def _assess_licenses(requirement: TechniqueRequirement, candidate: Mapping[str, Any], add: Any) -> None:
    ...
    for kind in required:
        ...
        if identifier in _UNKNOWN_LICENSES or license_value.get('reviewed') is not True:
            ...
            unresolved.append(kind)
    if denied:
        add('licenses', 'fail', 'Reviewed license terms do not permit the intended use for: ' + ', '.join(denied))
    elif unresolved:
        add('licenses', 'unresolved', 'Reviewed code/model/data license and intended-use evidence is missing for: ' + ', '.join(unresolved))
    else:
...
def _assess_artifact_format(requirement: TechniqueRequirement, candidate: Mapping[str, Any], add: Any) -> None:
    ...
    if candidate['topology'] == 'remote_api':
        add('safe_artifact_format', 'not_applicable', 'Remote service exposes no local weight artifact; API evidence remains required.')
        return
    ...
    if unsafe:
        add('safe_artifact_format', 'fail', 'Candidate includes executable or pickle-like weight formats: ' + ', '.join(sorted(unsafe)))
    elif not safe:
        add('safe_artifact_format', 'unresolved', 'No reviewed safe model format is recorded.')
    elif unreviewed:
        add('safe_artifact_format', 'unresolved', 'Unreviewed model artifact formats remain: ' + ', '.join(sorted(unreviewed)))
    elif not _SHA256.fullmatch(candidate['artifact_sha256']):
        add('safe_artifact_format', 'unresolved', 'Safe format is present but the artifact SHA-256 is missing.')
    else:
...
def _assess_dataset_provenance(requirement: TechniqueRequirement, candidate: Mapping[str, Any], add: Any) -> None:
    ...
    if not isinstance(sources, list) or not sources or provenance.get('verified') is not True:
        add('dataset_provenance', 'unresolved', 'Dataset sources and a verified provenance review are required.')
    else:
...
def _assess_privacy(requirement: TechniqueRequirement, candidate: Mapping[str, Any], add: Any) -> None:
    ...
    if requirement.offline_required and (candidate['topology'] == 'remote_api' or runtime.get('supports_offline') is not True):
    ...
    if leaves_device is True:
        if privacy.get('explicit_transfer_consent') is not True or not str(privacy.get('retention_policy') or '').strip() or privacy.get('deletion_supported') is not True:
            add('privacy', 'fail', 'Remote raw-data transfer lacks consent, retention or deletion controls.')
            return
...
def _valid_technology_test_receipt(value: Any, *, test_id: str, requirement_id: str, candidate: Mapping[str, Any], receipt_key: bytes | None) -> bool:
    ...
    for field in ('environment_sha256', 'result_sha256'):
        if not _SHA256.fullmatch(str(value.get(field) or '').lower()):
            return False
    for field in ('candidate_id', 'revision', 'artifact_sha256', 'evidence_sha256'):
        if field not in value or str(value.get(field)) != str(candidate.get(field)):
            return False
    if value.get('candidate_snapshot_sha256') != _technology_candidate_snapshot_sha256(candidate):
        return False
...
def _verify_technology_receipt(value: Mapping[str, Any], receipt_key: bytes | None) -> bool:
    ...
    expected_mac = 'hmac-sha256:' + hmac.new(receipt_key, canonical_json(mac_body).encode('utf-8'), hashlib.sha256).hexdigest()
...
def _normalize_license(value: Any) -> dict[str, Any]:
    if isinstance(value, str):
        return {'id': value.strip(), 'url': '', 'reviewed': False, 'use_allowed': None}
