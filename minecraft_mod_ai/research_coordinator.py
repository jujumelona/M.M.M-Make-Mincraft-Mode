    ...
    *,
    ...
    while True:
        ...
        processed = _strict_non_negative_int(
            page.get("processed_route_count"),
            "Ecosystem processed_route_count",
        )
        remaining = _strict_non_negative_int(
            page.get("remaining_route_count"),
            "Ecosystem remaining_route_count",
        )
    ...
    aggregate["remaining_route_count"] = max(
        0, route_count - processed_route_count
    )
...
def _require_equal_page_invariants(
    ...
    for key in (
        ...
        if page.get(key) != first_page.get(key):
            raise SpecValidationError(f"Technology pages changed invariant field: {key}")
