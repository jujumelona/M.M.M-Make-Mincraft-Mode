...
def collect_technology_radar(prompt: str, research_brief: Mapping[str, Any] | None=None, *, page_size: int=50, target: Any=None, page_builder: TechnologyPageBuilder=build_technology_radar) -> dict[str, Any]:
    ...
    while True:
        ...
        if not isinstance(pagination, dict) or not isinstance(raw_requirements, list):
            raise SpecValidationError('Technology page requires pagination and requirements.')
        offset = _strict_non_negative_int(pagination.get('offset'), 'Technology pagination.offset')
        returned = _strict_non_negative_int(pagination.get('returned'), 'Technology pagination.returned')
        declared_page_size = _strict_non_negative_int(pagination.get('page_size'), 'Technology pagination.page_size')
        declared_total = _strict_non_negative_int(pagination.get('total_requirements'), 'Technology pagination.total_requirements')
        next_cursor = pagination.get('next_cursor')
        if not isinstance(next_cursor, str):
            raise SpecValidationError('Technology pagination.next_cursor must be a string.')
        if declared_page_size != page_size:
            raise SpecValidationError('Technology page changed the requested page size.')
        if offset != expected_offset:
            raise SpecValidationError('Technology pagination did not advance monotonically.')
        if returned != len(raw_requirements):
            raise SpecValidationError('Technology pagination.returned does not match requirements.')
        ...
        if first_page is None:
            ...
            if declared_total != total_requirements:
                raise SpecValidationError('Technology pages changed their declared total.')
        ...
        for raw_requirement in raw_requirements:
            ...
            if requirement_id in requirement_ids:
                raise SpecValidationError(f'Technology pagination repeated requirement: {requirement_id}')
            requirement_ids.add(requirement_id)
            requirements.append(deepcopy(raw_requirement))
        page_receipts.append({'offset': offset, 'returned': returned, 'radar_sha256': str(page.get('radar_sha256', ''))})
        next_offset = offset + returned
        if not next_cursor:
            if next_offset != declared_total:
                raise SpecValidationError('Technology pagination ended before its declared total.')
            break
        if next_offset <= offset:
            raise SpecValidationError('Technology pagination cursor did not advance.')
        if next_offset >= declared_total:
            raise SpecValidationError('Technology pagination returned a cursor after its declared total.')
    ...
    aggregate['pagination'] = {'offset': 0, 'page_size': page_size, 'returned': len(requirements), 'total_requirements': total_requirements, 'next_cursor': '', 'pages_collected': len(page_receipts), 'complete': True}
    aggregate['collection_receipt'] = {'schema_version': 'mmm/technology-page-collection-receipt-v1', 'page_count': len(page_receipts), 'page_size': page_size, 'pages_sha256': _sha256(page_receipts)}
    aggregate['radar_sha256'] = _sha256(aggregate)
    return aggregate

def collect_ecosystem_seed_bundle(prompt: str, game_design: dict[str, Any], *, research_brief: dict[str, Any] | None=None, client: EcosystemDiscoveryClient | None=None, route_limit: int=12, page_builder: EcosystemPageBuilder=discover_seed_bundle, planning_seed_only: bool=False) -> dict[str, Any]:
    ...
    while True:
        page = page_builder(prompt, game_design, research_brief=research_brief, client=client, route_cursor=cursor, route_limit=route_limit)
        if not isinstance(page, dict):
            raise SpecValidationError('Ecosystem route page must be an object.')
        if page.get('schema_version') not in {'mmm/ecosystem-seed-bundle-v1', 'mmm/ecosystem-seed-bundle-v2'}:
            raise SpecValidationError('Unsupported ecosystem seed page schema.')
        offset = _strict_non_negative_int(page.get('route_offset'), 'Ecosystem route_offset')
        declared_count = _strict_non_negative_int(page.get('route_count'), 'Ecosystem route_count')
        processed = _strict_non_negative_int(page.get('processed_route_count'), 'Ecosystem processed_route_count')
        remaining = _strict_non_negative_int(page.get('remaining_route_count'), 'Ecosystem remaining_route_count')
        declared_candidates = _strict_non_negative_int(page.get('candidate_count'), 'Ecosystem candidate_count')
        ...
        if not isinstance(next_cursor, str) or type(complete) is not bool:
            raise SpecValidationError('Ecosystem route page requires boolean completion and a string cursor.')
        ...
        if next_offset is not None:
            if next_offset < offset or next_offset - offset > route_limit:
                raise SpecValidationError('Ecosystem route page violated its per-page route limit.')
            if processed > next_offset - offset:
                raise SpecValidationError('Ecosystem processed routes exceed this route page.')
        elif processed:
            raise SpecValidationError('Disabled ecosystem discovery may not report processed routes.')
        if complete != (not next_cursor):
            raise SpecValidationError('Ecosystem completion disagrees with next_route_cursor.')
        if disabled_page and next_cursor and (offset >= declared_count):
            raise SpecValidationError('Disabled ecosystem route cursor did not advance.')
        if disabled_page and (not next_cursor) and (declared_count - offset > route_limit):
            raise SpecValidationError('Disabled ecosystem route pagination ended before route completion.')
        ...
        elif current_route_sha != route_sha256 or current_query_sha != query_sha256 or declared_count != route_count:
        ...
        route_receipts.append({'route_offset': offset, 'advanced_to_offset': next_offset, 'processed_route_count': processed, 'candidate_count': declared_candidates})
        if planning_seed_only:
            ...
            planning_seed['status'] = _aggregate_ecosystem_status(statuses, pages, candidate_count)
            planning_seed['candidate_count'] = candidate_count
            planning_seed['pages'] = pages
            planning_seed['errors'] = errors
            planning_seed['coverage'] = 'planning seed only; remaining route catalog and provider cursors are intentionally deferred to specialist dependency/asset research'
            planning_seed['collection_receipt'] = {'schema_version': 'mmm/ecosystem-route-collection-receipt-v1', 'route_page_count': 1, 'route_limit': route_limit, 'route_pages_sha256': _sha256(route_receipts), 'planning_seed_only': True}
            return planning_seed
        if not next_cursor:
            if not disabled_page and (remaining != 0 or next_offset != declared_count):
                raise SpecValidationError('Ecosystem route pagination ended before route completion.')
            break
    ...
    aggregate['status'] = _aggregate_ecosystem_status(statuses, pages, candidate_count)
    ...
    aggregate['collection_receipt'] = {'schema_version': 'mmm/ecosystem-route-collection-receipt-v1', 'route_page_count': len(route_receipts), 'route_limit': route_limit, 'route_pages_sha256': _sha256(route_receipts)}
    return aggregate

def _require_equal_page_invariants(first_page: Mapping[str, Any], page: Mapping[str, Any]) -> None:
    pass
...
def _aggregate_ecosystem_status(statuses: list[str], pages: list[dict[str, Any]], candidate_count: int) -> str:
    if candidate_count:
        return 'available'
    if pages:
        return 'empty'
