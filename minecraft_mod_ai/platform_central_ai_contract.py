...
"""Single planning owner for platform selection and explicit live-target lowering."""
...
def lower_live_modules(planner: Any, result: CompleteProposal) -> CompleteProposal:
    """Lower semantic production modules for a validated live Fabric target.

    This is deliberately an ordinary function call, not a runtime wrapper. The planner
    remains inspectable and the authoritative production-contract module is resolved at
    the point of recompilation.
    """
                ...
                "platform_migration": {
                    "from": (
                        ...
                        if isinstance(migration_from, dict)
                            ...
                            "minecraft_version": "existing-project",
                            "loader": "unknown",
                        }
                    ),
                    "to": dict(target),
...
__all__ = ["install", "lower_live_modules"]
