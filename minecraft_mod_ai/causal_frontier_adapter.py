...
"""Per-turn causal action-class and tool exposure for the live agent loop."""
...
from .causal_action_state import CausalAction, action_for_tool, select_action_frontier
...
_CURRENT_ACTION: ContextVar[CausalAction] = ContextVar(
    "mmm_causal_current_action", default=CausalAction.FINISH
)
...
class CausalFrontierAdapter:
    ...
    def generate_turn(self, request: Any) -> Any:
        ...
        if not request.tools and request.tool_choice is None:
            self._publish_frontier(CausalAction.FINISH, ())
        ...
        if forced_name:
            action = action_for_tool(forced_name)
            ...
            ranked_names = executable_frontier(
                frontier_candidates,
                state=state,
                goals=goals,
            ...
            action, names = select_action_frontier(ranked_names)
            ...
            if action is CausalAction.MUTATE and selected_names:
                tool_choice = {
                    "type": "function",
                ...
                parallel_tool_calls = len(selected) > 1

        selected_names = tuple(_name(schema) for schema in selected)
        self._publish_frontier(action, selected_names)
        rebuilt = replace(
            request,
        ...
        if fingerprint == self._last_stale_frontier:
            raise ModelConfigurationError(
                ...
                f"action={action.value} stale={','.join(stale_names)} "
