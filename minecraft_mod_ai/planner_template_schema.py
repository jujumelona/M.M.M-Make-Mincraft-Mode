...
TOP_LEVEL_KEYS = frozenset(
    {"modules", "assets", "acceptance_tests", "completed_deliverables", "complete", "next_cursor"}
)

# Optional non-authoritative implementation notes are allowed only for non-canonical
# callers of the merge helper. Evidence-owned production pages use the host contract.
...
def _contract_values(contracts: Mapping[str, Mapping[str, Any]], *keys: str) -> list[str]:
    values: list[str] = []
...
def build_batch_skeleton(
    ...
    """Create a complete host-owned production page.

    Evidence-owned pages contain the exact coder execution contract. No planner-model
    refinement is required for a page to be complete.
    """
    batch = _normalize_id(batch_id, "batch")
    module_ids = list(dict.fromkeys(_normalize_id(item, "module") for item in exports)) or [batch]
    known = set(known_module_ids)
    dependencies = [item for item in _unique_strings(depends_on_batches) if item in known]
    ...
    for module_id in module_ids:
        ...
        if isinstance(contract, Mapping):
            ...
            config.update(
                {
                    "evidence_plan_sha256": str(contract.get("evidence_plan_sha256") or ""),
                    "evidence_task": evidence_task,
                    "requirement_refs": _unique_strings(contract.get("requirement_refs")),
            ...
            if coder_contract is not None:
                config["coder_execution_contract"] = coder_contract
        modules.append(
            {
                "module_id": module_id,
                "kind": "custom_java",
                ...
                "required_gates": _unique_strings(contract.get("required_gates")) if isinstance(contract, Mapping) else [],
            }
        )
    ...
    if contracts:
        derived_acceptance = _contract_values(
            contracts, "public_acceptance", "runtime_acceptance", "acceptance", "internal_invariants"
        )
        ...
        page_acceptance = explicit_acceptance or [f"test_{module_id}_registers" for module_id in module_ids]
...
def _merge_modules(
    ...
    """Legacy-compatible merge for non-evidence pages; host contracts stay immutable."""
    skeleton_modules = [
        dict(item)
    ...
    for host_item in skeleton_modules:
        ...
        if evidence_owned:
            ...
            config = {**host_config, "optional_coder_notes": details}
        ...
        gates = _unique_strings(host_item.get("required_gates")) if evidence_owned else _unique_strings(item.get("required_gates"))
        merged.append(
            {
                "module_id": module_id,
                "kind": kind,
...
def _merge_assets(model_output: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for raw in raw_assets:
        ...
        if asset_id in seen_ids or target_path in seen_paths or kind not in ASSET_KINDS or not prompt or not target_path:
            continue
        assets.append(
            {
                "asset_id": asset_id,
...
def merge_model_output_into_skeleton(
    ...
    """Merge optional non-authoritative values without mutating host evidence contracts."""
    allowed_output = {key: model_output[key] for key in TOP_LEVEL_KEYS if key in model_output}
    evidence_owned = any(
        isinstance(item, Mapping) and isinstance(_mapping_config(item).get("evidence_task"), Mapping)
        for item in skeleton.get("modules", ())
    )
    return {
        ...
        "acceptance_tests": (
            ...
            if evidence_owned
            else _unique_strings(allowed_output.get("acceptance_tests")) or _unique_strings(skeleton.get("acceptance_tests"))
        ),
        "completed_deliverables": (
            _unique_strings(skeleton.get("completed_deliverables"))
            if evidence_owned
            else _unique_strings(allowed_output.get("completed_deliverables")) or _unique_strings(skeleton.get("completed_deliverables"))
        ),
        "complete": True,
        "next_cursor": "",
    }
