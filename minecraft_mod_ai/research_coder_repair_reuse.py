...
owning modules.  This module only adds bounded state, dependency-directed query hints,
and compact host receipts that let later repair reuse already-paid-for evidence.
"""
...
def _round_budget() -> int:
    ...
    # lower the budget, but cannot silently turn the production coder into an
    # unbounded retrieval agent.
...
def _dependency_neighborhood_query(
    ...
    query_tokens = research_module._tokens(query)
    ...
    for path, unit in context.units.items():
        ...
        overlap = research_module._overlap(query_tokens, tokens)
        if overlap > 0.0:
            scored.append((overlap, path))
    scored.sort(key=lambda item: (-item[0], item[1]))
    seeds = [path for _score, path in scored[:4]]
    if not seeds:
        return ""
    ...
    for seed in seeds:
        ...
        seed_fqcns.update(
            f"{unit.package}.{name}" if unit.package else name for name in unit.types
        )
        for imported in unit.imports:
            if imported.endswith(".*"):
                package = imported[:-2]
                direct.update(
                    path for path, candidate in context.units.items()
                    if candidate.package == package and path != seed
                )
                continue
            target = by_fqcn.get(imported)
            if target and target != seed:
                direct.add(target)
                continue
            leaf = imported.rsplit(".", 1)[-1]
            direct.update(path for path in by_type.get(leaf, ()) if path != seed)

    reverse: set[str] = set()
    for path, unit in context.units.items():
        if path in seeds:
            continue
        imports = set(unit.imports)
        if imports & seed_fqcns:
            reverse.add(path)
            continue
        if any(
            imported.endswith(".*") and imported[:-2] in seed_packages
    ...
    for path, unit in context.units.items():
        ...
        if not any(term in lowered for term in ("contract", "interface", "api")):
            continue
    ...
    wants_build = bool(
        query_tokens
        & {"dependency", "dependencies", "import", "api", "registry", "build", "gradle", "fabric"}
    )
    ...
    ordered = [
        ...
        *sorted(reverse),
        *sorted(contracts),
        *(sorted(build_paths) if wants_build else []),
    ]
    ordered = list(dict.fromkeys(ordered))[:14]
...
def _install_research_context_hardening(research_module: Any) -> None:
    ...
    if not getattr(current_paths, _MARKER, False):
        ...
        def query_paths(self: Any, query: str, plan_step: Any | None):
            paths = list(current_paths(self, query, plan_step))
            ...
            if dependency_query and dependency_query not in paths:
                # Keep the canonical query first, then its dependency-directed
                # neighborhood so RRF sees it before broad vocabulary expansion.
    ...
    *,
    ...
    with _RECEIPT_LOCK:
        ...
        receipts = [
            ...
            if (str(item.get("module_id", "")), str(item.get("bundle_sha256", ""))) != key
        ]
        receipts.append(
            {
                "module_id": module_id,
...
def _read_log_tail(command: Mapping[str, Any]) -> str:
    ...
    if isinstance(log_path, str):
        ...
        if path.is_file() and not path.is_symlink():
            try:
                pieces.append(path.read_text(encoding="utf-8", errors="replace")[-16_000:])
            except OSError:
                pass
    return "\n".join(pieces)
    ...
    for stored in reversed(receipts):
        ...
        for raw in reusable:
            ...
            haystack = " ".join(
                [
                    str(item.get("path", "")),
                    " ".join(str(value) for value in item.get("symbols", []) if isinstance(value, str)),
        ...
        base.update(
            {
                ...
                "repair_evidence_receipt": {
                    **receipt_payload,
                    ...
                    + __import__("hashlib").sha256(diagnostic_receipt.encode("utf-8")).hexdigest(),
