...
provenance, research queues, state transitions, evidence, coverage, and readiness.
"""
...
def _build_host_state(prompt: str, model_value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    state: dict[str, Any] = {
        ...
        "references": [_reference_item(prompt, item, index=i) for i, item in enumerate(references_raw) if isinstance(item, Mapping)],
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "If scope is not authored, mark it partial or unspecified instead of guessing."
            ),
        },
