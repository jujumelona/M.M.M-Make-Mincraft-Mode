...
"""Host-owned tool execution state machine.

The host owns mutation authority. A host-pinned target is sufficient to enter ACT;
retrieval is only a localization aid when no target is bound. Source mutation and
verification form a direct ACT -> VERIFY -> ACT(repair) cycle. RECOVER is reserved
for tool/runtime failures and never forces external retrieval.
"""
...
_LOCALIZATION_EVIDENCE_TOOLS = frozenset({
    "search_code_rag", "search_project_rag", "java_workspace_symbols", "read_reuse_source",
})
_READ_OBSERVE_TOOLS = frozenset({
    *_LOCALIZATION_EVIDENCE_TOOLS,
    "discover_ecosystem_resources", "inspect_modrinth_project", "inspect_github_repository",
    "inspect_huggingface_model", "assess_technology_compatibility",
    "read_complete_plan_section", "read_quality_contract", "quality_status", "work_status",
    "work_tasks", "external_mcp_capabilities", "external_mcp_schema", "external_mcp_call",
})
_RECOVERY_EVIDENCE_TOOLS = frozenset(_LOCALIZATION_EVIDENCE_TOOLS)
...
_SOURCE_CREATE_OPERATIONS = frozenset({
    "create", "create_file", "create_java_type", "create_class", "create_type", "write", "write_file",
})
_MODEL_REJECTION_TOOL_NAME = "__mmm_rejected_tool_call__"
_HOST_AUTHORITY_ROLES = frozenset({"system", "developer", "tool"})
_ATOMIC_OUTPUT_RECOVERY_MARKER = "MMM_ATOMIC_OUTPUT_RECOVERY_V2"
...
def evidence_fingerprint(value: Any) -> str | None:
    ...
    if stable in (None, "", [], {}):
        return None
    canonical = json.dumps(stable, ensure_ascii=False, sort_keys=True, separators=(",", ":"), default=str)
...
class TargetMutationContext:
    ...
    def localization_stage(self) -> LocalizationStage:
        path = _canonical_mutation_path(self.target_path)
        if not path:
        ...
        if self.is_new_file:
            return LocalizationStage.READY
        if isinstance(self.source_body, str) and self.source_body.strip():
            return LocalizationStage.READY
    ...
    def merge(self, other: "TargetMutationContext") -> "TargetMutationContext":
        ...
        if left and right and left != right:
            return self if self.target_pinned else other
        return TargetMutationContext(
            ...
            writable_paths=tuple(dict.fromkeys((*self.writable_paths, *other.writable_paths))),
            creatable_paths=tuple(dict.fromkeys((*self.creatable_paths, *other.creatable_paths))),
...
def _task_authority_context(payload: Mapping[str, Any]) -> TargetMutationContext | None:
    ...
    direct_writable = [
        path for path in (_canonical_mutation_path(item) for item in _sequence(payload.get("writable_paths")))
    ...
    if isinstance(task, Mapping):
        ...
        collections = [item for item in _sequence(task.get("production_bindings")) if isinstance(item, Mapping)]
        ...
        for binding in collections:
            ...
            for anchor in _sequence(binding.get("owned_anchors")):
                ...
                fresh = str(anchor.get("status") or "").strip().casefold() == "host_reserved" or reuse == "fresh"
    ...
    if direct_primary:
        chosen = next((item for item in candidates if item[0] == direct_primary), None)
        if chosen is None:
...
def _extract_search_context(payload: Mapping[str, Any]) -> TargetMutationContext | None:
    ...
    for hit in _sequence(hits):
        ...
        return TargetMutationContext(
            target_path=path,
            target_symbol=symbol or None,
            source_body=text if isinstance(text, str) and text.strip() else None,
...
def _extract_mutation_context_from_payload(payload: Any) -> TargetMutationContext | None:
    ...
    if authority is not None:
        ...
        if isinstance(initial, Mapping):
            ...
            if exact is not None and _canonical_mutation_path(exact.target_path) == _canonical_mutation_path(authority.target_path) and exact.source_body:
                return replace(
                    authority,
    ...
    for symbol in _sequence(payload.get("symbols")):
        ...
        if path:
            return TargetMutationContext(
                target_path=path,
                target_symbol=str(symbol.get("name") or "").strip() or None,
    ...
    target = _canonical_mutation_path(payload.get("target_file") or payload.get("target_path") or payload.get("path"))
    if target and _is_workspace_file_path(target):
        source = payload.get("source") or payload.get("content") or payload.get("code")
        return TargetMutationContext(
            target_path=target,
            source_body=source if isinstance(source, str) and source.strip() else None,
...
class HostRunState:
    ...
    failed_verifier_fingerprints: set[str] = field(default_factory=set)
    ...
    def record_mutation(self, tool_name: str, arguments: Mapping[str, Any], payload: Mapping[str, Any]) -> bool:
    ...
    def record_verification(self, tool_name: str, payload: Mapping[str, Any], status: str) -> bool:
        ...
        with self._lock:
            ...
            if status == "FAIL" and fp:
                if fp in self.failed_verifier_fingerprints:
                    self.semantic_fixed_point = True
                self.failed_verifier_fingerprints.add(fp)
            elif status == "PASS":
                self.failed_verifier_fingerprints.clear()
    ...
    def take_verifier_repair_guidance(self) -> str | None:
        with self._lock:
            if self.validation_status != "FAIL" or not self.latest_verifier_fingerprint or self.latest_verifier_fingerprint == self.repair_guidance_fingerprint:
                return None
            ...
            payload = {
                ...
                "current_source_sha256": hashlib.sha256(source.encode("utf-8")).hexdigest() if source is not None else None,
                "current_source": source,
            }
        return (
            "MMM_CORE_VERIFIER_REPAIR_V5\n"
            "The verifier failure is the active repair obligation. The host target remains authoritative. "
            "Do not return to retrieval or recreate an existing path. Make one materially different source "
            "edit against the pinned target; the next successful mutation goes directly to VERIFY.\n"
    ...
    def record_no_progress_result(self, value: Any) -> bool:
        canonical = json.dumps(_stable_value(value), ensure_ascii=False, sort_keys=True, separators=(",", ":"), default=str)
    ...
    def next_untried_internal_tool(self, exposed_tools: Sequence[str] | set[str] | frozenset[str], *, preferred: Sequence[str], localization_stage: LocalizationStage | None = None) -> str | None:
...
def is_mutation_ready(messages: Sequence[Mapping[str, Any]], state: HostRunState) -> bool:
    for message in messages:
        ...
        if not isinstance(payload, Mapping):
            continue
        authority_allowed = role in _HOST_AUTHORITY_ROLES or (role == "user" and _trusted_internal_user_payload(payload))
        ...
        with state._lock:
            ...
            elif _canonical_mutation_path(state.mutation_context.target_path) == _canonical_mutation_path(context.target_path):
...
def _mutation_target_error(tool_name: str, arguments: Mapping[str, Any], context: TargetMutationContext | None) -> str | None:
    if tool_name not in _MUTATION_ACT_TOOLS:
        return None
    if context is None or not context.is_mutation_ready:
        return "MUTATION_TARGET_UNBOUND: no host-bound mutation target is READY"
    supplied = ""
    for key in _SOURCE_EDIT_PATH_KEYS:
        value = arguments.get(key)
        if isinstance(value, str) and value.strip():
            ...
            break
    if not supplied and tool_name != "apply_source_edit":
        return None
    ...
    if not supplied or not pinned:
        return "MUTATION_TARGET_UNBOUND: source edit requires an explicit host-bound path"
    if supplied not in allowed:
        return f"MUTATION_TARGET_DRIFT: writable exact-set {sorted(allowed)!r} does not authorize {supplied!r}"
    operation = str(arguments.get("operation") or "").strip().casefold()
    if operation in _SOURCE_CREATE_OPERATIONS:
        can_create = supplied in set(context.creatable_paths) or (supplied == pinned and context.is_new_file)
        if not can_create:
            return f"MUTATION_TARGET_CREATION_CONFLICT: create operation is not authorized for existing target {supplied!r}"
    return None
...
def _verification_outcome(tool_name: str, payload: Mapping[str, Any]) -> str:
    ...
    if tool_name in {"java_diagnostics", "jdt_diagnostics"}:
        ...
        unavailable_codes = {"JDT_DIAGNOSTICS_UNAVAILABLE", "JDT_WORKSPACE_NOT_READY"}
...
def _fixed_point_tool_results(executed: Sequence[tuple[Any, Mapping[str, Any]]]) -> list[dict[str, Any]]:
    stable: list[dict[str, Any]] = []
    for call, payload in executed:
        if call.name in _VERIFY_TOOLS:
            stable.append({"name": call.name, "verification_outcome": _verification_outcome(call.name, payload)})
        else:
            stable.append({"name": call.name, "ok": bool(payload.get("ok")), "failure_code": payload.get("failure_code")})
    return stable
...
class RetrievalProgress:
    ...
    def begin(self, tool_name: str, arguments: Mapping[str, Any]) -> RetrievalDecision:
        return RetrievalDecision.EXECUTE if self._state.record_query(tool_name, arguments) else RetrievalDecision.DUPLICATE_QUERY
    ...
    def next_untried_internal_tool(self, exposed_tools: Sequence[str] | set[str] | frozenset[str], *, preferred: Sequence[str]) -> str | None:
        return self._state.next_untried_internal_tool(exposed_tools, preferred=preferred)


def _source_edit_schema_for_context(schema: Mapping[str, Any], context: TargetMutationContext | None) -> Mapping[str, Any]:
    ...
    if isinstance(operation, dict):
        ...
        if isinstance(enum, list):
            operation["enum"] = [value for value in enum if str(value).strip().casefold() not in _SOURCE_CREATE_OPERATIONS]
...
def _filter_tools_for_phase(
    ...
    *,
    ...
    del role, localization_active, semantic_retrieval_choice
    ...
    if phase == LoopPhase.ACT:
        names = ["apply_source_edit"] if "apply_source_edit" in by_name else [name for name in by_name if name in _MUTATION_ACT_TOOLS]
    elif phase == LoopPhase.VERIFY:
        names = [name for name in by_name if name in _VERIFY_TOOLS]
    elif phase == LoopPhase.RECOVER:
        names = []
    else:
        preferred = ("search_code_rag", "java_workspace_symbols", "search_project_rag", "read_reuse_source", "external_mcp_call")
        names = [name for name in preferred if name in by_name and name not in attempted][:1]
    return tuple(_source_edit_schema_for_context(by_name[name], mutation_context) for name in names if name in by_name)
...
def _completion_boundary_error(exc: BaseException) -> bool:
    ...
    while current is not None and id(current) not in seen:
        ...
        if name == "LlamaCompletionBoundaryError":
            return True
        if "completion boundary" in text or "completion token limit" in text or "maximum completion" in text or ("finish_reason" in text and "length" in text):
            return True
...
def _atomic_output_recovery_instruction(request: GenerationRequest) -> str:
    ...
    if names & _MUTATION_ACT_TOOLS:
        return (
            "The previous oversized assistant output was discarded. Do not continue it. "
            "The host target is already authoritative. Call exactly one exposed mutation tool with one bounded "
            "semantic edit and no prose. Do not retrieve evidence before mutating a pinned target."
        )
    return "The previous oversized assistant output was discarded. Produce exactly one concise visible tool call or one concise final answer."


def _model_tool_rejection_feedback(calls: Sequence[Any]) -> tuple[str, list[Mapping[str, Any]]] | None:
    rejections: list[Mapping[str, Any]] = []
    for call in calls:
        if str(getattr(call, "name", "") or "").strip() != _MODEL_REJECTION_TOOL_NAME:
            continue
        arguments = getattr(call, "arguments", None)
        rejections.append(dict(arguments) if isinstance(arguments, Mapping) else {
            "failure_code": "MODEL_TOOL_CALL_REJECTED", "error": "invalid rejection payload",
        })
    if not rejections:
        return None
    details = []
    for payload in rejections:
        ...
        details.append(line)
    return (
        "The previous model tool call was rejected before execution. Correct the tool name/arguments to the currently exposed schema and retry the same phase. Rejections: " + " | ".join(details),
        rejections,
    )
    ...
    *,
    ...
    try:
        ...
        already_recovered = any(
            isinstance(message.get("content"), str) and _ATOMIC_OUTPUT_RECOVERY_MARKER in str(message.get("content"))
        ...
        messages.append({
            "role": "system",
            "content": _ATOMIC_OUTPUT_RECOVERY_MARKER + "\n" + _atomic_output_recovery_instruction(turn_request),
        })
        fitted_recovery = fit_messages_to_context(messages, config=config, tools=request.tools)
        messages[:] = [dict(message) for message in fitted_recovery]
        recovery_request = replace(turn_request, messages=tuple(messages), media_paths=())
        emit_root_cause(
...
def _verifier_tool(schemas: Sequence[Mapping[str, Any]], unavailable: set[str]) -> str | None:
...
def _fixed_point_error(state: HostRunState) -> ModelConfigurationError:
    ...
    if state.validation_status == "FAIL":
        return ModelConfigurationError(
            "VERIFICATION_REPAIR_FIXED_POINT: verifier diagnostics repeated after a repair without semantic improvement.\n" + trajectory
        )
    return ModelConfigurationError(
        "AGENT_SEMANTIC_FIXED_POINT: the same action/result state repeated without workspace, localization, or verification progress.\n" + trajectory
    )
...
def _append_tool_result(messages: list[dict[str, Any]], *, config: Any, tools: Sequence[Mapping[str, Any]], call: Any, payload: Mapping[str, Any]) -> None:
...
def _assistant_tool_message(turn: Any) -> dict[str, Any]:
    return {
        "role": "assistant",
        ...
        "tool_calls": [
            {
                ...
                "function": {
                    "name": call.name,
                    "arguments": call.raw_arguments or json.dumps(dict(call.arguments), ensure_ascii=False, separators=(",", ":")),
                },
            }
...
def _runtime_call(runtime: Any, *, stage: str, call: Any, reviewed_external_servers: Sequence[str]) -> Any:
    if call.name.startswith("external_mcp_"):
        scoped = getattr(runtime, "call_scoped", None)
        if not callable(scoped):
            raise ModelConfigurationError("External MCP execution requires a role-scoped runtime.")
        return scoped(stage, call.name, call.arguments, external_server_ids=reviewed_external_servers)
    return runtime.call(stage, call.name, call.arguments)


def _generate_generic_tool_loop(
    router: Any,
    *,
    config: Any,
    adapter: Any,
    ...
    while True:
        turn = _generate_turn_with_context_recovery(
            ...
            request=request,
            messages=messages,
            media_paths=request.media_paths if first_turn else (),
        ...
        if rejection is not None:
            feedback, payloads = rejection
            digest = evidence_fingerprint(payloads) or ""
            if digest and digest in seen_no_progress:
                raise ModelConfigurationError("GENERIC_TOOL_FIXED_POINT: identical rejected tool call repeated.")
            if digest:
                seen_no_progress.add(digest)
            messages.extend([
        ...
        def execute(call: Any) -> tuple[Any, Mapping[str, Any]]:
            try:
                result = _runtime_call(runtime, stage=stage, call=call, reviewed_external_servers=reviewed_external_servers)
                return call, {"ok": True, "tool": call.name, "result": result}
            except Exception as exc:
                return call, {
                    "ok": False,
                    "tool": call.name,
        ...
        for call, payload in executed:
            _append_tool_result(messages, config=config, tools=request.tools, call=call, payload=payload)

        digest = evidence_fingerprint({
            "calls": [{"name": call.name, "arguments": dict(call.arguments)} for call in turn.tool_calls],
            "results": _fixed_point_tool_results(executed),
        }) or ""
        if digest and digest in seen_no_progress:
            raise ModelConfigurationError("GENERIC_TOOL_FIXED_POINT: identical tool action/outcome repeated without progress.")
        if digest:
            seen_no_progress.add(digest)


def _transition_recover(state: HostRunState) -> None:
...
def _generate_with_tools_impl(
    ...
    *,
    ...
    if not implementation:
        return _generate_generic_tool_loop(
            router,
            config=config,
            adapter=adapter,
            request=request,
    ...
    prior_mutation = mutation_history_applied(messages)
    state.workspace_changed = bool(prior_mutation)

    if prior_mutation and all_names & _VERIFY_TOOLS:
    ...
    emit_root_cause(
        ...
        stage=stage,
        operation="generate_with_tools",
        gate="host_target_authority",
        result="PASS",
        details={
            "role": role,
            "implementation_requires_mutation": True,
            "mutation_ready": mutation_ready,
            "target_pinned": bool(state.mutation_context and state.mutation_context.target_pinned),
            "prior_mutation": bool(prior_mutation),
            "initial_phase": state.phase.value,
            "retrieval_is_authority_gate": False,
        },
    )
    ...
    while True:
        ...
        if state.phase == LoopPhase.RECOVER:
            previous = state.phase
            _transition_recover(state)
            emit_root_cause(
                "tool_loop_recover_transition",
                stage=stage,
                operation="generate_with_tools",
                gate="runtime_recovery",
                result="RETRY",
            ...
            if state.last_failure_reason:
                messages.append({
                    "role": "system",
                    "content": (
                        "MMM_RUNTIME_RECOVERY\n"
                        f"The previous tool/runtime action failed: {state.last_failure_reason}. "
                        "The host mutation target remains authoritative. Retry the required phase with corrected "
                        "tool arguments; do not use retrieval as a write gate."
                    ),
                })
            continue

        if state.workspace_changed and state.validation_status == "PASS":
        ...
        loc_before = state.mutation_context.localization_stage.value if state.mutation_context else LocalizationStage.NEED_FILE.value
        ...
        if state.phase == LoopPhase.VERIFY:
            ...
            if forced_verifier is None:
                if state.workspace_changed and not (all_names & _VERIFY_TOOLS):
                    state.validation_status = "PASS"
                    continue
                raise ModelConfigurationError("VERIFIER_UNAVAILABLE: no healthy verifier remains for the mutated workspace.")
            phase_tools = tuple(schema for schema in phase_tools if _tool_name(schema) == forced_verifier)

        if state.phase == LoopPhase.ACT and not phase_tools:
            raise ModelConfigurationError("MUTATION_TOOL_UNAVAILABLE: no reviewed source mutation tool is exposed.")

        if state.phase == LoopPhase.OBSERVE:
            if is_mutation_ready(messages, state):
                state.phase = LoopPhase.ACT
                continue
            if not phase_tools:
                raise ModelConfigurationError(
                    "MUTATION_LOCALIZATION_STALLED: target is not host-bound and no untried localization route remains."
                )
        ...
        if state.phase == LoopPhase.ACT:
            ...
            elif state.mutation_context and state.mutation_context.target_pinned:
                messages.append({
                    "role": "system",
                    "content": (
                        "MMM_HOST_TARGET_AUTHORITY_V1\n"
                        "The host has already bound the mutation target. Retrieval is not required before writing. "
                        "Perform exactly one bounded source mutation against the host-authorized path, then let the host verify it."
                    ),
                })
        elif forced_verifier:
            tool_choice = {"type": "function", "function": {"name": forced_verifier}}
            parallel = False
        elif state.phase == LoopPhase.OBSERVE and len(phase_names) == 1:
        ...
        turn_request = replace(request, tools=phase_tools, tool_choice=tool_choice, parallel_tool_calls=parallel)
        turn = _generate_turn_with_context_recovery(
            router,
            config=config,
            adapter=adapter,
        ...
        if rejection is not None:
            ...
            for payload in rejection_payloads:
                state.record_failure(
                    ...
                    str(payload.get("error") or payload.get("failure_code") or "model tool call rejected"),
                )
        ...
        if not turn.tool_calls:
            ...
            messages.extend([
                ...
                {
                    "role": "system",
                    "content": f"Phase {state.phase.value} requires the exposed tool action. Prose cannot complete this implementation.",
                },
            ])
            continue

        if forced_verifier and (len(turn.tool_calls) != 1 or turn.tool_calls[0].name != forced_verifier):
            called = ", ".join(call.name for call in turn.tool_calls) or "<none>"
            raise ModelConfigurationError(f"VERIFIER_PROTOCOL_VIOLATION: expected {forced_verifier!r}; received {called}.")

        messages.append(_assistant_tool_message(turn))

        def is_evidence_tool(call: Any) -> bool:
            return state.phase == LoopPhase.OBSERVE and (
        ...
        def execute(call: Any) -> tuple[Any, Mapping[str, Any]]:
            ...
            if call.name not in phase_names:
                return call, {
                    ...
                    **metadata,
                    ...
                    "error": f"PHASE_PROTOCOL_VIOLATION: {call.name!r} is not allowed in {state.phase.value}; allowed={sorted(phase_names)}",
                }
            ...
            target_error = _mutation_target_error(call.name, call.arguments, state.mutation_context)
            if target_error:
                return call, {
                    "ok": False,
                    "tool": call.name,
            ...
            try:
                ...
                return call, {"ok": True, "tool": call.name, **metadata, "result": result}
            except Exception as exc:
                if is_evidence_tool(call):
                    state.record_query(call.name, call.arguments)
                error = f"{type(exc).__name__}: {exc}"
        ...
        for call, payload in executed:
            ...
            if call.name in _VERIFY_TOOLS:
                ...
                if status in {"VERIFIER_ARGUMENT_INVALID", "VERIFIER_TARGET_INVALID"}:
                    raise ModelConfigurationError(f"{status}: {payload.get('error', '')}")
                if status == "UNAVAILABLE":
                    ...
                    state.phase = LoopPhase.VERIFY
                    continue
                changed = state.record_verification(call.name, payload, status)
                progress = progress or changed
                if status == "FAIL":
                    state.record_failure(call.name, "verification reported source defects")
                    state.phase = LoopPhase.ACT if state.mutation_context and state.mutation_context.is_mutation_ready else LoopPhase.OBSERVE
                elif status == "PASS":
                    state.clear_failure()
                continue
            ...
            if is_evidence_tool(call):
                if not bool(payload.get("ok")):
                    state.record_failure(call.name, payload.get("error", "localization tool failed"))
                    if str(payload.get("failure_code") or "") == "TOOL_RUNTIME_UNAVAILABLE":
                        state.phase = LoopPhase.RECOVER
                    continue
        ...
        loc_after = state.mutation_context.localization_stage.value if state.mutation_context else LocalizationStage.NEED_FILE.value
...
_mmm_host_target_direct_act_v2 = True
