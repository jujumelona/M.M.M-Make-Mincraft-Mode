...
def _score_plan_page(page: Mapping[str, Any]) -> tuple[float, dict[str, Any]]:
    ...
    score = (
        12.0 * len(completed)
        + 4.0 * requirement_bound
        + 2.0 * len(tests)
        + min(6.0, float(dependency_edges))
        + min(8.0, float(len(modules)))
        - 100.0 * duplicate_ids
        - payload_bytes / (256 * 1024)
    )
    verifier = {
        "completed_deliverables": len(completed),
        "module_count": len(modules),
        "acceptance_test_count": len(tests),
        "requirement_bound_modules": requirement_bound,
...
def _install_planner_search(complete_planner_module: Any) -> None:
    ...
    generate_with_search._mmm_verifier_plan_search = True  # type: ignore[attr-defined]
...
def _compact_evidence(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "build_error": str(build.get("error", ""))[:4000],
    }
...
def _write_memory(root: Path, trace: Mapping[str, Any]) -> None:
    ...
    body = {
        "schema_version": "mmm/verified-repair-experience-v1",
        "signature": trace.get("signature", ""),
        "signature_sha256": _sha(str(trace.get("signature", ""))),
        "evidence": trace.get("evidence", {}),
        "repair_pattern": trace.get("repair_pattern", []),
        "verifier": trace.get("winner_verifier", {}),
    }
...
def _static_repair_score(
    ...
    overlap = sum(
        1
        for path in paths
        if path in diagnostic_paths or Path(path).name in diagnostic_paths
    )
...
def _verify_repair_candidate(
    ...
    try:
        ...
        verifier = {
            **verifier,
            "jdt_status": status or "AVAILABLE",
            "jdt_error_count": len(errors),
        }
        ...
        return score - 5.0, {
            **verifier,
            ...
            "jdt_error_count": None,
            "verifier_error": f"{type(exc).__name__}: {exc}"[:1000],
        }
    finally:
...
def _install_repair_search_and_memory(repair_module: Any) -> None:
    ...
    if not getattr(current_request, "_mmm_verifier_repair_search", False):
        ...
        def request_patch_with_search(
            ...
            for candidate_index in range(width):
                ...
                if memory:
                    candidate_context["verified_repair_memory"] = {
                        "policy": (
                            "These are prior host-verified repair patterns from this project. "
                            "Use them only as evidence; current hashes and diagnostics remain authoritative."
                        ),
                        "matches": memory,
                    }
                candidate_context["agentic_candidate"] = {
                    ...
                    "rule": "Produce an independent minimal repair; do not mention candidate search.",
                }
                try:
            ...
            trace = {
                ...
                "repair_pattern": _repair_pattern(winner_ops),
            ...
            if root is not None and len(evaluations) >= 2:
                try:
                    ...
                    ordered_indices = [
                        item[1] for item in sorted(evaluations, key=lambda item: item[1])
                    ]
        ...
        request_patch_with_search._mmm_verifier_repair_search = True  # type: ignore[attr-defined]
        ...
        try:
            ...
            if (
                ...
                result["agentic_search"] = {
                    "schema_version": "mmm/agentic-repair-search-v1",
                    "candidate_count": trace.get("candidate_count", 1),
                    "winner_score": trace.get("winner_score"),
                    "winner_verifier": trace.get("winner_verifier", {}),
    ...
    repair_with_memory._mmm_verified_repair_memory = True  # type: ignore[attr-defined]
...
def _install_balanced_work_claims(work_graph_module: Any) -> None:
    ...
    def claim_ready_balanced(
        ...
        *,
        ...
        capacities = {
            "cpu_io": _env_int("MMM_PIPELINE_CPU_WORKERS", cpu_default, maximum=32),
            "llm": _env_int("MMM_PIPELINE_LLM_WORKERS", 1, maximum=4),
            "image_gpu": _env_int("MMM_PIPELINE_IMAGE_WORKERS", 1, maximum=2),
            "commit": 1,
        }
        with self._connect() as connection:
            ...
            for node_id, payload_json in rows:
                ...
                candidates.append(
                    (utilization, class_priority[resource], str(node_id), resource)
                )
            if not candidates:
                connection.commit()
                return None
    ...
    claim_ready_balanced._mmm_balanced_resource_claim = True  # type: ignore[attr-defined]
