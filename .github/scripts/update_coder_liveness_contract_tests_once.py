...
text = output_test.read_text(encoding="utf-8")
pattern = re.compile(
    r"def test_output_boundary_loop_is_tool_enabled_checkpointed_and_fixed_point_bounded\(\) -> None:\n.*?\n(?=\Z)",
    re.S,
)
replacement = '''def test_output_boundary_is_tool_enabled_checkpointed_and_single_owner() -> None:
...
if count != 1:
    raise SystemExit(f"output boundary contract replacement count={count}")
output_test.write_text(text, encoding="utf-8")


efficiency_test = Path("tests/test_custom_module_structural_repair_efficiency.py")
text = efficiency_test.read_text(encoding="utf-8")
pattern = re.compile(
...
replacement = '''def test_output_exhaustion_does_not_restart_host_run_state(
    tmp_path: Path,
) -> None:
...
text, count = pattern.subn(replacement, text, count=1)
if count != 1:
    raise SystemExit(f"structural continuation contract replacement count={count}")
efficiency_test.write_text(text, encoding="utf-8")
