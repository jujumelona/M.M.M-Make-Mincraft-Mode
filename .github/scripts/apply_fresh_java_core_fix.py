...
INIT = ROOT / "minecraft_mod_ai" / "__init__.py"
INSTALLER = ROOT / "minecraft_mod_ai" / "fresh_java_grounding_transition_installation.py"
TEST = ROOT / "tests" / "test_fresh_java_grounding_transition.py"
WORKFLOW = ROOT / ".github" / "workflows" / "one-shot-fresh-java-core-fix.yml"
...
'''
    if helpers not in text:
        if text.count(anchor) != 1:
            raise RuntimeError(f"helper anchor: expected 1, found {text.count(anchor)}")
...
'''
    text = replace_once(text, old_ready, new_ready, label="fresh Java READY tool selection")
    ...
    new_progress = '''                fresh_java_grounding_progress = _fresh_java_code_rag_progress(
                ...
                if (
                    ...
                    if (
                        ...
                        state.phase = LoopPhase.ACT
'''
    text = replace_once(text, old_progress, new_progress, label="fresh Java evidence progress")

    LOOP.write_text(text, encoding="utf-8")


def remove_runtime_patch_layer() -> None:
