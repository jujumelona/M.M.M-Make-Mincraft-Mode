...
# Keep the request-atomic snapshot optimization while preserving the shared dynamic
# content contract that imports this symbol from agent_capability_context.
replace(
    "minecraft_mod_ai/agent_capability_context.py",
    "from .agent_roles import AgentRoleRoute, load_agent_role_routes\n",
    "from .agent_roles import AgentRoleRoute, load_agent_role_routes, skills_for_model_role\n",
)

# Prepare the tool surface and context from one role-policy snapshot.
replace(
...
# Preserve serial mutation barriers while allowing external MCP calls whose explicit
# access ceiling is read-only to join existing read waves.
replace(
    "minecraft_mod_ai/model_router.py",
...
# The temporary-skill dedup wrapper must use the scheduler's live classification;
# otherwise it silently serializes new read-only tool classes before the scheduler.
replace(
...
# Parallel read calls can hit the lazy bridge initializer concurrently.
replace(
...
# Make deliberate cleanup boundaries and async CM types explicit.
replace(
...
# Clean small control-flow issues and bind per-turn closure values at definition time.
replace(
