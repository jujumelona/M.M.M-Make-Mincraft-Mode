...
```yaml
activate_when:
- An approved immutable proposal requires a screen, menu, screen handler, custom payload, client/server synchronization, or
  GUI-triggered gameplay action.
...
retry_policy:
  max_attempts: null
  strategy: repair only the failing registration, codec, authority, synchronization, client/server placement, Java, build,
    GameTest or runtime slice from fresh diagnostics
...
exit_conditions:
  success:
  - Client/server placement, payload registration/codecs, server authority, synchronization, Java diagnostics, build, required
    GameTests and runtime logs pass against the final persisted hashes.
  blocked:
