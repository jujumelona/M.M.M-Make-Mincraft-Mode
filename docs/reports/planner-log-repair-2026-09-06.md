...
## 추가 로그 및 즉시 업로드 검증 (두 번째 수정)

새로 전달된 전체 로그는 617줄(마지막 줄의 개행 없음)이며 SHA-256은 `c45e438a3939fedd5339e7ddede9361f142e9b52a22749bd77fc495a35c764ff`다. 마지막 실패는 구현 hole 재시도 응답의 JSON 구문 오류(102행 69열)다. 앞서 분석한 1,182줄 자료와 구분한다.

- 구현 hole 응답에서 중첩 JSON 출력을 제거했다. 번호가 붙은 텍스트 블록을 호스트가 검증하여 실제 hole ID와 연결하고 JSON을 조립한다. 따옴표, 역슬래시가 JSON 구문을 깨뜨리지 않는다.
- 한 페이지는 최대 16개 hole이다. 정상 블록은 보존하고 미완성 블록만 한 번 재시도한다. 끝내 불완전한 응답은 원문과 누락 ID를 기록하고 실패한다. 호스트가 구현 내용을 지어내지 않는다.
- 출력 예산은 분류 `128 + 96 × leaf 수`, 구현 `128 + 256 × hole 수`, 연구 facet 판정 768, 짧은 설계 필드 256, 기타 설계 필드 1,024 token으로 제한한다. 이는 측정된 최적값이 아닌 명시적 초기 정책이며 실제 모델 성능은 후속 실측이 필요하다.
- 관련 회귀 테스트 43개 통과. 잘린 블록, 깨진 재시도, 따옴표·역슬래시 보존, 정상 이웃 보존, 동적 출력 예산 제한과 예외 시 복구를 포함한다. Python 치명적 문법/이름 lint 통과.
- 원격 main의 동시 수정도 병합했다. 이전 154개 작업 재현은 현재 직접 템플릿 정책에서 142개가 된다. 제거된 선행 단계의 의존성 도달 가능성은 유지된다. 병합 검증 57개 통과에는 이전에 실패하던 evidence planning/session 테스트도 포함된다. 위 초기 보고서의 기존 실패 목록은 당시 스냅샷 기록이다.

아직 이 추가 수정으로 GPU 실측, 전체 호출 수 감소, semantic 원자성, 모든 설계 대안의 의미 검증까지 입증한 것은 아니다. 각 항목의 후속 수정과 검증 결과를 별도로 기록한다.
