...
- Minecraft runtime operates only on disposable approved-target instances.
- The server command channel is a regular-expression allowlist, not a shell.
- Mineflayer connects only to localhost and is given the explicit host-selected protocol/version.
